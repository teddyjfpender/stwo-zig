...
pub const configure = [_]Configure{
    ...
    .{ .scope = .riscv_metal, .role = .product, .product_ids = &.{"stwo-riscv-metal"}, .module_roots = &.{ "src/riscv_metal_bench_cli.zig", "src/products/riscv_metal/root.zig", "src/integrations/riscv_metal/mod.zig", "src/tests/riscv/metal_backend_test.zig" }, .dependency_module_roots = &.{ "src/backends/metal_surface.zig", "src/frontends/riscv/mod.zig" }, .external_tools = &.{ "python3", "xcrun" }, .runtime_probes = &.{ "Metal.framework", "Foundation.framework", "libobjc" }, .constructors = &.{"products/matrix.construct.riscv_metal"} },
    ...
    .{ .scope = .cuda_tools, .role = .backend_tools, .product_ids = &.{"stwo-native-cuda-tools"}, .module_roots = &.{"build_support/backends/cuda.zig"}, .allowed_module_prefixes = &.{ "build_support/backends", "src/backends/cuda" }, .external_tools = &.{ "cargo", "python3" }, .constructors = &.{"backends/cuda_tools.addProducts"} },
