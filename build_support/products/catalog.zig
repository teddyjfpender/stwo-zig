...
pub const configure = [_]Configure{
    ...
    .{ .scope = .cuda_tools, .role = .backend_tools, .product_ids = &.{"stwo-native-cuda-tools"}, .module_roots = &.{"build_support/backends/cuda.zig"}, .allowed_module_prefixes = &.{"build_support/backends"}, .external_tools = &.{"python3"}, .constructors = &.{"backends/cuda_tools.addProducts"} },
