...
pub const Options = struct {
    ...
    cuda_build_jobs: ?u16,
    cuda_cumetal_clang: ?[]const u8,
    cuda_cumetalc: ?[]const u8,
    cuda_cumetal_root: ?[]const u8,
    cuda_cumetal_library: ?[]const u8,
    cuda_cumetal_archiver: ?[]const u8,
    cuda_air_inspect: ?[]const u8,
    cuda_air_validate: ?[]const u8,
    ...
    pub fn read(b: *std.Build) Options {
        ...
        return .{
            ...
            .cuda_cumetal_clang = b.option(
                []const u8,
                "cuda-cumetal-clang",
                "Explicit Clang CUDA frontend for the CuMetal provider",
            ),
            ...
            .cuda_cumetal_root = b.option(
                []const u8,
                "cuda-cumetal-root",
                "Pinned CuMetal checkout used by the optional macOS audit lane",
            ),
            .cuda_cumetal_library = b.option(
                []const u8,
                "cuda-cumetal-library",
                "Explicit CuMetal runtime dynamic library",
            ),
            .cuda_cumetal_archiver = b.option(
                []const u8,
                "cuda-cumetal-ar",
                "Explicit Darwin archiver for the CuMetal provider",
            ),
            .cuda_air_inspect = b.option(
                []const u8,
                "cuda-air-inspect",
                "Explicit CuMetal air_inspect executable",
            ),
            .cuda_air_validate = b.option(
                []const u8,
                "cuda-air-validate",
                "Explicit CuMetal air_validate executable",
            ),
...
fn addCudaArguments(
    ...
    for ([_]StringOption{
        ...
        .{ .name = "cuda-cumetal-clang", .value = options.cuda_cumetal_clang },
        .{ .name = "cuda-cumetalc", .value = options.cuda_cumetalc },
        .{ .name = "cuda-cumetal-root", .value = options.cuda_cumetal_root },
        .{ .name = "cuda-cumetal-library", .value = options.cuda_cumetal_library },
        .{ .name = "cuda-cumetal-ar", .value = options.cuda_cumetal_archiver },
        .{ .name = "cuda-air-inspect", .value = options.cuda_air_inspect },
        .{ .name = "cuda-air-validate", .value = options.cuda_air_validate },
