use crate::model::{
    Cli, CudaKernelPolicy, Example, FriConfigWire, Mode, PcsConfigWire, ProveMode, ProverBackend,
};
...
pub(crate) fn parse_cli(args: Vec<String>) -> Result<Cli> {
    ...
    let mut cuda_kernel_policy = CudaKernelPolicy::StrictAot;
    ...
    while i < args.len() {
        ...
        match flag.as_str() {
            ...
            "--cuda-kernel-policy" => {
                cuda_kernel_policy = match value.as_str() {
                    "strict-aot" => CudaKernelPolicy::StrictAot,
                    "jit-diagnostic" => CudaKernelPolicy::JitDiagnostic,
                    _ => bail!("invalid CUDA kernel policy {value}"),
                }
            }
...
mod tests {
    ...
    use crate::model::{CudaKernelPolicy, FriConfigWire, PcsConfigWire, ProverBackend};
    ...
    fn backend_selection_is_explicit_and_scalar_by_default() {
        ...
        .unwrap();
        assert_eq!(cuda.backend, ProverBackend::Cuda);
        assert_eq!(cuda.cuda_kernel_policy, CudaKernelPolicy::StrictAot);

        let diagnostic = parse_cli(vec![
            "tool".into(),
            "--mode".into(),
            "bench".into(),
            "--artifact".into(),
            ...
            "--backend".into(),
            "cuda".into(),
            "--cuda-kernel-policy".into(),
            "jit-diagnostic".into(),
        ])
        .unwrap();
        assert_eq!(
            diagnostic.cuda_kernel_policy,
            CudaKernelPolicy::JitDiagnostic
        );
    }
}
