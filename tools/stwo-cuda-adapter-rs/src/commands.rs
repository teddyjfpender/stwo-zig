...
fn run_cuda<T>(cli: &Cli, operation: impl FnOnce() -> Result<T>) -> Result<T> {
    use crate::model::CudaKernelPolicy;

    if !stwo_backend_cuda_kernels::CUDA_KERNELS_BUILT {
        bail!("CUDA adapter was built without the native CUDA kernel archive");
    }
    stwo_backend_cuda::aot::reset_runtime_stats();
    if cli.cuda_kernel_policy == CudaKernelPolicy::StrictAot {
        if stwo_backend_cuda::aot::loaded_kernel_count() == 0 {
            bail!("strict CUDA admission requires a nonempty copied AOT pack");
        }
    ...
    let receipt = CudaKernelReceipt {
        schema: "stwo-zig-cuda-kernel-receipt-v1",
        policy: match cli.cuda_kernel_policy {
            CudaKernelPolicy::StrictAot => "strict-aot",
            CudaKernelPolicy::JitDiagnostic => "jit-diagnostic",
        },
        aot_loads: stats.aot_loads,
        aot_cache_hits: stats.aot_cache_hits,
    ...
    if cli.cuda_kernel_policy == CudaKernelPolicy::StrictAot
        && (stats.aot_misses != 0
            || stats.runtime_loads != 0
            || stats.runtime_cache_hits != 0
            || stats.strict_rejections != 0)
    {
        bail!("strict CUDA proof rejected a missing or non-AOT generated kernel");
    }
    if stats.aot_loads + stats.aot_cache_hits + stats.runtime_loads + stats.runtime_cache_hits == 0 {
        bail!("CUDA proof resolved no generated constraint kernel");
    }
    result
}
...
macro_rules! selected_backend {
    ($cli:expr, $function:ident ( $($argument:expr),* $(,)? )) => {
        match $cli.backend {
            ...
            crate::model::ProverBackend::Cuda => {
                run_cuda($cli, || $function::<CudaBackend>($($argument),*))
            },
        }
    };
}
...
mod tests {
    ...
    fn config() -> PcsConfig {
        PcsConfig {
            pow_bits: 3,
            fri_config: FriConfig::new(1, 2, 5, 1),
