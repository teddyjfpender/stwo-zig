...
from scripts import riscv_benchmark_matrix_runner as matrix_runner
...
class HostEnvironmentTests(unittest.TestCase):
    ...
    def test_field_set_stays_exactly_what_the_matrix_contract_admits(self) -> None:
        ...
        through ``exact_fields``, and three harnesses embed this same block -- so
    ...
    def test_every_harness_that_publishes_timings_records_power_conditions(self) -> None:
        ...
        self.assertIs(csp_host.power_evidence_block, matrix_runner.power_evidence_block)
...
class HostEnvironmentContractTests(unittest.TestCase):
    ...
    def test_report_validation_reaches_the_host_block_validator(self) -> None:
        # A validator no caller reaches is the defect this whole task is about.
        # Behavioural coverage of `validate_host_environment` cannot show that
        # `validate_report` still calls it, so the call site is asserted directly.
        #
        # KNOWN GAP (issue #152): this is a source-text pin. A verifier defeated
        # it by keeping the text byte-identical and wrapping the call in
        # `if not True:`, leaving the suite green. Driving `validate_report` to
        # the call is not available here -- it runs `exact_fields` first and
        # refuses any report short of a complete one -- so closing this needs a
        # complete report fixture, tracked rather than bodged.
        source = inspect.getsource(contract.validate_report)
        self.assertIn(
            'validate_host_environment(root["host_environment"]', source,
        )
    ...
    def test_each_publishing_harness_captures_power_before_it_measures(self) -> None:
        ...
        for label, entry in (
            ...
            ("staged matrix", matrix_runner.produce),
        ):
    ...
    def test_a_v1_block_is_refused(self) -> None:
        ...
        with self.assertRaisesRegex(contract.MatrixContractError, "fields drifted"):
            contract.validate_host_environment(env, "host")

    def test_a_v1_schema_string_on_a_v2_shape_is_refused(self) -> None:
        with self.assertRaisesRegex(contract.MatrixContractError, "schema drifted"):
    ...
    def test_a_verdict_that_does_not_follow_from_the_evidence_is_refused(self) -> None:
        ...
        with self.assertRaisesRegex(
            contract.MatrixContractError, "does not follow from",
        ):
