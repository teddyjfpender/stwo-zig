...
from scripts import ethereum_bounded_verifier as bounded_verifier
from scripts.zig_serial_build import build_lock


def validate_leaf_receipt(receipt: dict, index: int, leaf: dict, metadata_path: Path, plan: dict) -> None:
    """Bind a successful, separately pinned verifier execution to its campaign."""
    metadata = store.read_regular(metadata_path, "leaf metadata", maximum=128 * 1024)
    protocol.require(protocol.canonical_bytes(json.loads(metadata)) == protocol.canonical_bytes(leaf),
                     "published leaf differs from verified metadata")
    claim_admission = plan["claim_admission"]
    count, workers = plan["segment_count"], plan["workers"]
    ...
    protocol.require(receipt["endpoint"] == "verified_native_selected_leaf"
                     ...
                     and bytes(receipt["metadata_file_sha256"]).hex() == protocol.sha256_bytes(metadata)
...
def run(*, prover: Path, verifier: Path, materialization_path: Path,
        ...
        verification_policy: Path | None = None,
        verification_policy_sha256: str | None = None) -> dict:
    ...
    protocol.require((verification_policy is None) == (verification_policy_sha256 is None),
                     "bounded verification requires both policy path and independent pin")
    verification_schedule = None
    if verification_policy is not None:
        protocol.require(claim_admission == "fixed_program_narrow_v5" and workers == 1,
                         "bounded verification requires fixed-program v5 and one worker")
        verification_schedule = bounded_verifier.load_policy(verification_policy, verification_policy_sha256)
        protocol.require(verification_schedule["verifier"]["path"] == str(verifier.resolve())
                         and verification_schedule["materialization"]["path"] == str(materialization_path.resolve()),
                         "bounded verifier scheduling admission differs from campaign")
    aot_files = {}
    if backend == "metal":
    ...
    with (output / "controller.lock").open("a+b") as lock:
        ...
        for index in range(count):
            ...
            verify_request = {"argv": argv, "plan_sha256": protocol.sha256_bytes(protocol.canonical_bytes(plan))}
            if verification_schedule is not None:
                # Scheduling is per-attempt custody, separate from the immutable
                # proof campaign. Existing accepted artifacts keep their identity.
                verify_request["verification_policy_sha256"] = verification_policy_sha256
            publish(verify_attempt / "request.json", verify_request)
            if verification_schedule is None:
                ...
                observation = {"policy_sha256": verification_policy_sha256}
                try:
                    with (verify_attempt / "stdout.json").open("xb") as stdout, (verify_attempt / "stderr.log").open("xb") as stderr:
                        process_started = time.monotonic_ns()
                        verified = bounded_verifier.run(argv, verification_schedule, stdout=stdout, stderr=stderr,
                                                        timeout=timeout_seconds, observation=observation)
                        process_ns = time.monotonic_ns() - process_started
                finally:
                    publish(verify_attempt / "scheduling.json", observation)
            publish(verify_attempt / "execution.json", {"exit_code": verified.returncode, "process_ns": process_ns})
            check_binary(verifier, "verifier")
            store.validate_file_identity(output / f"{proof_sha}.bin",
                                         {"bytes": leaf["proof_bytes"], "sha256": proof_sha}, "freshly verified full leaf")
...
def main() -> None:
    ...
    parser.add_argument("--verification-policy", type=Path, help="Explicit measured bounded native verifier lane; producers and final bundle remain serialized")
    parser.add_argument("--verification-policy-sha256", help="Independent pin for the bounded verification scheduling policy")
