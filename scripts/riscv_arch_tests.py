...
def _run_json(
    ...
    completed = _run_checked(command, label=label, timeout=timeout)
    try:
...
def _materialize_adapter(
    ...
    for override_path in profile.sail_overrides:
        ...
        _deep_merge(sail_config, override)
...
def _expected_sources(arch_source: Path) -> set[Path]:
...
def _prepend_tool_paths(clang: Path) -> None:
