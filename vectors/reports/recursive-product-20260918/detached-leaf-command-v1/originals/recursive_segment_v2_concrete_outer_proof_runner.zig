...
fn run(comptime Metal: type) !void {
    ...
    while (at < args.len) : (at += 2) {
        ...
        if (std.mem.eql(u8, key, "--native-ingress-profile")) {
            ...
            std.debug.print("usage: {s} [--check-workload | --check-memory-workload | --check-two-segment-workload | --check-segment-ladder | (--native-steps 1|4|16|64 | --memory-addresses 1|4|16 [--segments-output NEW_DIRECTORY [--segment-count 1|2|4|8] [--initial-memory-word U32] [--child-0-key PATH --child-0-key-sha256 SHA256 --child-1-key PATH --child-1-key-sha256 SHA256]]) [--proof-profile development_q3_v1|recursive_q193_v1 (segment output only)] [--native-ingress-profile development_q1|protocol_v1 (memory workload only, no outer proof)] [--initial-register7 U32] [--native-backend cpu|metal] [--recursive-backend cpu|metal (segment output only)] [--aot-bundle PATH --aot-manifest-sha256 SHA256 [--aot-profile core-v2|recursive-framework-v1]]]\n", .{args[0]});
