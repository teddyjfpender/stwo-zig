...
out=root/'vectors/reports/recursive-product-20260918/typed-final-authority-v1'
...
for backend in ('cpu','metal'):
 p=Path('/tmp/pr198-product-'+backend+'-typed-final-20260918-v1');d=json.loads((p/'product.json').read_text());t=json.loads((p/'tree/report.json').read_text());assert d['passed'] and t['passed'];snapshots.append(d['source_sha256'])
...
(out/'summary.json').write_text(json.dumps({'passed':True,'production_security_qualified':False,'canonical_tree_cases':384,'canonical_baseline_identical_artifacts':21,'source_ownership_tests':38,'wide_authority_tests_passed':177,'wide_authority_tests_skipped':1,'infrastructure_kinds_admitted':11,'wide_specialization_mutations':14,'wide_direct_constraints':430,'wide_materialization_cuts':426,'wide_ordered_events':4,'wide_interaction_recurrences':2,'native_prover_verifier_admission':True,'profile_tests_passed':21,'inventory_tests_passed':2,'canonical_merkle_profile_admission':True,'allocation_failure_sweep_passed':True},indent=2)+'\n')
shutil.copyfile('/tmp/pr198-save-typed-final-v1.py',out/'save-evidence.py')
print('Saved final typed infrastructure qualification: 384 proof checks; 21 identical artifacts.')
