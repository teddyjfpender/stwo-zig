...
const legacy_mul = @import("air/qm31_mul_full.zig");
...
pub const Definitions = struct {
    ...
    legacy_mul_definition: legacy_mul.Definition = undefined,
    ...
    pub fn initLegacyMul(self: *Definitions) !void {
        std.debug.assert(!self.legacy_mul_initialized);
        self.legacy_mul_definition = try legacy_mul.build(self.allocator, .generated);
        self.legacy_mul_initialized = true;
    }
    ...
    pub fn deinit(self: *Definitions) void {
        ...
        if (self.legacy_mul_initialized) self.legacy_mul_definition.deinit();
