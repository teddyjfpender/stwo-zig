...
pub const OwnedComponentsV1 = opaque {
    ...
    pub fn init(allocator: std.mem.Allocator, manifest: *const manifest_mod.Manifest, parameters: ParametersV1, relations: *const Relations, claims: ClaimsV1) !*Self {
        ...
        if (usesLegacyPoseidon(manifest)) {
            storage.legacy_poseidon = try initPoseidon(false, storage, parameters, claims);
            try append(storage, 34, storage.legacy_poseidon.asVerifierComponent());
        } else {
            storage.poseidon = try initPoseidon(true, storage, parameters, claims);
    ...
    fn initLogical(storage: *Storage, comptime entry: catalog_entry.Entry, comptime index: usize, parameters: ParametersV1, claims: ClaimsV1) !void {
        ...
        if (comptime row == 30) {
            if (usesLegacyMul(manifest)) {
                ...
                const relation = try binding.Binding(@import("air/qm31_mul_full.zig")).authenticate(&storage.definitions.legacy_mul_definition);
                ...
                try append(storage, row, storage.legacy_mul.asVerifierComponent());
                return;
            }
        }
    ...
    fn initPoseidon(comptime compact: bool, storage: *Storage, parameters: ParametersV1, claims: ClaimsV1) !if (compact) Compact else Wide {
        const placement = try admission.poseidon(manifest_mod, compact, .allow_reviewed_legacy, storage.allocator, &storage.manifest, storage.manifest.placements[34].?.geometry.log_size, parameters.poseidon_active_rows, &storage.providers, &storage.relations, claims.poseidon_partials);
        const component = (if (compact) Compact else Wide){
