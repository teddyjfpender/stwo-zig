...
pub const PreparedV1 = opaque {
    ...
    pub fn initForManifest(allocator: std.mem.Allocator, rows: LogicalRowsV1, calls: []const ProviderCall, admitted_manifest: ?*const manifest_mod.Manifest) !*Self {
        ...
        var poseidon_geometry = PoseidonAdapter.manifestGeometry(try traceLogSize(calls.len));
        if (admitted_manifest) |manifest_value| {
            ...
            if (!Compatible.acceptsGeometry(admitted.geometry) or admitted.geometry.log_size != poseidon_geometry.log_size)
                return error.DetachedParentManifestMismatch;
            poseidon_geometry.semantic_digest = admitted.geometry.semantic_digest;
        }
...
pub fn testSnapshotAdmission() !*PreparedV1 {
    ...
    // Independently sealed retained geometry selects only the reviewed identity.
    ...
    const retained = try PreparedV1.initForManifest(allocator, rows, &.{}, &retained_manifest);
    defer retained.deinit();
    try std.testing.expectEqualSlices(u8, &retained_manifest.seal, &retained.manifest().seal);
    var changed_builder = manifest_mod.Builder{};
    for (retained_manifest.roster_rows[0..retained_manifest.roster_count]) |row| {
        var geometry = retained_manifest.placements[row].?.geometry;
