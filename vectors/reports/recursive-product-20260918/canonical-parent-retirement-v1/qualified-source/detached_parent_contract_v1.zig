...
const CompactGeometry = shared.PoseidonForManifest(manifest_mod, true, .canonical_only);
const RangeGeometry = shared.RangeForManifest(manifest_mod);

// Detached parents admit only the current catalog and canonical provider identity.
...
pub const ParametersV1 = struct {
    ...
    pub fn validate(self: ParametersV1, manifest: *const manifest_mod.Manifest) !void {
        ...
        if (!CompactGeometry.acceptsGeometry(poseidon.geometry) or
    ...
    fn validateRow(self: ParametersV1, comptime entry: air.universal_catalog.Entry, manifest: *const manifest_mod.Manifest) !void {
        ...
        const expected = geometry.manifestGeometryForAir(entry.Air, manifest_mod, entry.row, placement.geometry.log_size);
        const parameter_count = geometry.parameterColumnCount(entry.Air);
