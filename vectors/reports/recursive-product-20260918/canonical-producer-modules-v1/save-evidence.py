...
out=root/'vectors/reports/recursive-product-20260918/canonical-producer-modules-v1'
...
for backend in ('cpu','metal'):
 p=Path('/tmp/pr198-product-'+backend+'-producer-modules-20260918-v1');d=json.loads((p/'product.json').read_text());t=json.loads((p/'tree/report.json').read_text());assert d['passed'] and t['passed'];snapshots.append(d['source_sha256'])
...
(out/'summary.json').write_text(json.dumps({'passed':True,'production_security_qualified':False,'canonical_tree_cases':384,'canonical_baseline_identical_artifacts':21,'source_ownership_tests':35,'removed_facade_exports':5,'dedicated_parent_module':True,'producer_integration_sources':24},indent=2)+'\n')
shutil.copyfile('/tmp/pr198-save-producer-modules-v1.py',out/'save-evidence.py')
print('Saved canonical producer module qualification: 384 proof checks; 21 identical artifacts.')
