...
fn computeImpl(comptime Air: type, comptime typed_authority: bool, allocator: std.mem.Allocator) !Digest {
    ...
    if (typed_authority) {
        ...
        const normalized = try expression.commutativeExpressions(allocator, arena.nodes.items, arena.names.items.len);
        defer allocator.free(normalized);
        for (direct, expected.direct) |actual, required| try requireEquivalent(normalized, actual, required);
        if (lookups.len != expected.lookups.len or lookups.batch_size != expected.lookups.batch_size)
            return error.CompactPoseidonSpecializationMismatch;
        for (lookups.entries[0..lookups.len], expected.lookups.entries[0..expected.lookups.len]) |actual, required| {
            if (actual.domain != required.domain or actual.role != required.role or actual.access_ordinal != required.access_ordinal or actual.arity != required.arity)
                return error.CompactPoseidonSpecializationMismatch;
            try requireEquivalent(normalized, actual.numerator, required.numerator);
            for (actual.values[0..actual.arity], required.values[0..required.arity]) |value, required_value| try requireEquivalent(normalized, value, required_value);
        }
        for (interaction, typed_interaction) |actual, required| try requireEquivalent(normalized, actual, required);
    }
...
fn requireEquivalent(digests: []const Digest, actual: S, required: S) !void {
    if (!std.mem.eql(u8, &digests[actual.id], &digests[required.id])) return error.CompactPoseidonSpecializationMismatch;
}
...
fn Relation(comptime Scalar: type) type {
    return struct {
        z: Scalar,
        alpha: Scalar,
