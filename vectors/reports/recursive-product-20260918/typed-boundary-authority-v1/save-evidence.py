...
out=root/'vectors/reports/recursive-product-20260918/typed-boundary-authority-v1'
...
for backend in ('cpu','metal'):
 p=Path('/tmp/pr198-product-'+backend+'-typed-boundary-20260918-v1');d=json.loads((p/'product.json').read_text());t=json.loads((p/'tree/report.json').read_text());assert d['passed'] and t['passed'];snapshots.append(d['source_sha256'])
...
(out/'summary.json').write_text(json.dumps({'passed':True,'production_security_qualified':False,'canonical_tree_cases':384,'canonical_baseline_identical_artifacts':21,'source_ownership_tests':37,'boundary_tests_passed':156,'boundary_tests_skipped':1,'new_native_kinds':9,'checked_boundary_policies':11,'specialization_mutations':73,'merkle_tests_passed':150,'merkle_tests_skipped':1,'compact_tests_passed':127,'profile_tests_passed':21,'inventory_tests_passed':2,'canonical_merkle_profile_admission':True,'allocation_failure_sweep_passed':True},indent=2)+'\n')
shutil.copyfile('/tmp/pr198-save-typed-boundary-v1.py',out/'save-evidence.py')
print('Saved typed native boundary qualification: 384 proof checks; 21 identical artifacts.')
