...
for backend in ('cpu','metal'):
 p=Path('/tmp/pr198-product-'+backend+'-typed-closure-20260921-v1')
...
subprocess.run([sys.executable,'/tmp/pr198-run-typed-closure-ladder-v1.py'],check=True)
subprocess.run([sys.executable,'/tmp/pr198-save-typed-closure-ladder-v1.py'],check=True)
assert source_snapshot()==snapshot
(out/'summary.json').write_text(json.dumps({'passed':True,'canonical_tree_cases':384,'canonical_baseline_identical_artifacts':21,'continuation_cases':1110,'continuation_baseline_identical_artifacts':78,'focused_package_ownership_checks':49,'production_security_qualified':False,'goal_complete':False},indent=2)+'\n')
shutil.copyfile('/tmp/pr198-qualify-typed-closure-v1.py',out/'qualify.py')
