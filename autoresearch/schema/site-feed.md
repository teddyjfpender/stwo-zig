...
| `promotion_scope` | v2: the decided benchmark set — manifest class registry (scored, resource, timeout, and sampling policy), workload groups (board, enabled, disabled_reason, `promotion_blocked_reason`, `retirement`, per-workload class + native unit), `owned_boards`, `staged_boards`, `retired_boards`, `future_boards`, and committed baseline directories. A board in `future_boards` exists only as scoring universe; consumers render it as out-of-scope, never as empty-but-live. **A retired board is never in `future_boards`** — see `retired_boards` below. Each frontend group also carries its `function_basket` (TRACKS §3.3): the named whole-pipeline benchmark functions, each mapping to rows whose `status` is DERIVED from the vehicle holding the row — `scored` (runnable workload), `staged` (era-gated, admitted), or `pending` (pinned and loudly blocked, with the blocker in the manifest's provisioning reason) — plus the row's class and expected native cycles where committed. Consumers MUST render the basket as the track's benchmark statement; coverage programs and ISA micro-tests are deliberately absent from it |
