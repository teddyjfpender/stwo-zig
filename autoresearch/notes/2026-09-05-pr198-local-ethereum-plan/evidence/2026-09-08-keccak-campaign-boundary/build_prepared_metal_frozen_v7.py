...
repo=Path.cwd();source=repo/'.git/local-ethereum/prepared-leaf-metal-source-v7';product=repo/'.git/local-ethereum/prepared-leaf-metal-product-v7';product.mkdir()
...
check();command=['python3',str(repo/'scripts/zig_serial_build.py'),'--cwd',str(source/'source/src/integrations/riscv_metal'),'test-ethereum-prepared-leaf-metal-v1','install-ethereum-prepared-leaf-metal-v1','-Doptimize=ReleaseSafe','--prefix',str(product),'--summary','all'];(product/'build-plan.json').write_text(json.dumps({'command':command,'source_manifest':ident(manifest)},indent=2)+'\n');start=time.monotonic_ns()
...
exe=product/'bin/ethereum-prepared-leaf-metal-v1'
