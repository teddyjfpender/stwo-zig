...
produced = base / 'metal-field5-segment11-log18-v2'
exe = repo / '.git/local-ethereum/fixed-program-native-verifier-v6/bin/ethereum-full-leaf-bundle-verify-v1'
out = repo / '.git/local-ethereum/selected-real-metal-leaf11-log18-fresh-v2'
...
with open('/tmp/stwo-zig-build.lock', 'a+') as lock:
    ...
    build = json.loads((exe.parent.parent / 'build-receipt.json').read_text())
    if build['exit_code'] != 0: raise SystemExit('Verifier build failed')
    pins = {str(exe): build['executable']['sha256'], str(proof): producer['artifact']['sha256'], str(metadata): producer['global_metadata']['sha256'], str(materialization): 'e9d9ba5619d5780155bf7f23e3475a1af0aae85ec74a0660b837c0cdbb237f4e'}
