import ctypes, fcntl, hashlib, json, os, pathlib, re, signal, subprocess, time
...
cpu_root=campaign/'cpu-field5-segment9-selected-v1'
...
root=campaign/'metal-field5-segment9-selected-v2'; root.mkdir()
admission={'schema':'stwo.stage101-benchmark-admission.v2','artifact_bytes':len(reference_bytes),'artifact_sha256':hashlib.sha256(reference_bytes).hexdigest(),'claim_schema':5,'small_circle_host_placements':0,'manifest_sha256':aot_receipt['manifest_sha256'],'metallib_sha256':aot_receipt['metallib']['sha256']}
admission_path=root/'benchmark-admission-v2.json';admission_path.write_text(json.dumps(admission,indent=2)+'\n')
del reference_bytes
product=repo/'.git/local-ethereum/prepared-leaf-metal-product-v2'
...
exe=pathlib.Path(build_receipt['benchmark_executable']['path'])
if hashlib.sha256(exe.read_bytes()).hexdigest()!=build_receipt['benchmark_executable']['sha256']:raise SystemExit('Frozen Metal5 executable changed')
proof=root/'segment-000009.stwief04'; metadata=root/'segment-000009.metadata.json'
args=[str(exe),'--retained-materialization-result',str(campaign/'authority/materialization-v2.json'),'--publication-root',str(campaign/'capture/publication-parent/ethereum-incremental-capture-v4'),'--selected-leaf-admission-root',str(campaign/'selected-leaf-000009-v1'),'--segment-index','9','--output',str(proof),'--campaign-geometry','authenticated-v1','--claim-admission','fixed_program_narrow_v5','--global-metadata-output',str(metadata),'--pcs-retained-byte-budget',str(24*2**30)]
def ident(path):
...
plan={'schema':'stwo.ethereum.real-native-leaf-measurement.v1','command':args,'policy':policy,'executable':ident(exe),'source_manifest':build_receipt['source_manifest'],'reference':ident(reference),'aot_manifest_sha256':aot_receipt['manifest_sha256'],'started_unix_ns':time.time_ns(),'segment_index':9,'execution_cycles':2097152,'core_rows':2096404,'keccak_calls':682,'signer_calls':66,'claim_admission':'fixed_program_narrow_v5','backend':'authenticated_AOT_Metal','claim':'one native full leaf core plus providers, serialized producer destruction and fresh native verification; not a block or succinct root'}
...
with (root/'stdout.log').open('wb') as out,(root/'stderr-and-time.log').open('wb') as err,(root/'process-samples.ndjson').open('w') as samples:
 ...
 while process.poll() is None:
  ...
  time.sleep(2)
receipt={'schema':plan['schema'],'exit_code':process.returncode,'monitor_elapsed_ns':time.monotonic_ns()-start,'sampled_peak_footprint_bytes':peak,'stopped_for_footprint_limit':stopped,'policy':policy,'verified_and_published':process.returncode==0 and proof.is_file() and metadata.is_file(),'ended_unix_ns':time.time_ns()}
...
receipt['measurement_lines']=[line for line in (root/'stderr-and-time.log').read_text().splitlines() if line.startswith(('INCREMENTAL_FIXED_PROGRAM_GEOMETRY_V1','INCREMENTAL_FULL_LEAF_INPUT_PHASES_V1','INCREMENTAL_FULL_LEAF_PHASES_V1','INCREMENTAL_FULL_LEAF_PREFLIGHT_V1'))]
if receipt['verified_and_published'] and receipt['artifact']['sha256']!=cpu_receipt['artifact']['sha256']:raise SystemExit('Published Metal proof differs from CPU reference')
