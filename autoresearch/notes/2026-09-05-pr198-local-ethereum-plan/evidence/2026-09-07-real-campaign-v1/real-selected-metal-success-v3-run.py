...
root=campaign/'metal-field4-segment9-selected-v3'; root.mkdir()
admission={'schema':'stwo.stage101-benchmark-admission.v2','artifact_bytes':len(reference_bytes),'artifact_sha256':hashlib.sha256(reference_bytes).hexdigest(),'claim_schema':4,'small_circle_host_placements':0,'manifest_sha256':aot_receipt['files']['stwo_zig_core.manifest.json']['sha256'],'metallib_sha256':aot_receipt['files']['stwo_zig_core.metallib']['sha256']}
admission_path=root/'benchmark-admission-v2.json';admission_path.write_text(json.dumps(admission,indent=2)+'\n')
del reference_bytes
exe=repo/'.git/local-ethereum/real-leaf-metal-product-v5/bin/stage101-metal-autoresearch-v1'
if hashlib.sha256(exe.read_bytes()).hexdigest()!='6af7f10b67d6f60cef948e7887fc279802329a36c57e1f2be07f567d87dec2e0':raise SystemExit('Frozen Metal v5 executable identity changed')
