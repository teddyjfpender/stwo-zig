...
| `crates/constraint-framework` | `src/core/constraints.zig` plus dedicated constraint DSL/evaluator modules | Partial/minimal | Port full constraint expression model, quotient-building semantics, batching rules, and evaluation ordering exactly; add law/property tests for algebraic identities and expression rewrites | Differential tests against Rust constraint outputs for fixed seeds/traces; all edge/failure tests green |
| `crates/air-utils` | `src/core/air/**` and supporting trace utilities | Partial/minimal | Port full AIR trace helpers, periodic column behavior, mask/query point construction semantics, and composition evaluation plumbing for prover+verifier paths | Rust parity vectors for AIR utilities and composition checkpoints; full integration tests with proof generation and verification |
| `crates/air-utils-derive` | Zig compile-time generation layer (comptime helpers/macros equivalent) | Not started | Implement Zig comptime equivalents for derive-generated behaviors required by the pinned Rust API surface; ensure generated metadata and wiring semantics match | Compile-time generated outputs parity-tested against Rust expectations; no manual per-AIR boilerplate required beyond declared conformance limit |
| `crates/examples` | `src/examples/**` and e2e fixtures | Partial | Complete full upstream-equivalent proving semantics for `xor` and `state_machine` wrappers, then expand parity to additional selected upstream examples; keep deterministic cross-language proof-exchange fixtures and rejection cases current | `xor` and `state_machine` Rust<->Zig proof exchange (both directions) passes with tamper rejection; every selected Rust example has Zig equivalent and bidirectional verification |
| `crates/std-shims` | freestanding/minimal Zig verifier profile and build flags | Not started | Implement minimal/freestanding compatibility profile for verifier path, allocator and feature boundaries, and deterministic behavior without host-only dependencies | Freestanding verifier build passes conformance test matrix; behavior matches standard build for identical inputs |
