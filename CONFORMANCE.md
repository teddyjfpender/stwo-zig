...
| `crates/examples` | `src/examples/**` (to be added) and e2e fixtures | Not started | Port canonical upstream examples needed for proving and verification comparability; add deterministic fixtures for cross-language proof exchange | Every selected Rust example has Zig equivalent; Rust proof verifies in Zig and Zig proof verifies in Rust |
