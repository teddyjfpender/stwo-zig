...
decomposition meaningful. `half_access_aligned` in
`RiscvRefinement/Opcodes/LoadStore.lean` derives `Memory.isHalfAligned` from
exactly these transcribed constraints. The only alignment-adjacent hypothesis
a load theorem carries is the environment's `baseInFieldRange` machine
premise, which exists because of issue #140, not because alignment is
unproved. (An earlier revision of this note claimed the range check was the
sole mechanism and required an explicit alignment premise; both halves were
wrong.)
...
5. The tracked #140 aliasing row still behaves as documented:
   `scripts/tests/test_riscv_team_b_witnesses.py::TrackedAirGapTest` passes,
   and the witness gate would fail if the row stopped being admitted.
