...
test "exact CUDA Plonk LogUp resident binding is a four-tree relation graph" {
    ...
    const relation_instance = bound.relation.instance();
...
test "exact interaction and composition execute in transcript order" {
    ...
    try std.testing.expectEqual(@as(usize, 1), Calls.inverse_transforms);
...
const FakeOps = struct {
    ...
    pub const Power = struct {
        pub fn expand(
            ...
            try std.testing.expectEqual(@as(usize, 3), powers.len);
