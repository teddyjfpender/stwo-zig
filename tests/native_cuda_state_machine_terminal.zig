...
const exact = stwo.integrations.native_cuda.state_machine;
...
test "exact terminal stages execute one ordered resident transaction" {
    ...
    for (&expected, 9..) |*step, value| step.* = @intCast(value);
    try std.testing.expectEqualSlices(
        u32,
        &expected,
        Calls.steps[0..Calls.step_count],
    );
    // One canonical draw plus one point derivation per mixed-height batch.
    try std.testing.expectEqual(@as(usize, 7), Calls.oods_derivations);
    try std.testing.expectEqual(@as(usize, 7), Calls.oods_evaluations);
    try std.testing.expectEqual(@as(usize, 7), Calls.oods_stores);
    ...
    try std.testing.expectEqual(@as(usize, 3), Calls.trace_preparations);
    try std.testing.expectEqual(@as(usize, 3), Calls.trace_packs);
    try std.testing.expectEqual(@as(usize, 3), Calls.trace_assemblies);
...
test "exact terminal artifact seals four roots and 28 canonical samples" {
    ...
    try std.testing.expectEqual(@as(usize, 11), bundle.decommitment.trees.len);
...
test "exact finish splits one envelope into typed statement and proof" {
    ...
    try std.testing.expectEqual(@as(u32, 8), output.statement.stmt0.n);
    for (
        output.statement.stmt1.x_axis_claimed_sum.toM31Array(),
        [_]u32{ 1, 2, 3, 4 },
    ) |actual, expected| {
...
test "exact CPU proof targets remain frozen for CUDA byte parity" {
    for (exact.parity_targets.functional) |target| {
        try exact.parity_targets.checkCpuOracle(
...
const FinishTransaction = struct {
    ...
    pub fn assembleStarkBundleAndStatementFinishWith(
        ...
        if (slot != exact.slots.proof_bundle or statement_words != 8)
...
fn makeTerminalArtifact(
    ...
    const tree_count: usize = 3 + log_n_rows;
    ...
    for (0..tree_count) |tree_index| {
        ...
        meta[0] = @intFromEnum(if (tree_index < 3)
