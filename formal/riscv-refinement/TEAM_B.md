...
`load_store` AIR now bounds addresses twice: C69 pins the high limb of `rs1`
to zero on active rows, and the aligned-address
...
away. The Lean theorem `base_add_4096_lt_modulus` derives the needed headroom
from C69, so the opcode refinements no longer carry a machine-side address
premise.

**The historical address-aliasing row is rejected.** The effective address is
computed in the M31 base field, so the old AIR admitted base `0x7FFFFFFB` with
displacement `+8` as a read at `0x00000004`, even though the architectural
address is `0x80000003`. The source fix pins the base's high limb to zero on
active rows, bounding the base below `2^24`; the old counterexample now fails
at constraint root C69. `scripts/riscv_team_b_witnesses.py` requires that exact
rejection and also checks that a zero-high-byte control row remains reachable.
...
## Issue #140 integration status

The historical address-aliasing gap is filed as
...
is not modulo 2^32*). Status as of 2026-07-29: the upstream issue is **open**,
and
...
but this issue-136 branch already contains the equivalent source change in
commit `6c3c4989`. Its fresh `load_store` export has SHA-256
`cadb1b662ec30864615aa84541c1bcd863e921f306446ea1c7a328c650180b20`,
342 nodes and 79 constraint roots.

The closure evidence on this branch is:

- `LoadStoreHolds.baseHighLimbZero` transcribes C69 and
  `base_add_4096_lt_modulus` derives the M31 headroom used by every load/store
  refinement;
- `LoadStoreEnvironment` has no `baseInFieldRange` premise, so the theorems
  cover every row admitted by the fixed capsule;
- `check_address_aliasing_rejected` and
  `AddressAliasingRegressionTest` require the historical counterexample to be
  rejected by C69 alone.
