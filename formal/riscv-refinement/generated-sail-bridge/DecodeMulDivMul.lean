...
private theorem ext_decode_mul_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .mul rs2 rs1 rd) =
        generatedMultiplyMTypeGate
          (admittedMTypeInstruction .mul rs2 rs1 rd) fallback := by
...
theorem decode_mul_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .mul rs2 rs1 rd)
      (admittedMTypeInstruction .mul rs2 rs1 rd)
      initial := by
  rcases ext_decode_mul_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
