...
private theorem generated_srai_probe_facts
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5) :
    generatedSlliProbe (encodeAdmittedShiftIType .srai shamt rs1 rd) =
        pure none ∧
      generatedSrliProbe (encodeAdmittedShiftIType .srai shamt rs1 rd) =
        pure none ∧
      generatedSraiProbe (encodeAdmittedShiftIType .srai shamt rs1 rd) =
        pure (some (admittedShiftITypeInstruction .srai shamt rs1 rd)) := by
...
private theorem generated_srai_body_exact
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (fallback : SailM instruction) :
    generatedShiftITypeBody
        (encodeAdmittedShiftIType .srai shamt rs1 rd) fallback =
      pure (admittedShiftITypeInstruction .srai shamt rs1 rd) :=
  generatedShiftITypeBody_srai _ fallback _
    (generated_srai_probe_facts shamt rs1 rd).1
    (generated_srai_probe_facts shamt rs1 rd).2.1
    (generated_srai_probe_facts shamt rs1 rd).2.2

private theorem ext_decode_admitted_srai_branch
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (pauseDisabled : hartSupports extension.Ext_Zihintpause = false) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedShiftIType .srai shamt rs1 rd) =
        generatedShiftITypeGate
          (encodeAdmittedShiftIType .srai shamt rs1 rd) fallback := by
...
theorem decode_admitted_srai_certificate
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (pauseDisabled : hartSupports extension.Ext_Zihintpause = false) :
    GeneratedDecodeCertificate
      (encodeAdmittedShiftIType .srai shamt rs1 rd)
      (admittedShiftITypeInstruction .srai shamt rs1 rd) := by
  constructor
  intro initial final actual outcome
  rcases ext_decode_admitted_srai_branch shamt rs1 rd pauseDisabled with
    ⟨fallback, gateEq⟩
  rw [gateEq] at outcome
  exact generatedShiftITypeGate_success _ fallback
    (admittedShiftITypeInstruction .srai shamt rs1 rd)
    actual initial final
    (generated_srai_body_exact shamt rs1 rd fallback) outcome

theorem decode_admitted_srai_certificate_at
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
  ...
  exact generatedShiftITypeGate_exact_at _ fallback _ initial mseccfgValue
    ...
    (generated_srai_body_exact shamt rs1 rd fallback)
