...
private theorem ext_decode_remu_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .remu rs2 rs1 rd) =
        generatedDivisionMTypeGate
          (admittedMTypeInstruction .remu rs2 rs1 rd) fallback := by
  ...
  rw [
    ...
    encodeAdmittedRemuMType_not_multiply,
    encodeAdmittedRemuMType_unsignedBit,
    encodeAdmittedRemuMType_class,
    mtypeClass3_not_div,
    mtypeClass3_is_rem,
...
theorem decode_remu_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .remu rs2 rs1 rd)
      (admittedMTypeInstruction .remu rs2 rs1 rd)
      initial := by
  rcases ext_decode_remu_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
