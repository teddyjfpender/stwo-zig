...
private theorem ext_decode_rem_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .rem rs2 rs1 rd) =
        generatedDivisionMTypeGate
          (admittedMTypeInstruction .rem rs2 rs1 rd) fallback := by
  ...
  rw [
    ...
    encodeAdmittedRemMType_not_multiply,
    encodeAdmittedRemMType_unsignedBit,
    encodeAdmittedRemMType_class,
    mtypeClass3_not_div,
    mtypeClass3_is_rem,
...
theorem decode_rem_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .rem rs2 rs1 rd)
      (admittedMTypeInstruction .rem rs2 rs1 rd)
      initial := by
  rcases ext_decode_rem_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
