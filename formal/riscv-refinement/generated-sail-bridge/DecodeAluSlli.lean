...
private theorem generated_slli_probe_exact
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5) :
    generatedSlliProbe (encodeAdmittedShiftIType .slli shamt rs1 rd) =
      pure (some (admittedShiftITypeInstruction .slli shamt rs1 rd)) := by
...
private theorem generated_slli_body_exact
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (fallback : SailM instruction) :
    generatedShiftITypeBody
        (encodeAdmittedShiftIType .slli shamt rs1 rd) fallback =
      pure (admittedShiftITypeInstruction .slli shamt rs1 rd) :=
  generatedShiftITypeBody_slli _ fallback _
    (generated_slli_probe_exact shamt rs1 rd)

private theorem ext_decode_admitted_slli_branch
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (pauseDisabled : hartSupports extension.Ext_Zihintpause = false) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedShiftIType .slli shamt rs1 rd) =
        generatedShiftITypeGate
          (encodeAdmittedShiftIType .slli shamt rs1 rd) fallback := by
...
theorem decode_admitted_slli_certificate
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
    (pauseDisabled : hartSupports extension.Ext_Zihintpause = false) :
    GeneratedDecodeCertificate
      (encodeAdmittedShiftIType .slli shamt rs1 rd)
      (admittedShiftITypeInstruction .slli shamt rs1 rd) := by
  constructor
  intro initial final actual outcome
  rcases ext_decode_admitted_slli_branch shamt rs1 rd pauseDisabled with
    ⟨fallback, gateEq⟩
  rw [gateEq] at outcome
  exact generatedShiftITypeGate_success _ fallback
    (admittedShiftITypeInstruction .slli shamt rs1 rd)
    actual initial final
    (generated_slli_body_exact shamt rs1 rd fallback) outcome

theorem decode_admitted_slli_certificate_at
    (shamt : BitVec 5)
    (rs1 rd : BitVec 5)
  ...
  exact generatedShiftITypeGate_exact_at _ fallback _ initial mseccfgValue
    ...
    (generated_slli_body_exact shamt rs1 rd fallback)
