...
private theorem ext_decode_mulh_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .mulh rs2 rs1 rd) =
        generatedMultiplyMTypeGate
          (admittedMTypeInstruction .mulh rs2 rs1 rd) fallback := by
...
theorem decode_mulh_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .mulh rs2 rs1 rd)
      (admittedMTypeInstruction .mulh rs2 rs1 rd)
      initial := by
  rcases ext_decode_mulh_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
