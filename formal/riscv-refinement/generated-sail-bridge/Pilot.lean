...
theorem complete_LUI_normalizes
  ...
  rw [execute_UTYPE_LUI_eq, normalizedRegisterCompletion_erases]
    ...
    completeBaseExecution pc
      ...
      eraseObservation
        (normalizedRegisterCompletion pc rd
          (do
            let source ← rX_bits (.Regidx rs1)
            pure (source + sign_extend (m := 32) imm))) := by
  rw [execute_ITYPE_ADDI_eq, normalizedRegisterCompletion_erases]
  simp [completeBaseExecution, completeRegisterEffects]
