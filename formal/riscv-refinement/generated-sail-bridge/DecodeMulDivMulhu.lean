...
private theorem ext_decode_mulhu_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .mulhu rs2 rs1 rd) =
        generatedMultiplyMTypeGate
          (admittedMTypeInstruction .mulhu rs2 rs1 rd) fallback := by
...
theorem decode_mulhu_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .mulhu rs2 rs1 rd)
      (admittedMTypeInstruction .mulhu rs2 rs1 rd)
      initial := by
  rcases ext_decode_mulhu_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
