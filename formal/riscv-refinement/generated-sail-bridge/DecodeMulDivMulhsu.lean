...
private theorem ext_decode_mulhsu_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .mulhsu rs2 rs1 rd) =
        generatedMultiplyMTypeGate
          (admittedMTypeInstruction .mulhsu rs2 rs1 rd) fallback := by
  ...
  apply Exists.intro
  exact generatedMultiplyMTypeGate_factor _ _

theorem decode_mulhsu_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .mulhsu rs2 rs1 rd)
      (admittedMTypeInstruction .mulhsu rs2 rs1 rd)
      initial := by
  rcases ext_decode_mulhsu_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
  exact generatedMultiplyMTypeGate_exact_at
    (admittedMTypeInstruction .mulhsu rs2 rs1 rd)
