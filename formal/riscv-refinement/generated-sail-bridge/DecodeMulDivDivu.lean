...
private theorem ext_decode_divu_gate
    (rs2 rs1 rd : BitVec 5) :
    ∃ fallback : SailM instruction,
      ext_decode (encodeAdmittedMType .divu rs2 rs1 rd) =
        generatedDivisionMTypeGate
          (admittedMTypeInstruction .divu rs2 rs1 rd) fallback := by
  ...
  rw [
    ...
    encodeAdmittedDivuMType_not_multiply,
    encodeAdmittedDivuMType_unsignedBit,
    encodeAdmittedDivuMType_class,
    mtypeClass2_is_div,
    mtypeClass2_not_rem,
    mtypeUnsignedBit1_matches,
    mtypeUnsignedBit1_value,
...
theorem decode_divu_certificate_at
    (rs2 rs1 rd : BitVec 5)
    ...
    GeneratedDecodeCertificateAt
      (encodeAdmittedMType .divu rs2 rs1 rd)
      (admittedMTypeInstruction .divu rs2 rs1 rd)
      initial := by
  rcases ext_decode_divu_gate rs2 rs1 rd with ⟨fallback, gateEq⟩
  constructor
  rw [gateEq]
