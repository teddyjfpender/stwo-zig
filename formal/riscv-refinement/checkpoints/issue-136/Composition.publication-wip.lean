...
structure AcceptedGeneratedOpcodeComposition
    ...
    (StateBindings ProfileAdmission Admission LocalRefinement ExactTuple
      GeneratedExecuteClause NormalizedRetirement : Prop)
    (stepNo : Nat)
...
/--
Both generated execution clauses selected by the exact FENCE input
classification.  Each branch ends in generated successful retirement; the
ordinary branch retains the exact generated FIOM/barrier dispatch.
-/
    ...
    Functions.execute
      ...
      then Functions.execute_FENCE_TSO ()
        ...
        Functions.execute_FENCE
          ...
          (.Regidx row.rd)
  tsoSuccessfulClause :
    Functions.execute_FENCE_TSO () =
      ((do
        Sail.ConcurrencyInterfaceV1.sail_barrier
          Barrier_RISCV_tso
        pure Functions.RETIRE_SUCCESS) : SailM ExecutionResult)
  ordinarySuccessfulClause :
    Functions.execute_FENCE
        (Sail.BitVec.extractLsb row.immediate 11 8)
        (Sail.BitVec.extractLsb row.immediate 7 4)
        (Sail.BitVec.extractLsb row.immediate 3 0)
        ...
        (.Regidx row.rd) =
      ((do
        let fiom ← Functions.is_fiom_active ()
        let pred :=
          Functions.effective_fence_set
            (Sail.BitVec.extractLsb row.immediate 7 4)
            fiom
        let succ :=
          Functions.effective_fence_set
...
theorem FENCE_accepted_air_refines
  ...
  have decoder :
      Functions.GeneratedDecodeCertificate
        (Functions.encodeFenceTrace
          row.immediate row.rs1 row.rd)
        (Functions.decodedFenceTrace
          row.immediate row.rs1 row.rd) :=
    Functions.decode_fence_certificate
      row.immediate row.rs1 row.rd
  ...
  exact {
    ...
    generatedExecuteSuccess := {
      exactDispatch :=
        Functions.execute_decodedFenceTrace_clause
          row.immediate row.rs1 row.rd
      tsoSuccessfulClause :=
        Functions.execute_FENCE_TSO_success_clause
      ordinarySuccessfulClause :=
        Functions.execute_FENCE_eq
          (Sail.BitVec.extractLsb row.immediate 11 8)
          (Sail.BitVec.extractLsb row.immediate 7 4)
          (Sail.BitVec.extractLsb row.immediate 3 0)
          (.Regidx row.rs1)
    ...
    fullStepFraming :=
      Functions.generated_full_step_retirement_composition
        stepNo exitWait
    successfulTraceDecode :=
      successfulTraceDecode_of_certificate
        stepNo exitWait
        (Functions.encodeFenceTrace
          row.immediate row.rs1 row.rd)
        (Functions.decodedFenceTrace
...
def generatedRegisterValue?
  ...
  | 0 => some RiscvRefinement.zeroWord
  | 1 => state.regs.get? Register.x1
  | 2 => state.regs.get? Register.x2
  | 3 => state.regs.get? Register.x3
  | 4 => state.regs.get? Register.x4
  | 5 => state.regs.get? Register.x5
  | 6 => state.regs.get? Register.x6
  | 7 => state.regs.get? Register.x7
  | 8 => state.regs.get? Register.x8
  | 9 => state.regs.get? Register.x9
  | 10 => state.regs.get? Register.x10
  | 11 => state.regs.get? Register.x11
  | 12 => state.regs.get? Register.x12
  | 13 => state.regs.get? Register.x13
  | 14 => state.regs.get? Register.x14
  | 15 => state.regs.get? Register.x15
  | 16 => state.regs.get? Register.x16
  | 17 => state.regs.get? Register.x17
  | 18 => state.regs.get? Register.x18
  | 19 => state.regs.get? Register.x19
  | 20 => state.regs.get? Register.x20
  | 21 => state.regs.get? Register.x21
  | 22 => state.regs.get? Register.x22
  | 23 => state.regs.get? Register.x23
  | 24 => state.regs.get? Register.x24
  | 25 => state.regs.get? Register.x25
  | 26 => state.regs.get? Register.x26
  | 27 => state.regs.get? Register.x27
  | 28 => state.regs.get? Register.x28
  | 29 => state.regs.get? Register.x29
  | 30 => state.regs.get? Register.x30
  | _ => state.regs.get? Register.x31
  ...
  normalizedRetirement :
    Functions.completeBaseExecution environment.pre.pc
        (Functions.execute_UTYPE
          (luiImmediate row.imm0 row.imm1 row.imm2)
          (.Regidx row.rd) .LUI) =
      Functions.runNormalizedRetirement
        (luiRetirement
          (Air.Bridge.Lui.interpretedRow row)).nextPc
        (pure
          (luiRetirement
            (Air.Bridge.Lui.interpretedRow row)).write)
  ...
  successfulTraceDecode :
    ∀ (final : Functions.GeneratedState)
      ...
      Functions.generatedFullStepWithTrace stepNo exitWait initial =
          .ok (waiting, trace) final →
        trace.decodedBase = some decodedBase →
        decodedBase.word = environment.word →
        decodedBase.decoded =
          Functions.decodedLuiTrace
            (luiImmediate row.imm0 row.imm1 row.imm2)
            row.rd
...
theorem LUI_accepted_air_refines
  ...
  have decoder :
    ...
    exact
      Functions.decode_lui_certificate
        (luiImmediate row.imm0 row.imm1 row.imm2)
        row.rd
  exact {
    exactProductionIdentity :=
    ...
    fullStepFraming :=
      Functions.generated_full_step_retirement_composition
        stepNo exitWait
    successfulTraceDecode := by
      intro final waiting trace decodedBase
        _fullStepOutcome _traceDecode wordEq
      exact
