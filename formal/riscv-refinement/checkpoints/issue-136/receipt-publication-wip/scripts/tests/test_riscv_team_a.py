...
class TeamAGateTest(unittest.TestCase):
    ...
    def _generated_sail_fixture(
        ...
        payload = {
            ...
            "claim_boundary": {
                ...
                "input_bound_selectors": FULL_NORMALIZED,
    ...
    def test_generated_retirement_binding_requires_its_exact_theorem(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch.object(team_a, "REPOSITORY_ROOT", root):
                ...
                certificate["sail_theorem"] = (
                    team_a.GENERATED_SAIL_RETIREMENT_THEOREMS["auipc"]
                )
                team_a._check_generated_sail_binding(
                    "auipc",
                    "generated-retirement",
            ...
            with tempfile.TemporaryDirectory() as raw:
                ...
                with (
                    ...
                    self.assertRaisesRegex(
                        team_a.TeamAError,
                        "exact 46-selector manifest order",
                    ),
                ):
                    team_a._check_generated_sail_binding(
                        "lui",
    ...
    def test_generated_receipt_must_bind_the_full_selector_inventory(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            certificate = self._generated_sail_fixture(
                root,
            ...
            payload["claim_boundary"]["input_bound_selectors"] = [
                "LUI"
            ]
            ...
            with (
                    ...
                    self.assertRaisesRegex(
                        team_a.TeamAError,
                        "does not bind the exact 46-selector",
                    ),
            ):
                team_a._check_generated_sail_binding(
                    "auipc",
