...
private theorem sourceClockOfGap
  ...
  cases kind <;>
    simp_all [
      ...
      loadField, activeField, storeField, boolM31,
      LoadStoreRow.isStore,
    ]
  all_goals
    first
...
private theorem destinationClockOfGap
  ...
  cases kind <;>
    simp_all [
      ...
      destinationAccessClockField, storeField, boolM31,
      LoadStoreRow.isStore,
    ]
  all_goals
    first
...
private theorem baseNextPcNode
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).nodes.getSymbolic 317 =
...
theorem loadStoreHoldsOfAccepted
  ...
  refine {
    ...
    baseReadOnly :=
      baseReadOnlyOfEquations kind row witness flags equations
    sourceReadOnly :=
      sourceReadOnlyOfEquations kind row witness flags equations
    ...
    nextPcResult :=
      nextPcResultOfBindings kind row witness admission bindings
  }
...
structure ExactTupleProjection
  ...
  sourceConsume :
    (evaluation kind row witness).lookup? 79 =
      some
        (memoryConsumeLookup 79 2 (-(1 : M31))
          (boolM31 row.isLoad)
          (M31.reduce row.sourceSelector)
          (M31.reduce row.srcPreviousClock)
          row.srcPrevious)
  sourceEmit :
    (evaluation kind row witness).lookup? 80 =
      some
        (memoryEmitLookup 80 2 1
          (boolM31 row.isLoad)
          (M31.reduce row.sourceSelector)
          (sourceAccessClockField row)
          row.srcNext)
  destinationConsume :
    (evaluation kind row witness).lookup? 82 =
      some
        (memoryConsumeLookup 82 3 (-(1 : M31))
          (boolM31 row.isStore)
          (M31.reduce row.destinationSelector)
          (M31.reduce row.dstPreviousClock)
          row.dstPrevious)
  destinationEmit :
    (evaluation kind row witness).lookup? 83 =
      some
        (memoryEmitLookup 83 3 1
          (boolM31 row.isStore)
          (M31.reduce row.destinationSelector)
          (destinationAccessClockField row)
          row.dstNext)
...
private theorem baseRawStateConsume
    ...
    (baseEvaluation row witness).lookup? 72 =
      some
        (stateConsumeLookup 72 (-(activeField row))
...
private theorem baseRawStateEmit
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).lookup? 73 =
      some
        (stateEmitLookup 73 (activeField row)
          ((baseEvaluation row witness).nodes.getSymbolic 317)
...
private theorem baseRawBaseConsume
    ...
    (baseEvaluation row witness).lookup? 74 =
      some
        (memoryConsumeLookup 74 1 (-(activeField row)) 0
          (bitVecM31 row.rs1Addr)
          (M31.reduce row.rs1PreviousClock)
          row.rs1Previous) := by
  rfl
...
private theorem baseRawBaseEmit
    ...
    (baseEvaluation row witness).lookup? 75 =
      some
        (memoryEmitLookup 75 1 (activeField row) 0
          (bitVecM31 row.rs1Addr)
          (accessClockField row 1)
          row.rs1Next) := by
  rfl
...
private theorem baseRawSourceConsume
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).lookup? 79 =
      some
        (memoryConsumeLookup 79 2 (-(activeField row))
          (loadField row)
          (M31.reduce row.sourceSelector)
          (M31.reduce row.srcPreviousClock)
          row.srcPrevious) := by
...
private theorem baseRawSourceEmit
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).lookup? 80 =
      some
        (memoryEmitLookup 80 2 (activeField row)
          (loadField row)
          (M31.reduce row.sourceSelector)
          (sourceAccessClockField row)
          row.srcNext) := by
...
private theorem baseRawDestinationConsume
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).lookup? 82 =
      some
        (memoryConsumeLookup 82 3 (-(activeField row))
          (storeField row)
          (M31.reduce row.destinationSelector)
          (M31.reduce row.dstPreviousClock)
          row.dstPrevious) := by
...
private theorem baseRawDestinationEmit
    (row : LoadStoreRow)
    (witness : Witness row) :
    (baseEvaluation row witness).lookup? 83 =
      some
        (memoryEmitLookup 83 3 (activeField row)
          (storeField row)
          (M31.reduce row.destinationSelector)
          (destinationAccessClockField row)
          row.dstNext) := by
...
theorem exactTupleProjection
  ...
  · rw [evaluationLookupShared kind row witness]
    simpa [activeField, next] using baseRawStateEmit row witness
  · rw [evaluationLookupShared kind row witness]
