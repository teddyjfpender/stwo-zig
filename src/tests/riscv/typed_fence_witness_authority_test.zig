//! Serial proof-level acceptance for the generated typed FENCE authority.
...
const BODY = [_]u32{
    0x0000_000f, // FENCE rd=0, rs1=0, imm12=0
    0x7fff_8f8f, // FENCE rd=31, rs1=31, imm12=0x7ff
    0x8008_848f, // FENCE rd=9, rs1=17, imm12=0x800
    0xf538_8f8f, // FENCE rd=31, rs1=17, imm12=0xf53
};

test "typed FENCE generated and legacy authorities produce identical proofs" {
    ...
    defer guest.deinit();
    try std.testing.expectEqual(@as(usize, BODY.len), try guest.familyRowCount(.fence));

    const generated = try runArm(allocator, &guest, null);
    defer generated.deinit(allocator);
    const legacy = try runArm(allocator, &guest, .legacy_fence_authority);
    ...
    std.debug.print(
        "\n  typed FENCE authority proof: bytes={d} sha256={s} transcript={s} draws={d}\n",
        .{
