//! Fixed shared-corpus acceptance for typed SHIFTS_IMM production authority.
...
const typed = frontend.testing.typed_shifts_imm;
const witness = frontend.testing.typed_shifts_imm_witness;
const legacy_oracle = frontend.testing.shifts_imm_legacy_test_oracle;

test "witness rigidity: typed SHIFTS_IMM authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .shifts_imm) continue;
        try std.testing.expect(switch (sample.trace_row.opcode) {
            .SLLI, .SRLI, .SRAI => true,
            else => false,
        });
    ...
    var production = try trace.columnsForFamily(allocator, .shifts_imm, log_size);
