...
//! claim; full guest caller closure belongs to the selected Ethereum profile.
...
const air = @import("stwo_riscv_frontend").testing.component_proof.narrow_air;
const legacy = @import("stwo_riscv_frontend").testing.component_proof.poseidon_air;
const Component = @import("stwo_riscv_frontend").testing.component_proof.narrow_component.Component;
...
const LOG: u32 = 4;
const CALLS = [_]legacy.Call{ legacy.Call.narrow(123, 456), legacy.Call.narrow(789, 123456), legacy.Call.narrow(7, 11) };
...
fn component(comptime trace_log: u32, relations: *const relations_mod.Relations, claims: [2]QM31) Component {
    return .{ .log_size = trace_log, .n_rows = CALLS.len, .is_first_col_idx = 0, .is_active_col_idx = 1, .main_col_offset = 0, .interaction_col_offset = 0, .relations = relations, .claims = claims };
}
...
test "Ethereum narrow degree3 Poseidon serializes destroys and freshly verifies complete component proof" {
