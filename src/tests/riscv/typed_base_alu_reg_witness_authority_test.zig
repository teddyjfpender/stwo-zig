//! Exact proof-level A/B acceptance for typed BASE_ALU_REG authority.
...
const BODY = [_]u32{
    ...
    0x4053_03b3, // SUB  x7, x6, x5
    0x0053_c433, // XOR  x8, x7, x5
    0x0054_64b3, // OR   x9, x8, x5
    0x0054_f533, // AND  x10, x9, x5
    0x00a5_0033, // ADD  x0, x10, x10 (architectural discard)
};

test "typed BASE_ALU_REG generated and legacy authorities produce identical proofs" {
    ...
    defer guest.deinit();
    try std.testing.expect(try guest.familyRowCount(.base_alu_reg) >= 6);

    const generated = try runArm(allocator, &guest, null);
    defer generated.deinit(allocator);
    const legacy = try runArm(allocator, &guest, .legacy_base_alu_reg_authority);
    ...
    std.debug.print(
        "\n  typed BASE_ALU_REG authority proof: bytes={d} sha256={s} " ++
