//! Shared-corpus acceptance evidence for the typed JAL production witness.
...
const typed_jal = frontend.testing.typed_jal;
const witness = frontend.testing.typed_jal_witness;
const legacy_oracle = frontend.testing.jal_legacy_test_oracle;

test "witness rigidity: typed JAL authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .jal) continue;
        try std.testing.expectEqual(frontend.isa.decode.Opcode.JAL, sample.trace_row.opcode);
    ...
    var production = try trace.columnsForFamily(allocator, .jal, log_size);
