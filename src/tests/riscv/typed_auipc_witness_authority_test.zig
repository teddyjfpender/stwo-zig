//! Serial proof-level acceptance for the generated typed AUIPC authority.
//!
//! The A arm uses production dispatch. The B arm clears every final AUIPC
//! shard, regenerates it with the retired handwritten oracle, discards the
//! generated lookup counters, and re-ingests from those rewritten cells.
//! Statement, interaction claim, terminal transcript state, and serialized
//! proof bytes must all remain exactly identical.
...
const BODY = [_]u32{
    ...
    0x7fff_f397, // AUIPC x7, 0x7ffff000
    0x8000_0417, // AUIPC x8, 0x80000000
    0xffff_f497, // AUIPC x9, 0xfffff000
};

test "typed AUIPC generated and legacy authorities produce identical proofs" {
    ...
    defer guest.deinit();
    try std.testing.expectEqual(@as(usize, BODY.len), try guest.familyRowCount(.auipc));

    // Deliberately serial: finish and verify the production proof before the
    // alternate authority owns the workspace.
    const generated = try runArm(allocator, &guest, null);
    defer generated.deinit(allocator);
    const legacy = try runArm(allocator, &guest, .legacy_auipc_authority);
    ...
    std.debug.print(
        "\n  typed AUIPC authority proof: bytes={d} sha256={s} transcript={s} draws={d}\n",
        .{
