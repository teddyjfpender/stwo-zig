//! Exact proof-level A/B acceptance for typed BASE_ALU_IMM authority.
...
const BODY = [_]u32{
    0x8000_0313, // ADDI x6, x0, -2048
    0xffff_4393, // XORI x7, x6, -1
    0x7ff3_e413, // ORI  x8, x7, 2047
    0x0ff4_7493, // ANDI x9, x8, 255
    0x0014_8013, // ADDI x0, x9, 1 (architectural discard)
};

test "typed BASE_ALU_IMM generated and legacy authorities produce identical proofs" {
    ...
    defer guest.deinit();
    try std.testing.expect(try guest.familyRowCount(.base_alu_imm) >= BODY.len);

    // Complete and verify each transaction before starting its alternate arm.
    const generated = try runArm(allocator, &guest, null);
    defer generated.deinit(allocator);
    const legacy = try runArm(allocator, &guest, .legacy_base_alu_imm_authority);
    ...
    std.debug.print(
        "\n  typed BASE_ALU_IMM authority proof: bytes={d} sha256={s} " ++
