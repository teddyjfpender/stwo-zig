//! Fixed shared-corpus acceptance for typed MULH production authority.
...
const typed = frontend.testing.typed_mulh;
const witness = frontend.testing.typed_mulh_witness;
const legacy_oracle = frontend.testing.mulh_legacy_test_oracle;

test "witness rigidity: typed MULH authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .mulh) continue;
    ...
    var production = try trace.columnsForFamily(allocator, .mulh, log_size);
