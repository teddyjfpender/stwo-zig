//! Fixed shared-corpus acceptance for typed SHIFTS_REG production authority.
...
const typed = frontend.testing.typed_shifts_reg;
const witness = frontend.testing.typed_shifts_reg_witness;
const legacy_oracle = frontend.testing.shifts_reg_legacy_test_oracle;

test "witness rigidity: typed SHIFTS_REG authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .shifts_reg) continue;
    ...
    var production = try trace.columnsForFamily(allocator, .shifts_reg, log_size);
