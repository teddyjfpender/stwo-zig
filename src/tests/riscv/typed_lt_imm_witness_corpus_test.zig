//! Shared-corpus acceptance evidence for typed LT_IMM production authority.
...
const typed = frontend.testing.typed_lt_imm;
const witness = frontend.testing.typed_lt_imm_witness;
const legacy_oracle = frontend.testing.lt_imm_legacy_test_oracle;

test "witness rigidity: typed LT_IMM authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .lt_imm) continue;
    ...
    var production = try trace.columnsForFamily(allocator, .lt_imm, log_size);
