//! Shared-corpus acceptance evidence for typed BRANCH_LT production authority.
...
const typed = frontend.testing.typed_branch_lt;
const witness = frontend.testing.typed_branch_lt_witness;
const legacy_oracle = frontend.testing.branch_lt_legacy_test_oracle;

test "witness rigidity: typed BRANCH_LT authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .branch_lt) continue;
    ...
    var production = try trace.columnsForFamily(allocator, .branch_lt, log_size);
