//! Shared-corpus acceptance evidence for typed LT_REG production authority.
...
const typed = frontend.testing.typed_lt_reg;
const witness = frontend.testing.typed_lt_reg_witness;
const legacy_oracle = frontend.testing.lt_reg_legacy_test_oracle;

test "witness rigidity: typed LT_REG authority and legacy oracle match every sampled row" {
    ...
    for (corpus.samples) |sample| {
        if (sample.family != .lt_reg) continue;
    ...
    var production = try trace.columnsForFamily(allocator, .lt_reg, log_size);
