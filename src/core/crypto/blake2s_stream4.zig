...
/// Absorbs a column group into four independent streams and finalizes them,
/// keeping the transposed SIMD state resident for the whole message.
///
/// `updateM31Columns4` followed by `finalizeEqualTail4` produces the same
/// digests, but each half owns the transposition: the update gathers the four
/// scalar states into `[8]V4`, scatters them back plus a per-lane byte buffer,
/// and the finalize immediately re-gathers them and rebuilds the terminal
/// block from that buffer. When the caller has no use for the intermediate
/// hasher state — a leaf that is finalized the moment its last column is
/// absorbed — that round trip is pure overhead: four state scatters, four
/// buffer writes, and four state gathers per four leaves.
///
/// This entry point performs the gather once, streams whole blocks straight
/// out of column-major storage, and finishes with the terminal compression,
/// so no scalar hasher state is ever written back.
pub fn finalizeM31Columns4(
    ...
    // The already-buffered words head the first block; the caller's absorb
    // order is preserved exactly.
    const buffered_words = hashers[0].buf_len / @sizeOf(u32);
    var messages: [16]V4 = @splat(@splat(0));
    for (0..buffered_words) |word| {
        var lanes: [4]u32 = undefined;
        for (0..4) |lane| {
            lanes[lane] = std.mem.readInt(
                u32,
                hashers[lane].buf[word * @sizeOf(u32) ..][0..@sizeOf(u32)],
                .little,
            );
        }
        messages[word] = lanes;
    ...
    var filled = buffered_words;
    var word_at: usize = 0;
    while (word_at < columns.len) {
        const take = @min(16 - filled, columns.len - word_at);
        for (0..take) |offset| {
            const values: *const [4]u32 = @ptrCast(
                columns[word_at + offset].values.ptr + position,
            );
            messages[filled + offset] = values.*;
        }
        filled += take;
        word_at += take;
        // BLAKE2s marks the *last* block, so a full block is only compressed
        // once another word is known to follow.
        if (filled == 16 and word_at < columns.len) {
            addBlockCounter(&t0, &t1);
            compressParallel4(&states, &messages, t0, t1, 0);
            messages = @splat(@splat(0));
            filled = 0;
        }
    }

    const counter_sum = @as(u64, t0) + @as(u64, filled * @sizeOf(u32));
    t0 = @truncate(counter_sum);
    t1 +%= @intCast(counter_sum >> 32);
    compressParallel4(&states, &messages, t0, t1, 0xFFFF_FFFF);
    return statesToDigests(&states);
}
