...
@property(nonatomic) uint32_t inputs, profiles, relations, batches;
@property(nonatomic,strong) NSData *inputTrees;
@end
@implementation StwoFrameworkInteractionPlan
@end

static NSArray<NSString *> *stwo_framework_interaction_names(NSString *fraction) {
...
static void *stwo_framework_interaction_plan(id owner, id<MTLDevice> device, id<MTLCommandQueue> queue,
    ...
    if (device==nil || queue==nil || name==nil || ![name hasPrefix:@"stwo_zig_framework_interaction_v1_"] ||
        tags==NULL || inputs==0u || relations==0u || relations%4u!=0u || batches==0u) return NULL;
    ...
    plan.inputs=inputs;plan.profiles=profiles;plan.relations=relations;plan.batches=batches;
...
void *stwo_zig_framework_interaction_prepare(void *runtime_ptr,const char *name_bytes,size_t name_len,
    ...
    @autoreleasepool {
        ...
        return stwo_framework_interaction_plan(runtime,runtime.device,runtime.queue,runtime.riscvPolynomialPipelines,name,
            tags,inputs,profiles,relations,batches);
    }
}
...
uint32_t stwo_zig_framework_interaction_generate(void *plan_ptr,void *tree0_ptr,void *tree1_ptr,
    ...
    @autoreleasepool {
        ...
        uint64_t scratch_words=(uint64_t)batches*blocks*4u;
        ...
        [encoder dispatchThreadgroups:MTLSizeMake((NSUInteger)batches*blocks,1u,1u) threadsPerThreadgroup:MTLSizeMake(256u,1u,1u)];
        [encoder memoryBarrierWithScope:MTLBarrierScopeBuffers];
        [encoder setComputePipelineState:plan.pipelines[2]];
        [encoder setBytes:&batches length:4u atIndex:4];
        [encoder dispatchThreads:MTLSizeMake(batches,1u,1u) threadsPerThreadgroup:MTLSizeMake(256u,1u,1u)];
        [encoder memoryBarrierWithScope:MTLBarrierScopeBuffers];
        [encoder setComputePipelineState:plan.pipelines[3]];
        [encoder dispatchThreads:MTLSizeMake(rows,batches,1u) threadsPerThreadgroup:MTLSizeMake(256u,1u,1u)];
