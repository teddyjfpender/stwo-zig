//! Recording builder for witness-JIT bytecode.
//!
//! Mirrors the constraint lane's [`super::super::jit::recording`] but for the witness
//! domain. The constraint recorder implements the framework's `EvalAtRow` trait so the
//! *existing generic* `FrameworkEval::evaluate` closure records itself for free. Witness
//! writers have **no** such generic entry point — the generated `write_trace_simd`
//! bodies are monomorphic over `PackedM31`/`PackedUInt16` (see the blocker note in
//! [`super`]). So this recorder exposes an explicit builder API: a per-row witness body
//! is written (or, in the target end-state, *code-generated by stwo-air-infra*) against
//! [`WitnessRecorder`], and the recorder captures it as [`WitnessProgram`] bytecode.
//!
