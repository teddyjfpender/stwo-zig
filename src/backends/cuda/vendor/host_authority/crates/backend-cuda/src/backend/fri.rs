...
// Removed from the public API upstream; the circle->line fold halves the domain.
...
impl FriOps for CudaBackend {
    /// `alphas[i] = alpha^(2^i)`: fold one layer per entry, mirroring the reference
    /// backend's chain semantics.
    fn fold_line(
        ...
        let mut result = fold_line_once_cuda(eval, alphas[0], twiddles);
        for &alpha in &alphas[1..] {
            result = fold_line_once_cuda(&result, alpha, twiddles);
        }
        result
    }
    ...
    fn fold_circle_into_line(
        ...
        let line_domain = LineDomain::new(src.domain.half_coset);
        // The kernel accumulates into dst; a zeroed dst yields the plain fold.
        let mut dst = LineEvaluation::new_zero(line_domain);
        fold_circle_into_line_cuda(&mut dst, src, alpha, twiddles);
        dst
    }
    ...
    fn decompose(
        ...
        // Deprecated upstream: FRI inputs are already in the FFT space.
        unimplemented!("FriOps::decompose has no CUDA lane")
    }
}
