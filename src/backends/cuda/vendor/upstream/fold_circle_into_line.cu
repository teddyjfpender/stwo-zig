...
__global__ void fold_circle_into_line_device_alpha_kernel(
    m31 *domain,
    const uint32_t twiddle_offset,
    const uint32_t n,
    ...
    const qm31 f_1 = mul_by_scalar(sub(f_x, f_x_minus), x_inverse);
    const qm31 f_prime = add(f_0, mul(alpha, f_1));
    const qm31 previous_value = qm31 {
        folded_values[0][i],
        folded_values[1][i],
        folded_values[2][i],
        folded_values[3][i],
    };
    const qm31 new_value = add(mul(previous_value, alpha_sq), f_prime);
    folded_values[0][i] = new_value.a.a;
    folded_values[1][i] = new_value.a.b;
    folded_values[2][i] = new_value.b.a;
    folded_values[3][i] = new_value.b.b;
...
extern "C" int stwo_fold_circle_into_line_on(
    ...
    fold_circle_into_line_device_alpha_kernel<<<num_blocks, block_dim, 0,
