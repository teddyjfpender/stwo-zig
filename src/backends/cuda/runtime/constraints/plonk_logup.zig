//! Arena-native binding for the exact Plonk/LogUp composition kernel.
...
pub const source_column_count: u32 = 16;
pub const constraint_count: u32 = 3;
pub const argument_count: u32 = 17;
pub const cache_key: u64 = 0xbfa4f42232b92676;
...
pub fn descriptor(
    ...
    return .{
        ...
        .abi_schema = .native_plonk_logup_constraint_v1,
        .cache_key = cache_key,
        .name = kernel_name,
...
test "exact Plonk descriptor covers every expanded-domain row once" {
...
test "exact Plonk binding launches only a fully admitted descriptor" {
...
test "exact Plonk binding rejects malformed shapes and ownership" {
    var session = TestSession{};
...
test "exact Plonk binding rejects every read-write alias class" {
    var session = TestSession{};
    const buffers = testBuffers();
...
test "exact Plonk binding rejects aliases among immutable inputs" {
    var session = TestSession{};
    const buffers = testBuffers();
    var alias = buffers;
...
const TestSession = struct {
    ...
    pub fn launchKernel(
        ...
        if (kernel.stage != .constraint_evaluation or
            kernel.abi_schema != .native_plonk_logup_constraint_v1 or
...
fn testBuffers() Buffers {
    return .{
        .source_evaluations = .{
            .storage = testWords(0x1000, 128),
            .column_stride_words = 8,
        },
        .random_coefficient_powers = testSecure(0x1300, 3),
