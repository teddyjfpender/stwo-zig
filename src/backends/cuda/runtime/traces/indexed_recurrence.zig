//! Strict-AOT binding for an indexed order-two M31 recurrence trace.
...
pub const cache_key: u64 = 0x6af438997e7d2cef;
...
pub const Recipe = struct {
    index_base: u64,
    index_step: u64,
    preprocessed_constant: u64,
    recurrence_seed0: u64,
    recurrence_seed1: u64,
    selector_default: u64,
    selector_last: u64,
    selector_penultimate: u64,
};
...
const Arguments = struct {
    ...
    fn pointers(self: *Arguments) [argument_count]?*anyopaque {
        return .{
            ...
            @ptrCast(&self.index_base),
            @ptrCast(&self.index_step),
            @ptrCast(&self.preprocessed_constant),
            @ptrCast(&self.recurrence_seed0),
            @ptrCast(&self.recurrence_seed1),
            @ptrCast(&self.selector_default),
            @ptrCast(&self.selector_last),
            @ptrCast(&self.selector_penultimate),
        };
    }
};
...
pub fn prepare(
    ...
    try validateRecipe(recipe, rows);
    ...
    const main = try exactMatrix(session, destinations.main, rows);
    ...
    return .{
        ...
        .arguments = .{
            ...
            .index_base = recipe.index_base,
            .index_step = recipe.index_step,
            .preprocessed_constant = recipe.preprocessed_constant,
            .recurrence_seed0 = recipe.recurrence_seed0,
            .recurrence_seed1 = recipe.recurrence_seed1,
            .selector_default = recipe.selector_default,
            .selector_last = recipe.selector_last,
            .selector_penultimate = recipe.selector_penultimate,
        },
    };
}
...
pub fn descriptor(log_n_rows: u32) runtime_error.Error!kernel_module.Kernel {
    ...
    return .{
        ...
        .abi_schema = .native_indexed_recurrence_trace_v1,
        .cache_key = cache_key,
        .name = kernel_name,
...
fn exactMatrix(
    ...
    if (resident.column_count != column_count)
...
fn validateRecipe(recipe: Recipe, rows: u32) runtime_error.Error!void {
    if (recipe.index_step == 0) return error.InvalidKernelDescriptor;
    const final_step = std.math.mul(
        u64,
        @as(u64, rows) + 1,
        recipe.index_step,
    ) catch return error.SizeOverflow;
...
test "indexed recurrence admits guard and non-target geometries" {
...
test "indexed recurrence rejects shape aliases and recipe drift" {
    var session = TestSession{};
    ...
    try std.testing.expectError(
        ...
        prepare(&session, alias, geometry, testRecipe()),
    );
    var invalid = testRecipe();
    invalid.index_step = 0;
    try std.testing.expectError(
        error.InvalidKernelDescriptor,
        prepare(&session, destinations, geometry, invalid),
    );
    try std.testing.expectError(
        error.InvalidKernelDescriptor,
        descriptor(0),
...
fn testDestinations(stride: usize) Destinations {
    return .{
        .preprocessed = testMatrix(0x1000, stride),
        .main = testMatrix(0x11_0000, stride),
    };
}

fn testMatrix(address: usize, stride: usize) common.WordMatrix {
    return .{
        .storage = .{
            .address = address,
            .len = stride * column_count,
            .owner = 7,
            .generation = 11,
        },
