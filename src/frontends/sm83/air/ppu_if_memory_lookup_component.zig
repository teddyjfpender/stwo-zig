//! AIR adapter for PPU interrupt writes into authenticated memory.
//!
//! This component owns the FF0F predecessor and masked-bit witnesses plus one
//! shared-memory accumulator. It borrows PPU binding columns; PPU semantics,
//! binding, and global memory cancellation remain separate owners.
...
pub const Component = struct {
    ...
    pub fn evaluateRow(
        ...
        const semantic = lookup.evaluate(QM31, ppu, predecessor);
    ...
    pub fn evaluateSampled(
        ...
        var binding_values: [ppu_binding.N_MAIN_COLUMNS]QM31 =
            undefined;
    ...
    pub fn evaluateConstraintQuotientsOnDomain(
        ...
        const predecessor_start =
            binding_start + ppu_binding.N_MAIN_COLUMNS;
    ...
    fn validateConfiguration(self: *const Self) !void {
        if (self.log_size < 4 or self.log_size > 24)
            return error.InvalidPpuLogSize;
    }
...
/// Masked predecessor columns keep the dual-bit OR result linear while their
/// quadratic definitions make the complete recurrence exactly cubic.
fn linearizedPair(
    ppu: lookup.PpuRow(QM31),
    ...
    const requested = vblank.add(stat).sub(vblank.mul(stat));
    const previous_value = compose(predecessor.previous_value);
    const next_value = previous_value
        .add(vblank)
        .sub(predecessor.masked_previous[0])
        .add(stat.sub(predecessor.masked_previous[1]).mul(constant(2)));
    return .{
        .n1 = requested.neg(),
        .d1 = relation.combine(
...
fn phasedClock(ppu: lookup.PpuRow(QM31)) QM31 {
    return ppu.mcycle
        .mul(constant(memory_clock.PHASES))
        .add(constant(memory_clock.PPU_PHASE + 1));
}
