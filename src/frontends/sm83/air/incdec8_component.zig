//! INC/DEC r8 specialization of the shared execution-bound family component.
...
pub const N_CONSTRAINTS: usize = incdec8.N_BOUND_CONSTRAINTS + 1;
pub const Component = family_component.Component(incdec8);

test "INC DEC r8 component rejects a half-carry mutation" {
    ...
    defer memory.deinit();
    memory.write(0, 0x04);
    var cpu = runner.Cpu{ .b = 0x0f };
    const step = try runner.step(&cpu, &memory);
    var values: [incdec8.N_MAIN_COLUMNS]QM31 = undefined;
    for (
        &values,
        incdec8.columns(try incdec8.ValidatedStep.init(step)),
    ) |*value, source| {
...
test "INC DEC r8 component prover-domain path rejects a result mutation" {
    ...
    var main_values: [incdec8.N_MAIN_COLUMNS][evaluation_size]M31 = undefined;
    var main_polynomials: [incdec8.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    var combined: [execution.N_MAIN_COLUMNS + 1 + incdec8.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    @memset(&main_values[18], M31.zero());
