//! AIR adapters for execution/timer MMIO LogUp owners.
...
pub const Component = struct {
    ...
    pub fn nConstraints(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .timer => lookup.N_TIMER_CONSTRAINTS,
        };
    }
    ...
    pub fn evaluateSampled(
        ...
        var values: [timer_binding.N_MAIN_COLUMNS]QM31 = undefined;
        try sampledColumns(
            &values,
            main[self.binding_offset..][0..timer_binding.N_MAIN_COLUMNS],
        );
        const timer = try lookup.timerRow(QM31, &values);
        const pairs = lookup.timerPairs(timer, self.relations.*);
        for (pairs, 0..) |entry, index| {
            constraints[index] = lookup.pairConstraint(
                ...
                preprocessed[self.is_first_column][0],
                self.claims.timer[index],
                entry.numerator,
                entry.denominator,
            );
        }
        return lookup.N_TIMER_CONSTRAINTS;
    }
    ...
    fn mainEnd(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .timer => self.binding_offset + timer_binding.N_MAIN_COLUMNS,
        };
    }
    ...
    fn dataColumns(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .timer => timer_binding.N_MAIN_COLUMNS,
        };
    }
    ...
    fn interactionColumns(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .timer => lookup.N_TIMER_INTERACTION_COLUMNS,
        };
    }
};
