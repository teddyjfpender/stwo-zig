//! CB rotate/shift specialization of the shared execution-bound family component.
...
pub const N_CONSTRAINTS: usize = cb_rotate_shift.N_BOUND_CONSTRAINTS + 1;
pub const Component = family_component.Component(cb_rotate_shift);

test "CB rotate shift component rejects a carry mutation" {
    ...
    var cpu = runner.Cpu{ .h = 0x80 };
    const step = try runner.step(&cpu, &memory);
    var values: [cb_rotate_shift.N_MAIN_COLUMNS]QM31 = undefined;
    for (
        &values,
        cb_rotate_shift.columns(try cb_rotate_shift.ValidatedStep.init(step)),
...
test "CB rotate shift component prover-domain path rejects a bus mutation" {
    ...
    var main_values: [cb_rotate_shift.N_MAIN_COLUMNS][evaluation_size]M31 =
        undefined;
    var main_polynomials: [cb_rotate_shift.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    var combined: [execution.N_MAIN_COLUMNS + 1 + cb_rotate_shift.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    @memset(&execution_values[bus3_value], M31.zero());
