//! Proof component for ordered joypad-event clocks and P1-read markers.
//!
//! The companion joypad component owns device semantics and activity-prefix
//! soundness. This component binds the shared event clock, canonical
//! action/write/tick order, read markers, and public segment clock endpoints.
    ...
    return struct {
        ...
        pub fn fromColumns(values: []const S) !@This() {
            ...
            return .{
                ...
                .read_enabled = values[binding.READ_ENABLED_OFFSET],
            };
        }
    };
}
...
pub fn evaluateRows(
    ...
    const set_pressed = event(current, .set_pressed);
    const write_p1 = event(current, .write_p1);
    const tick = event(current, .tick_mcycle);
    const next_set_pressed = event(next, .set_pressed);
    const next_write_p1 = event(next, .write_p1);
    var out: [N_CONSTRAINTS]S = undefined;
    var at: usize = 0;

    out[at] = one.sub(current.active).mul(current.mcycle);
    at += 1;
    out[at] = current.read_enabled.mul(current.read_enabled.sub(one));
    at += 1;
    out[at] = one.sub(current.active).mul(current.read_enabled);
    at += 1;
    out[at] = current.read_enabled.mul(one.sub(tick));
    at += 1;
    ...
    at += 1;
    out[at] = not_last.mul(set_pressed).mul(next_set_pressed);
    at += 1;
    out[at] = not_last.mul(write_p1).mul(
        next_set_pressed.add(next_write_p1),
    );
    at += 1;
...
fn event(row: anytype, selected: std.meta.Tag(joypad.Event)) @TypeOf(row.active) {
    return row.semantic.events[@intFromEnum(selected)];
}
...
fn validateBoundaries(initial: u32, final: u32) !void {
    ...
    if (initial >= final) return error.InvalidJoypadClockBoundary;
}
