//! AIR adapters for execution/joypad MMIO LogUp owners.
...
pub const Kind = enum { execution, joypad };
...
pub const Component = struct {
    ...
    pub fn nConstraints(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .joypad => lookup.N_JOYPAD_CONSTRAINTS,
        };
    }

    /// The joypad read relation is cubic; both owners use the same bound.
    ...
    pub fn evaluateSampled(
        ...
        var values: [joypad_binding.N_MAIN_COLUMNS]QM31 = undefined;
        try sampledColumns(
            &values,
            main[self.binding_offset..][0..joypad_binding.N_MAIN_COLUMNS],
        );
        const joypad = try lookup.joypadRow(QM31, &values);
        const pairs = lookup.joypadPairs(joypad, self.relations.*);
        for (pairs, 0..) |entry, index| {
            constraints[index] = lookup.pairConstraint(
                ...
                preprocessed[self.is_first_column][0],
                self.claims.joypad[index],
                entry.numerator,
                entry.denominator,
            );
        }
        const binding_constraints =
            lookup.joypadBindingConstraints(QM31, joypad);
        @memcpy(
            constraints[lookup.N_RELATIONS..lookup.N_JOYPAD_CONSTRAINTS],
            &binding_constraints,
        );
        return lookup.N_JOYPAD_CONSTRAINTS;
    }
    ...
    fn evaluateDomainRow(
        ...
        if (self.kind == .execution) {
            ...
            for (pairs, 0..) |relation_pairs, relation_index| {
                for (relation_pairs, 0..) |entry, sum_index| {
                    constraints[constraint_index] = lookup.pairConstraint(
                        QM31,
                        secureAt(
                            evaluations[interaction_start + 4 * constraint_index ..][0..4],
                            row,
                        ),
                        secureAt(
                            evaluations[interaction_start + 4 * constraint_index ..][0..4],
    ...
    fn mainEnd(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .joypad => self.binding_offset + joypad_binding.N_MAIN_COLUMNS,
        };
    }
    ...
    fn dataColumns(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .joypad => joypad_binding.N_MAIN_COLUMNS,
        };
    }
    ...
    fn interactionColumns(self: *const Self) usize {
        return switch (self.kind) {
            ...
            .joypad => lookup.N_JOYPAD_INTERACTION_COLUMNS,
        };
    }
};
