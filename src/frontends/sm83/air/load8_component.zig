//! LD r8 specialization of the shared execution-bound family component.
...
pub const N_CONSTRAINTS: usize = load8.N_BOUND_CONSTRAINTS + 1;
pub const Component = family_component.Component(load8);

test "LOAD8 component rejects an indirect transfer-byte mutation" {
    ...
    var cpu = runner.Cpu{ .b = 0x80 };
    const step = try runner.step(&cpu, &memory);
    var values: [load8.N_MAIN_COLUMNS]QM31 = undefined;
    for (
        &values,
        load8.columns(try load8.ValidatedStep.init(step)),
    ...
    values[33 + 1] = QM31.zero();
    try std.testing.expect(
        !(try component.evaluateRow(&values, machine, QM31.one())).allZero(),
    );
}

test "LOAD8 component prover-domain path rejects an indirect bus mutation" {
    ...
    var main_values: [load8.N_MAIN_COLUMNS][evaluation_size]M31 = undefined;
    var main_polynomials: [load8.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    var combined: [execution.N_MAIN_COLUMNS + 1 + load8.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    const bus1_address = 2 * execution.N_STATE_COLUMNS + execution.N_BUS_COLUMNS;
    @memset(&execution_values[bus1_address], M31.fromCanonical(0x8001));
