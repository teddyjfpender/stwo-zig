...
const subject = @import("joypad_if_memory_lookup.zig");

test "joypad IF row binds request clock predecessor and OR transition" {
    ...
    try std.testing.expectEqual(@as(u8, 0x15), access.next_value);

    const joypad_values = try joypad_binding.columns(event, &.{});
    const main_values = try subject.columnsForAccess(access);
    var joypad = try liftJoypad(joypad_values);
    var row = try liftRow(main_values);
    try std.testing.expect(subject.evaluate(QM31, joypad, row).allZero());

    row.previous_clock = row.previous_clock.add(QM31.one());
    try std.testing.expect(!subject.evaluate(QM31, joypad, row).allZero());
    row = try liftRow(main_values);
    row.previous_value[0] = row.previous_value[0].add(QM31.one());
    try std.testing.expect(!subject.evaluate(QM31, joypad, row).allZero());
    row = try liftRow(main_values);
    ...
    row = try liftRow(main_values);
    joypad.semantic.interrupt_requested = QM31.zero();
    try std.testing.expect(!subject.evaluate(QM31, joypad, row).allZero());
}

test "host witness rejects forged request metadata and ambiguous clocks" {
    const first = try requestingEvent(7);
    const second = try requestingEvent(7);
    try std.testing.expectError(
        error.JoypadIfClockCollision,
    ...
    try std.testing.expectError(
        error.NonCanonicalJoypadIfClock,
        subject.accessForEvent(late, .{}),
    );
    try std.testing.expectError(
        error.InvalidJoypadIfPredecessorClock,
        subject.accessForEvent(
            first,
...
test "interaction cancels only the exact authenticated IF transition" {
    ...
    const wrong_address = memory_lookup.RowPair{
        .n1 = QM31.one().neg(),
        .d1 = relation.combine(q(0xff0e), q(3), q(0x05)),
        .n2 = QM31.one(),
        .d2 = relation.combine(
            q(0xff0e),
            q(requestClock()),
...
test "witness is bit reversed padded and inactive rows are neutral" {
    const event = try requestingEvent(7);
    ...
    const inactive_joypad = joypad_binding.inactiveColumns();
    const inactive_main = subject.inactiveColumns();
    const joypad = try liftJoypad(inactive_joypad);
    const row = try liftRow(inactive_main);
    try std.testing.expect(subject.evaluate(QM31, joypad, row).allZero());
    const neutral = try subject.pair(.{}, memory_lookup.Relation.dummy());
    try std.testing.expect(neutral.n1.isZero());
    try std.testing.expect(neutral.n2.isZero());

    const semantic = joypad_air.Semantics(QM31).Row.fromColumns(
        &([_]QM31{QM31.zero()} ** joypad_air.N_MAIN_COLUMNS),
    ) catch unreachable;
    _ = semantic;
    const endpoint = runner.joypad.State{};
    const vacuous = try joypad_component.evaluateRows(
        QM31,
        &([_]QM31{QM31.zero()} ** joypad_air.N_MAIN_COLUMNS),
        &([_]QM31{QM31.zero()} ** joypad_air.N_MAIN_COLUMNS),
        QM31.zero(),
...
test "access validation rejects value drift and inactive garbage" {
    try std.testing.expectError(
        error.InvalidJoypadIfValueTransition,
...
fn quietEvent(mcycle: u32) !event_trace.EventRow {
    const transition = try runner.joypad.Transition.apply(
        runner.joypad.State{},
        .tick_mcycle,
    );
