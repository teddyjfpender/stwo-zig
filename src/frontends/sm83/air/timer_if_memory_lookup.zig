//! Timer interrupt-request contribution to authenticated system memory.
//!
//! Every proven timer reload request contributes one ordered mutable-memory
//! transition at FF0F. The predecessor value becomes `value | 0x04` at the
//! event's timer-tick phase. Global authenticated-memory claim cancellation
//! and the companion timer semantic/binding components remain required.
//!
//! A joypad selector edge can request IF at the same tick phase. The composed
//! replay must coalesce those simultaneous OR updates (or define a shared
//! subphase order); two independent accesses cannot reuse one memory clock.
    ...
    return struct {
        ...
        pub fn fromColumns(values: []const S) !@This() {
            if (values.len != N_MAIN_COLUMNS)
                return error.InvalidTimerIfMemoryShape;
            return .{
...
pub fn evaluate(
    ...
    at += 1;
    out[at] = requested.mul(one.sub(timer.active));
    at += 1;
...
pub fn generateWitness(
    ...
    const size = try traceSize(timer_log_size);
    if (events.len == 0) return error.EmptyTimerTrace;
    if (events.len > size) return error.TooManyTimerEvents;
    if (predecessors.len != events.len)
        return error.InvalidPredecessorCount;
    var result = Witness{
        .log_size = timer_log_size,
        .main = undefined,
    ...
    for (events, predecessors, 0..) |event, predecessor, index| {
        ...
        if (access.enabled) {
            if (last_request_clock != null and
                ...
                return error.TimerIfClockCollision;
            last_request_clock = access.clock;
        }
...
pub fn accessForEvent(
    ...
    const clock = memory_lookup.memory_clock.phaseClock(
        event.mcycle,
        memory_lookup.memory_clock.TICK_PHASE,
    ...
    return .{
        ...
        .clock = clock,
        .next_value = predecessor.value | timer_runner.TIMER_INTERRUPT,
    };
}
    ...
    const next_value = previous_value.add(
        requested.mul(
            QM31.fromBase(M31.fromCanonical(
                timer_runner.TIMER_INTERRUPT,
            )),
        ).mul(QM31.one().sub(row.previous_value[2])),
    );
    return .{
        .n1 = requested.neg(),
        .d1 = relation.combine(
...
pub fn accumulate(
    ...
    return current.add(numerator.mul(denominator.inv() catch
        return error.TimerIfMemoryLookupZeroDenominator));
}
...
fn validateAccess(access: Access) !void {
    ...
    if (access.clock >= M31_MODULUS)
        return error.NonCanonicalTimerIfClock;
    ...
    if (access.next_value !=
        access.previous_value | timer_runner.TIMER_INTERRUPT)
        return error.InvalidTimerIfValueTransition;
}
...
fn phasedClock(
    ...
    return timer.mcycle
        ...
        .add(constant(
        S,
        memory_lookup.memory_clock.TICK_PHASE + 1,
    ));
}
