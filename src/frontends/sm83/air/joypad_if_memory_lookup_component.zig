//! AIR adapter for joypad interrupt writes into authenticated memory.
//!
//! This component owns the IF predecessor witness and one memory-relation
//! accumulator. It borrows the shared joypad binding columns. The companion
//! joypad semantic/binding components and global memory-claim cancellation
//! remain required for a sound composed proof.
...
pub const Component = struct {
    ...
    pub fn evaluateRow(
        ...
        try self.validateConfiguration();
        const joypad = try lookup.JoypadRow(QM31).fromColumns(
            binding_values,
        );
        const predecessor = try lookup.Row(QM31).fromColumns(
            predecessor_values,
        );
        const semantic = lookup.evaluate(QM31, joypad, predecessor);
        var constraints: [N_CONSTRAINTS]QM31 = undefined;
        @memcpy(constraints[0..lookup.N_CONSTRAINTS], &semantic.values);
        constraints[lookup.N_CONSTRAINTS] = memory_lookup.pairConstraint(
            current,
            previous,
            is_first,
            self.claim,
            linearizedPair(joypad, predecessor, self.relation.*),
        );
        return constraints;
    }
    ...
    pub fn evaluateSampled(
        ...
        var binding_values: [joypad_binding.N_MAIN_COLUMNS]QM31 =
            undefined;
        var predecessor_values: [lookup.N_MAIN_COLUMNS]QM31 = undefined;
        try sampledColumns(
            &binding_values,
            main[self.binding_offset..][0..joypad_binding.N_MAIN_COLUMNS],
        );
        try sampledColumns(
            &predecessor_values,
    ...
    pub fn evaluateConstraintQuotientsOnDomain(
        ...
        const evaluations = try allocator.alloc(
            []const M31,
            1 + joypad_binding.N_MAIN_COLUMNS +
        ...
        const predecessor_start =
            binding_start + joypad_binding.N_MAIN_COLUMNS;
    ...
    fn mainEnd(self: *const Self) usize {
        return @max(
            self.binding_offset + joypad_binding.N_MAIN_COLUMNS,
...
/// On the leaf constraint surface every predecessor bit obeys
/// `bit * (bit - requested) = 0`. Therefore `requested * bit == bit`,
/// making this linear OR expression identical to the host transition while
/// keeping the recurrence cubic instead of quartic.
fn linearizedPair(
    joypad: lookup.JoypadRow(QM31),
    ...
    const interrupt = constant(runner.joypad.JOYPAD_INTERRUPT);
    const next_value = previous_value.add(
        interrupt.mul(
            requested.sub(predecessor.previous_value[4]),
        ),
    );
    return .{
        .n1 = requested.neg(),
...
fn phasedClock(joypad: lookup.JoypadRow(QM31)) QM31 {
    const phase = joypad.semantic.events[1]
        .mul(constant(memory_clock.CPU_PHASE))
        .add(joypad.semantic.events[2].mul(
        constant(memory_clock.JOYPAD_TICK_PHASE),
    ));
