//! CB RES/SET specialization of the shared execution-bound family component.
...
pub const N_CONSTRAINTS: usize = cb_res_set.N_BOUND_CONSTRAINTS + 1;
pub const Component = family_component.Component(cb_res_set);

test "CB RES SET component rejects result and flag mutations" {
    ...
    var cpu = runner.Cpu{ .f = 0xf0, .h = 0x80 };
    const step = try runner.step(&cpu, &memory);
    var values: [cb_res_set.N_MAIN_COLUMNS]QM31 = undefined;
    for (
        &values,
        cb_res_set.columns(try cb_res_set.ValidatedStep.init(step)),
...
test "CB RES SET component prover-domain path rejects an HL write mutation" {
    ...
    var main_values: [cb_res_set.N_MAIN_COLUMNS][evaluation_size]M31 =
        undefined;
    var main_polynomials: [cb_res_set.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    var combined: [execution.N_MAIN_COLUMNS + 1 + cb_res_set.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
