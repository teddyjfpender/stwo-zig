//! INC/DEC rr specialization of the shared execution-bound family component.
...
pub const N_CONSTRAINTS: usize = incdec16.N_BOUND_CONSTRAINTS + 1;
pub const Component = family_component.Component(incdec16);

test "INC DEC rr component rejects a result mutation" {
    ...
    defer memory.deinit();
    memory.write(0, 0x23);
    var cpu = runner.Cpu{ .h = 0xff, .l = 0xff };
    const step = try runner.step(&cpu, &memory);
    var values: [incdec16.N_MAIN_COLUMNS]QM31 = undefined;
    for (
        &values,
        incdec16.columns(try incdec16.ValidatedStep.init(step)),
    ) |*value, source| {
...
test "INC DEC rr component prover-domain path rejects a result mutation" {
    ...
    var cpu = runner.Cpu{ .h = 0xff, .l = 0xff };
    const step = try runner.step(&cpu, &memory);
    const witness = incdec16.columns(try incdec16.ValidatedStep.init(step));
    ...
    var main_values: [incdec16.N_MAIN_COLUMNS][evaluation_size]M31 = undefined;
    var main_polynomials: [incdec16.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
    ...
    var combined: [execution.N_MAIN_COLUMNS + 1 + incdec16.N_MAIN_COLUMNS]prover_component.Poly =
        undefined;
