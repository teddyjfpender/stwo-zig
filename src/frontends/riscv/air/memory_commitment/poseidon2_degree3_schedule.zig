...
pub fn external(comptime S: type, state: *[WIDTH]S) void {
    if (S == M31) return matrix.externalMatrixM31(state);
    matrix.externalMatrixSecure(S, state);
}
...
pub fn walk(comptime S: type, state: *[WIDTH]S, context: anytype) void {
    ...
    for (constants.INTERNAL_ROUND) |constant| {
        ...
        if (S == M31) matrix.internalMatrixM31(state, constants.INTERNAL_MATRIX) else matrix.internalMatrixSecure(S, state, constants.INTERNAL_MATRIX);
    }
