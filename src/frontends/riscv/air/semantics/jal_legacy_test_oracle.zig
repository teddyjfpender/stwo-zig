//! Test-only oracle for the retired Stark-V-shaped JAL AIR adapter.
//!
//! Production must use `typed_jal_authority.zig`. This independent copy is
//! retained solely for exact direct-root and ordered-relation differential
//! tests through the production cutover.
...
pub fn Semantics(comptime S: type) type {
    return struct {
        ...
        pub fn evaluate(row: Row) Constraints {
            ...
            @memcpy(
                out[2..5],
                &ops.destinationConstraints(row.rd.addr, row.destination),
            );
            @memcpy(
                out[5..9],
                &ops.destinationResultConstraints(
                    row.rd,
                    row.result,
                    row.destination,
                ),
        ...
        pub const RdLookups = struct {
            /// The pinned schema contains this predecessor request twice.
        ...
        /// Only the concrete shipped field instantiation compiles these tests;
        /// extraction scalars intentionally omit inverse/equality operations.
        fn selfTests() type {
            return struct {
                test "jal oracle: honest jump binds link and target" {
                    var row = zeroRow();
                    row.enabler = S.one();
                    row.clock = ops.q(12);
                    row.pc = ops.q(0x1000);
                    ...
                    try std.testing.expect(
                        requests.program.tuple.opcode_id.eql(ops.q(33)),
                    );
                    try std.testing.expect(
                        requests.program.tuple.rs1.eql(ops.q(24)),
                    );
                    try std.testing.expect(
                        requests.state.emit.tuple.pc.eql(ops.q(0x1018)),
                    );
                    try std.testing.expect(
                        requests.state.emit.tuple.clock.eql(ops.q(13)),
                    );
                }

                test "jal oracle: forged link register is rejected" {
                    var row = zeroRow();
                    row.enabler = S.one();
                    row.pc = ops.q(100);
                    row.rd.addr = S.one();
                ...
                test "jal oracle: duplicate predecessor request is exact" {
                    var row = zeroRow();
                    row.enabler = S.one();
                    row.rd.addr = ops.q(4);
                    ...
                    try std.testing.expect(requests.consume[0].tuple.addr.eql(
                        requests.consume[1].tuple.addr,
                    ));
                    try std.testing.expect(requests.consume[0].tuple.clock.eql(
                        requests.consume[1].tuple.clock,
                    ));
                    try std.testing.expect(
                        requests.consume[0].tuple.limbs[0].eql(
                            requests.consume[1].tuple.limbs[0],
                        ),
                    );
                }

                test "jal oracle: adapter preserves physical order" {
                    ...
                    try std.testing.expect(
                        row.rd.previous_clock.eql(ops.q(3)),
                    );
