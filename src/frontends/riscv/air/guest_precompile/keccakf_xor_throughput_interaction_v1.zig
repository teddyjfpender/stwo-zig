//! Shard interaction materializer for the hybrid Keccak candidate.
...
const plan = @import("keccakf_xor_throughput_interaction_plan_v1.zig");
...
pub const Result = struct {
    ...
    pub fn tableTotal(self: *const Result) QM31 {
        return sum(self.claims[0..plan.table_batch_count]);
    }

    pub fn ioTotal(self: *const Result) QM31 {
        return sum(self.claims[plan.table_batch_count..plan.permutation_batch_count]);
    }

    pub fn callerTotal(self: *const Result) QM31 {
        return sum(self.claims[plan.permutation_batch_count..]);
    }
};
...
comptime {
    if (column_count != 3_740 or batch_count != 935 or production_active)
        @compileError("Keccak xor-throughput materializer drifted");
}
