//! Genuine, nonproduction Stwo adapter for one bulk-memcpy trace family.
//!
//! Both caller and word configurations use a two-bit quotient expansion and
//! split.  The caller is intentionally overprovisioned so it can coexist with
//! the degree-four boundary-aware word component in one proof.  No production
//! registry imports this adapter.
...
pub fn ComponentWithCompositionLogSplit(
    ...
    {
        @compileError("invalid bulk memcpy composition log split");
    }
    ...
    return struct {
        ...
        pub fn evaluateConstraintQuotientsAtPoint(
            ...
            const pairs = Config.rowPairs(QM31, &main, self.relations);
        ...
        fn prepareDomainEvaluator(
            ...
            const local_eval_log_size = std.math.add(
                u32,
                self.claim.log_size,
                quotient_expansion_bits,
            ) catch return error.InvalidEvaluationLogSize;
            const eval_log_size = local_eval_log_size;
        ...
        fn run(self: *Self, context: *task_graph.TaskContext) !void {
            ...
            for (0..self.eval_size) |row| {
                ...
                try Config.evaluate(
                    ...
                    &previous,
                    &next,
                    self.evaluations[trace_mod.domain_first_column][row],
                    self.evaluations[trace_mod.domain_last_column][row],
                    self.evaluations[trace_mod.active_prefix_column][row],
                    self.component.relations,
                    &sink,
                );
                const pairs = Config.rowPairs(M31, &main, self.component.relations);
                for (pairs, 0..) |pair, batch| sink.add(logup.pairConstraint(
...
comptime {
    ...
    {
        @compileError("bulk memcpy candidate STARK contract drifted");
    }
}
