...
const legacy = @import("../semantics/base_alu_imm_legacy_test_oracle.zig");
const symbolic = @import("../extract/symbolic.zig");
const authority = @import("typed_base_alu_imm_authority.zig");
const typed_addi = @import("typed_addi.zig");
const witness = @import("typed_base_alu_imm_witness.zig");

test "E-020 BASE_ALU_IMM authority binding is exact pointer-free and source independent" {
    var generated = try typed_addi.build(std.testing.allocator, .generated);
    var relocated = try typed_addi.build(std.testing.allocator, .{ .file = .{
        .path = "relocated/base_alu_imm.air",
    ...
    try std.testing.expectEqual(witness.WITNESS_BINDING_DIGEST, binding.witness_binding_digest);
    ...
    try std.testing.expectEqual(actual.identityDigest(), relocated_authority.identityDigest());
    try std.testing.expectEqualDeep(actual, authority.Authority.pinned());

    generated.deinit();
    const retired = try actual.retire(instruction(.ANDI, 31, 7, -2048), 0xdead_beef);
    try std.testing.expectEqual(@as(u32, 0xdead_b800), retired.visible_value);
}

test "E-020 BASE_ALU_IMM authority rejects every binding surface" {
    var definition = try typed_addi.build(std.testing.allocator, .generated);
    ...
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
    malformed.lookups[7].domain = .range_check_8_8;
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
    malformed.lookups[13].access_ordinal = 1;
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
...
test "E-020 BASE_ALU_IMM direct roots and ordered relations equal independent legacy polynomials" {
...
test "E-020 BASE_ALU_IMM one authority owns every result x0 witness and AIR row" {
    const admitted = try authenticatedAuthority();
    const opcodes = [_]@import("../../runner/decode.zig").Opcode{ .ADDI, .XORI, .ORI, .ANDI };
    const sources = [_]u32{ 0, 1, 0x7fff_ffff, 0x8000_0000, 0xffff_ffff, 0x1357_9bdf };
    const immediates = [_]i32{ -2048, -1, 0, 1, 2047 };
    var clock: u32 = 1;
    for (opcodes) |opcode| for (sources) |source| for (immediates) |imm| {
        for ([_]u5{ 0, 1, 7, 31 }) |rd| {
            const rs1: u5 = if ((clock & 1) == 0) rd else 6;
            const inst = instruction(opcode, rd, rs1, imm);
            const effective_source = if (rs1 == 0) 0 else source;
            const retirement = try admitted.retire(inst, effective_source);
            const expected = resultFor(opcode, effective_source, imm);
            try std.testing.expectEqual(expected, retirement.attempted_value);
            try std.testing.expectEqual(if (rd == 0) 0 else expected, retirement.visible_value);

            const row = makeRow(inst, effective_source, retirement.visible_value, clock);
    ...
    try std.testing.expectError(
        error.WrongBaseAluImmOpcode,
        admitted.retire(instruction(.ADD, 1, 2, 0), 7),
    );
}

test "E-020 BASE_ALU_IMM fixed direct evaluator retains legacy throughput" {
    ...
    std.debug.print(
        "\n  E-020 BASE_ALU_IMM fixed direct={d} ns legacy={d} ns speed={d:.4}x\n",
        .{ fixed_median, old_median, @as(f64, @floatFromInt(old_median)) /
            @as(f64, @floatFromInt(fixed_median)) },
    );
    try std.testing.expect(@as(u128, fixed_median) * 100 <=
        @as(u128, old_median) * 115);
}
...
fn expectInvalidBinding(definition: *const typed_addi.Definition, binding: authority.Binding) !void {
    try std.testing.expectError(
...
fn instruction(
    ...
    return .{ .opcode = opcode, .rd = rd, .rs1 = rs1, .rs2 = @truncate(@as(u32, @bitCast(imm))), .imm = imm };
}

fn resultFor(opcode: @import("../../runner/decode.zig").Opcode, source: u32, imm: i32) u32 {
...
fn makeRow(inst: authority.DecodedInst, source: u32, result: u32, clock: u32) authority.TraceRow {
    const source_clock = (clock - 1) * 4 + 1;
    const destination_clock = source_clock + 1;
    const aliased = inst.rd == inst.rs1;
    return .{
        .clk = clock,
        .pc = 0x1000 +% (clock - 1) *% 4,
        .opcode = inst.opcode,
        ...
        .rs2 = inst.rs2,
        .imm = inst.imm,
        .rs1_val = if (inst.rs1 == 0) 0 else source,
        .rs2_val = 0,
        .rs1_prev_clk = if (source_clock > 1) source_clock - 1 else 0,
        .rs2_prev_clk = 0,
        .rd_prev_val = if (inst.rd == 0) 0 else if (aliased) source else 0x55aa_1234,
        .rd_prev_clk = if (aliased) source_clock else if (destination_clock > 1) destination_clock - 1 else 0,
        .rd_val = result,
        .mem_addr = 0,
        .mem_val = 0,
        .mem_prev_word = 0,
...
fn measureFixed(
    ...
    for (0..iterations) |index| {
        const direct = try admitted.evaluateDirect(QM31, &rows[index & (rows.len - 1)], active);
