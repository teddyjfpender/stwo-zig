//! Parallel prepared-domain owner for the degree-five Poseidon2 candidate.
//!
//! Coordinator work materializes immutable quotient-domain views once.  The
//! row kernel then partitions the log-N+2 domain across one bounded
...
pub fn Namespace(comptime contract: anytype) type {
    return struct {
        ...
        const telemetry = @import("typed_poseidon2_degree5_prepared_telemetry.zig");
        const Component = contract.Component;
        const PreparedProgram = contract.PreparedProgram;
        const MAIN_COLUMNS = contract.MAIN_COLUMNS;
        const INTERACTION_COLUMNS = contract.INTERACTION_COLUMNS;
        const DIRECT_CONSTRAINTS = contract.DIRECT_CONSTRAINTS;
        ...
        pub fn prepare(
            ...
            var program = try PreparedProgram.initValidated(
                allocator,
                component.candidate,
            );
            ...
            const layout_ns = stage_timer.lap();
            ...
            const source_stage_ns = stage_timer.lap();

            var twiddle_ns: u64 = 0;
            var retained_extension_ns: u64 = 0;
            var recomputed_extension_ns: u64 = 0;
            ...
            if (owned.items.len != 0) {
                ...
                if (interpolations.items.len != 0) {
                    ...
                    recomputed_extension_ns = stage_timer.lap();
                }
                ...
                retained_extension_ns = stage_timer.lap();
            }
            ...
            const reservation = try resources(
                eval_size,
                source_count,
                owned_buffers.len,
                program.allocatedBytes(),
            );
            ...
            program_owned = false;
            const finalize_ns = stage_timer.lap();
            telemetry.recordPrepare(.{
                .layout_ns = layout_ns,
                .source_stage_ns = source_stage_ns,
                .twiddle_ns = twiddle_ns,
                .retained_extension_ns = retained_extension_ns,
                .recomputed_extension_ns = recomputed_extension_ns,
                .finalize_ns = finalize_ns,
                .source_columns = source_count,
                .borrowed_columns = source_count - owned_buffers.len,
                .retained_columns = retained_extensions.items.len,
                .recomputed_columns = recomputed_extensions.items.len,
            });
            return .{
                .context = state,
        ...
        const PreparedDomainState = struct {
            ...
            fn runErased(
                ...
                defer telemetry.recordRun(timer.read(), self.eval_size);
            ...
            fn evaluateRange(
                ...
                for (row_start..row_end) |row| {
                    ...
                    var main: [MAIN_COLUMNS]M31 = undefined;
                    ...
                    const evaluated = try component.evaluateConstraintsPreparedBase(
                        main,
                        &self.program,
                        self.evaluations[1][row],
                        self.evaluations[0][row],
                        sums,
                        previous,
                    );
                    var folded = QM31.zero();
                    for (evaluated.base, 0..) |constraint, index| {
                        folded = folded.add(
                            powers[powers.len - 1 - index].mulM31(constraint),
                        );
                    }
                    for (evaluated.secure, 0..) |constraint, index| {
                        const constraint_index = DIRECT_CONSTRAINTS + index;
                        folded = folded.add(
                            powers[powers.len - 1 - constraint_index].mul(
                                constraint,
                            ),
                        );
                    }
        ...
        fn resources(
            ...
            shared = std.math.add(usize, shared, program_bytes) catch
                return error.ResourceReservationOverflow;
            return .{
