...
test "typed LUI has exact production columns direct roots and ordered events" {
    ...
    try std.testing.expectEqual(typed_lui.MAIN_COLUMN_COUNT, layout.main().len);
    const physical = authored.columns.physical();
    const physical_names = witness_layout.columnNames(.lui);
    ...
    var authored_bindings: [typed_lui.MAIN_COLUMN_COUNT + 1]Binding = undefined;
    for (physical, authored_bindings[0..typed_lui.MAIN_COLUMN_COUNT], 0..) |
        value,
        *binding,
        column,
    | binding.* = .{ .value = value, .column = @intCast(column) };
    authored_bindings[typed_lui.MAIN_COLUMN_COUNT] = .{
        .value = authored.is_active,
        .column = typed_lui.MAIN_COLUMN_COUNT,
    };
    var production_bindings: [typed_lui.MAIN_COLUMN_COUNT + 1]Binding = undefined;
    for (layout.main(), production_bindings[0..typed_lui.MAIN_COLUMN_COUNT]) |
        column,
        *binding,
    | binding.* = .{ .value = column.value, .column = column.reference.local_index };
    production_bindings[typed_lui.MAIN_COLUMN_COUNT] = .{
        .value = imported.selector,
        .column = typed_lui.MAIN_COLUMN_COUNT,
    };

    const authored_fingerprints = try fingerprintProgram(
        std.testing.allocator,
        &authored.arena,
        &authored_bindings,
    );
    defer std.testing.allocator.free(authored_fingerprints);
    const production_fingerprints = try fingerprintProgram(
        std.testing.allocator,
        &imported.imported.arena,
        &production_bindings,
    );
    defer std.testing.allocator.free(production_fingerprints);

    try std.testing.expectEqual(
        typed_lui.DIRECT_CONSTRAINT_COUNT,
    ...
    for (authored.constraints, imported.direct_constraints) |actual_id, expected_id| {
        ...
        try std.testing.expectEqual(
            production_fingerprints[types.idIndex(expected.root)],
            authored_fingerprints[types.idIndex(actual.root)],
        );
    }
    ...
    try std.testing.expectEqual(typed_lui.RELATION_EVENT_COUNT, imported.lookups.len);
    for (imported.lookups, expected_kinds, 0..) |expected, expected_kind, index| {
        ...
        for (expected_values, actual.values) |expected_value, actual_value| {
            try std.testing.expectEqual(
                production_fingerprints[types.idIndex(expected_value)],
                authored_fingerprints[types.idIndex(actual_value)],
            );
        }
        const signed_liveness = switch (expected.role) {
            .emit => authored_fingerprints[types.idIndex(actual.liveness)],
            .request, .consume => unaryFingerprint(
                5,
                authored_fingerprints[types.idIndex(actual.liveness)],
            ),
        };
        try std.testing.expectEqual(
            production_fingerprints[types.idIndex(expected.numerator)],
            signed_liveness,
        );
...
test "typed LUI identity is source-independent and pins v7 v5" {
    var generated = try typed_lui.build(std.testing.allocator, .generated);
    defer generated.deinit();
    var first = try typed_lui.build(std.testing.allocator, fileLocation(
        "guest/lui.air",
        17,
        4,
    ));
    defer first.deinit();
    var second = try typed_lui.build(std.testing.allocator, fileLocation(
        "moved/lui.air",
        91,
        12,
    ));
    ...
    try std.testing.expectEqual(
        ...
        try digest.computeV5(&generated.arena),
    );
    const identity_hex = std.fmt.bytesToHex(generated_identity.bytes, .lower);
    try std.testing.expectEqualStrings(
        typed_lui.SEMANTIC_DIGEST_HEX,
        &identity_hex,
    );

    try std.testing.expectError(
        error.MachineDerivedRequiresManifestV5,
        manifest.serializeAllocV4(std.testing.allocator, &generated.arena),
    );
    try std.testing.expectError(
        error.SequentialRetirementRequiresManifestV7,
        manifest.serializeAllocV5(std.testing.allocator, &generated.arena),
    );
    try std.testing.expectError(
        error.SequentialRetirementRequiresManifestV7,
        manifest.serializeAllocV6(std.testing.allocator, &generated.arena),
    );
    try std.testing.expectError(error.InvalidEffect, digest.computeV3(&generated.arena));
    try std.testing.expectError(error.InvalidEffect, digest.computeV4(&generated.arena));
    const exact = try manifest.serializeAllocV7(std.testing.allocator, &generated.arena);
    ...
    try std.testing.expectEqual(
        manifest.sequential_retirement_format_version,
    ...
    try std.testing.expectEqual(@as(usize, 1205), exact.len);
}

test "typed LUI honest and forged rows match production direct semantics" {
    var authored = try typed_lui.build(std.testing.allocator, .generated);
    defer authored.deinit();
    var imported = try shadow_program.buildProduction(
        std.testing.allocator,
    ...
    var row = honestRow(7, 1, 0xc, 0xab, 0xde);
    try expectDirectParityAndZero(&authored, &imported, &bindings, row);
    row = honestRow(0, 1, 0xf, 0x31, 0x82);
    try expectDirectParityAndZero(&authored, &imported, &bindings, row);

    var forged = honestRow(7, 1, 0xc, 0xab, 0xde);
    forged[11] = forged[11].add(M31.one());
    try expectConstraintNonZero(&authored, &bindings, forged, 6);
    forged = honestRow(7, 1, 0xc, 0xab, 0xde);
    forged[16] = M31.zero();
    try expectConstraintNonZero(&authored, &bindings, forged, 2);
    forged = honestRow(7, 1, 0xc, 0xab, 0xde);
    forged[17] = M31.one();
    try expectConstraintNonZero(&authored, &bindings, forged, 3);
    forged = honestRow(0, 1, 0xf, 0x31, 0x82);
    forged[12] = M31.one();
    try expectConstraintNonZero(&authored, &bindings, forged, 7);
    forged = honestRow(7, 1, 0xc, 0xab, 0xde);
    ...
    forged = honestRow(7, 1, 0xc, 0xab, 0xde);
    forged[18] = M31.zero();
    try expectConstraintNonZero(&authored, &bindings, forged, 8);
}

fn fileLocation(path: []const u8, line: u32, column: u32) typed_lui.Location {
    return .{ .file = .{
        .path = path,
...
fn typedBindings(authored: *const typed_lui.Definition) [19]Binding {
    var bindings: [19]Binding = undefined;
    for (authored.columns.physical(), bindings[0..18], 0..) |value, *binding, column|
        binding.* = .{ .value = value, .column = @intCast(column) };
    bindings[18] = .{ .value = authored.is_active, .column = 18 };
    return bindings;
}

fn honestRow(rd: u32, active: u32, imm0: u32, imm1: u32, imm2: u32) [19]M31 {
    var row = [_]M31{M31.zero()} ** 19;
    row[0] = M31.fromCanonical(active);
    row[1] = M31.fromCanonical(37);
    row[2] = M31.fromCanonical(0x1040);
    row[3] = M31.fromCanonical(rd);
    row[4] = M31.fromCanonical(0x11);
    row[5] = M31.fromCanonical(0x22);
    row[6] = M31.fromCanonical(0x33);
    row[7] = M31.fromCanonical(0x44);
    row[8] = M31.fromCanonical(3);
    if (rd != 0) {
        row[10] = M31.fromCanonical(imm0 << 4);
        row[11] = M31.fromCanonical(imm1);
        row[12] = M31.fromCanonical(imm2);
        row[16] = M31.one();
        row[17] = M31.fromCanonical(rd).inv() catch unreachable;
    }
    row[13] = M31.fromCanonical(imm0);
    row[14] = M31.fromCanonical(imm1);
    row[15] = M31.fromCanonical(imm2);
    row[18] = row[0];
    return row;
}

fn expectDirectParityAndZero(
    authored: *const typed_lui.Definition,
    ...
    for (authored.constraints, imported.direct_constraints) |actual_id, expected_id| {
        ...
        const actual = actual_values[types.idIndex(actual_root)];
        const expected = expected_values[types.idIndex(expected_root)];
