//! Allocation-conscious replay helpers for native BRANCH_LT semantic tests.
...
pub const BINDING_COUNT: usize = typed.MAIN_COLUMN_COUNT + 3;
pub const Binding = struct { value: types.ValueId, column: u32 };

/// Binds the two closed polynomial witnesses to their committed physical
/// columns. The replay row itself remains production-shaped: main columns plus
...
pub fn typedBindings(definition: *const typed.Definition) [BINDING_COUNT]Binding {
    ...
    bindings[typed.MAIN_COLUMN_COUNT + 2] = .{
        .value = definition.branch_target_polynomial,
        .column = 32,
    };
    return bindings;
}
