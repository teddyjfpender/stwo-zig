...
test "typed SHIFTS_IMM rejects every binding identity mutation" {
    ...
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[1].flag_column = 26;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[2].algorithm = .rv32_logical_left;
    try expectInvalidBinding(&definition, &malformed);
}

test "typed SHIFTS_IMM authenticates definition before row recipe" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
...
test "typed SHIFTS_IMM row forgeries reject atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
...
test "typed SHIFTS_IMM validates the complete batch before the first write" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
    ...
    var rows = [_]witness.TraceRow{
        support.makeRow(.SLLI, 3, 4, 0x1020_3040, 7, 20, 0x1000, 0, 0, 0),
        support.makeRow(.SRLI, 5, 6, 0x8000_0000, 8, 21, 0x1004, 0, 0, 0),
        support.makeRow(.SRAI, 7, 8, 0xffff_ffff, 31, 22, 0x1008, 0, 0, 0),
    };
    rows[1].rd_val ^= 1;
    try std.testing.expectError(
...
test "typed SHIFTS_IMM rejects shapes aliases and address overflow atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
...
test "typed SHIFTS_IMM construction releases every allocation failure" {
