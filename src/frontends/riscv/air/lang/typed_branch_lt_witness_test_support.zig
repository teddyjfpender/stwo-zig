...
pub fn less(opcode: Opcode, lhs: u32, rhs: u32) bool {
    return switch (opcode) {
        .BLT, .BGE => @as(i32, @bitCast(lhs)) < @as(i32, @bitCast(rhs)),
        .BLTU, .BGEU => lhs < rhs,
        else => unreachable,
    };
}
...
pub fn taken(opcode: Opcode, lhs: u32, rhs: u32) bool {
    const is_less = less(opcode, lhs, rhs);
    return switch (opcode) {
        .BLT, .BLTU => is_less,
        .BGE, .BGEU => !is_less,
        else => unreachable,
...
pub fn makeRow(
    ...
    std.debug.assert(switch (opcode) {
...
pub fn expectRelationParity(
    ...
    const expected = try opcode_entries.fromMain(.branch_lt, &main);
