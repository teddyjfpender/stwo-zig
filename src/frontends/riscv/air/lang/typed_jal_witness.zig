//! Allocation-free production witness generation for native typed RV32 JAL.
//!
//! Cold construction authenticates the semantic definition, exact twenty-slot
//! row-source recipe, wrapping link-address policy, and inverse-or-zero hint.
//! Accepted hot rows write directly into caller-owned final SoA storage with
//! no allocator, scratch copy, string dispatch, or runtime recipe loop.
...
pub const WITNESS_BINDING_DOMAIN_SEPARATOR =
    "stwo-zig/typed-air/jal-witness-binding/v1";
pub const WITNESS_BINDING_DIGEST_HEX =
    "296e9190d221a1e00ba13c7b8957ec2e9a53911eca308c8a5624279d1d86e47a";
pub const WITNESS_BINDING_DIGEST: digest.Digest = hexDigest(
    WITNESS_BINDING_DIGEST_HEX,
    "invalid typed JAL witness-binding digest",
);

/// Stable numeric source codes. Explicit values are persisted witness
/// identity, independent of field names and declaration layout.
pub const RowSource = enum(u8) {
    ...
    derived_link_byte_0 = 14,
    derived_link_byte_1 = 15,
    derived_link_byte_2 = 16,
    derived_link_byte_3 = 17,
    hint_rd_nonzero = 18,
...
pub const ArithmeticAlgorithm = enum(u8) {
    rv32_wrapping_pc_plus_four_link = 0,
};
...
pub const CANONICAL_ARITHMETIC = ArithmeticBinding{
    .algorithm = .rv32_wrapping_pc_plus_four_link,
    .pc_column = 2,
...
/// Pointer-free executable identity that remains valid after the authored
/// arena is destroyed.
...
pub const ConstructionError = typed_jal.ValidationError || error{
    InvalidWitnessBinding,
};
pub const ExecutionError = direct_witness_executor.Error;

/// Immutable authenticated JAL witness executor. Generation is allocation-free
/// and reentrant after initialization.
...
pub inline fn writeActiveRow(
    ...
    // Source and destination are compile-time known after inlining. The recipe
    // remains the single ordering authority without runtime tag dispatch.
...
fn validateRow(row: TraceRow) ExecutionError!void {
    ...
    const target = row.pc +% immediate_bits;
    const link = row.pc +% 4;
    const expected_taken = target != link;
    if (row.opcode != .JAL or
        row.imm < -1_048_576 or row.imm > 1_048_574 or
        immediate_bits & 1 != 0 or
        row.pc & 3 != 0 or row.pc >= (@as(u32, 1) << 30) or
        target & 3 != 0 or target >= (@as(u32, 1) << 30) or
        row.next_pc != target or row.branch_taken != expected_taken or
        row.rd_val != (if (row.rd == 0) 0 else link) or
...
comptime {
    if (MAIN_COLUMN_COUNT != production_columns.JalColumns.N_COLUMNS)
        @compileError("typed JAL witness width drifted from production");
    const fields = @typeInfo(production_columns.JalColumns).@"struct".fields;
    ...
    | {
        if (!std.mem.eql(u8, spec.name, field.name))
            @compileError("typed JAL witness name drifted from production");
        if (spec.source != source or @intFromEnum(source) != index)
            @compileError("typed JAL numeric witness recipe is not canonical");
    }
}
