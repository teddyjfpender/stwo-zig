...
const legacy = @import("../../runner/witness/shifts_reg_legacy_test_oracle.zig");
...
pub fn result(opcode: Opcode, value: u32, raw_amount: u32) u32 {
    const amount: u5 = @truncate(raw_amount);
    return switch (opcode) {
        .SLL => value << amount,
        .SRL => value >> amount,
        .SRA => @bitCast(@as(i32, @bitCast(value)) >> amount),
        else => unreachable,
    };
}
...
pub fn makeRow(
    ...
    const output = result(opcode, rs1_value, rs2_value);
    return .{
        .clk = clock,
        .pc = pc,
        .opcode = opcode,
...
pub fn expectRelationParity(
    ...
    const expected = try opcode_entries.fromMain(.shifts_reg, &main);
