//! ReleaseFast paired throughput and scaling admission for MULH.
...
test "typed MULH hot rows retain paired legacy throughput and scaling" {
    ...
    for (rows, 0..) |*row, index| row.* = support.makeRow(
        support.operations[index % support.operations.len],
        @intCast(index & 31),
    ...
    for (log_rows, 0..) |log_size, case_index| {
        ...
        std.debug.print(
            "\n  typed MULH log_rows={d}: generated={d} ns " ++
