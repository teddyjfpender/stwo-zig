//! Fixed executable authority for RV32 BASE_ALU_IMM.
//!
//! One authenticated, pointer-free binding owns ADDI, XORI, ORI, and ANDI
//! architectural retirement, physical witness projection, direct roots, and
//! ordered lookup events. Construction authenticates `typed_addi`'s complete
//! graph and the complete-family witness recipe; hot execution retains neither
//! arena pointers nor allocators and performs no dynamic or string dispatch.
...
pub const DIRECT_CONSTRAINT_COUNT: usize = typed_addi.DIRECT_CONSTRAINT_COUNT;
pub const LOOKUP_COUNT: usize = typed_addi.RELATION_EVENT_COUNT;
pub const LOOKUP_BATCH_SIZE: usize = typed_addi.RELATION_BATCH_SIZE;
pub const OPCODE_IDS = [4]u32{
    typed_addi.ADDI_OPCODE_ID,
    typed_addi.XORI_OPCODE_ID,
    typed_addi.ORI_OPCODE_ID,
    typed_addi.ANDI_OPCODE_ID,
};
...
pub const AUTHORITY_BINDING_DOMAIN_SEPARATOR =
    "stwo-zig/typed-air/base-alu-imm-fixed-authority/v1";

// Updated only through the binding-identity admission test. Keeping a literal
// receipt makes changes to execution, roots, lookup order, or witness geometry
// an explicit protocol review rather than an accidental rebuild effect.
pub const AUTHORITY_BINDING_DIGEST_HEX =
    "f56f71402fde0f68acede272b0719fd0fd6503346c39a7f6e6e2840484c89b16";
pub const AUTHORITY_BINDING_DIGEST: digest.Digest = blk: {
    var result: digest.Digest = undefined;
    _ = std.fmt.hexToBytes(&result, AUTHORITY_BINDING_DIGEST_HEX) catch
        @compileError("invalid typed BASE_ALU_IMM authority-binding digest");
    break :blk result;
};

pub const ExecutionRecipe = enum(u8) {
    rs1_immediate_write_x0_discard = 0,
};
...
pub const DirectRecipe = enum(u8) {
    ...
    ori_bit = 3,
    andi_bit = 4,
    immediate_sign_bit = 5,
    add_carry_0 = 6,
    add_carry_1 = 7,
    add_carry_2 = 8,
    add_carry_3 = 9,
    destination_nonzero_bit = 10,
    destination_zero_address = 11,
    destination_inverse = 12,
    destination_result_0 = 13,
    destination_result_1 = 14,
    destination_result_2 = 15,
    destination_result_3 = 16,
    source_read_only_0 = 17,
    source_read_only_1 = 18,
    source_read_only_2 = 19,
    source_read_only_3 = 20,
    active_placement = 21,
};
...
pub const CANONICAL_LOOKUP_RECIPE = [LOOKUP_COUNT]LookupDescriptor{
    ...
    .{ .recipe = .immediate_range, .domain = .range_check_8_11, .role = .request, .arity = 2, .access_ordinal = null },
...
pub const Binding = struct {
    ...
    pub fn canonical(definition: *const typed_addi.Definition) witness.ConstructionError!Binding {
...
pub const CANONICAL_BINDING = Binding{
    ...
    .execution = .rs1_immediate_write_x0_discard,
...
pub const ExecutionError = error{ WrongBaseAluImmOpcode, InvalidImmediate };
...
inline fn resultFor(opcode: decode.Opcode, source: u32, immediate: i32) u32 {
...
pub const Authority = struct {
    ...
    pub inline fn retire(
        ...
        if (!isFamilyOpcode(instruction.opcode)) return error.WrongBaseAluImmOpcode;
...
pub fn Evaluator(comptime S: type) type {
    return struct {
        ...
        pub fn directRow(row: Row, is_active: S) DirectConstraints {
            ...
            index += 1;
            inline for (.{ row.is_addi, row.is_xori, row.is_ori, row.is_andi }) |flag| {
            ...
            var carry = S.zero();
            inline for (0..4) |limb| {
                const numerator = row.rs1.next[limb]
                    .add(immediate[limb]).add(carry).sub(row.result[limb]);
                carry = numerator.mul(ops.INV_BYTE_RADIX());
                out[index] = ops.selected(row.is_addi, ops.bit(carry));
                index += 1;
            }
        ...
        fn lookupsRowInto(row: Row, result: *e.List) void {
            ...
            const immediate = immediateLimbs(row);
            const opcode = row.is_addi.mul(ops.q(OPCODE_IDS[0]))
                .add(row.is_xori.mul(ops.q(OPCODE_IDS[1])))
                .add(row.is_ori.mul(ops.q(OPCODE_IDS[2])))
                .add(row.is_andi.mul(ops.q(OPCODE_IDS[3])));
            const unsigned_immediate = row.imm_0
            ...
            appendLookup(result, 0, negative_active, .{
                row.pc, opcode, row.rd.addr, row.rs1.addr, unsigned_immediate,
            });
            appendLookup(result, 1, negative_active, .{
                row.imm_0, row.imm_1.mul(ops.q(1 << 8)),
            });
            ...
            inline for (0..4) |limb| appendLookup(result, 7 + limb, negative_bitwise, .{
                row.rs1.next[limb], immediate[limb], row.result[limb], operation_id,
            });
            appendLookup(result, 11, negative_active, .{ row.result[0], row.result[1] });
            appendLookup(result, 12, negative_active, .{ row.result[2], row.result[3] });
        ...
        inline fn appendLookup(
            ...
            {
                @compileError("typed BASE_ALU_IMM fixed lookup append drifted");
            };
        ...
        inline fn immediateLimbs(row: Row) [4]S {
...
comptime {
    ...
    )) @compileError("typed BASE_ALU_IMM canonical authority digest drifted");
}
