//! Backend-neutral polynomial export for the degree-six Poseidon2 candidate.
//!
//! The candidate compiler remains the sole owner of the 146 permutation
...
//! Production activation remains false until a backend evaluates the exact
//! three-bit quotient expansion rather than the legacy one-bit domain.
...
comptime {
    if (component_mod.DIRECT_CONSTRAINTS != 149 or
        component_mod.LOGUP_CONSTRAINTS != 2 or
        component_mod.MAIN_COLUMNS != 161 or
        component_mod.INTERACTION_COLUMNS != 8 or
        component_mod.QUOTIENT_EXPANSION_BITS != 3)
    {
        @compileError("degree-six backend export geometry drifted");
    }
}
