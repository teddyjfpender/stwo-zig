//! ReleaseFast paired throughput and scaling admission for BASE_ALU_IMM.

const std = @import("std");
const builtin = @import("builtin");
const M31 = @import("stwo_core").fields.m31.M31;
const Opcode = @import("../../isa/decode.zig").Opcode;
const legacy_oracle = @import("../../runner/witness/base_alu_imm_legacy_test_oracle.zig");
...
test "typed BASE_ALU_IMM hot rows retain paired legacy throughput and scaling" {
    // Debug and ReleaseSafe instrument a different hot loop. Correctness,
    // allocation, and memory-safety gates cover those modes separately.
    ...
    for (rows, 0..) |*row, index| {
        ...
        const rs1: u5 = @intCast((index *% 17 +% 3) & 31);
        row.* = support.makeRow(
            operations[index & 3],
            rd,
            rs1,
            @truncate(index *% 0x9e37_79b9),
            @as(i32, @intCast(index & 0xfff)) - 2048,
            @intCast(index + 2),
    ...
    // Warm both instruction streams and all destination pages before timing.
    ...
    for (log_rows, 0..) |log_size, case_index| {
        ...
        for (0..nine_samples) |sample| {
            // Alternating order cancels first-run and temperature bias within
            // each pair without admitting cross-process benchmark history.
            if ((sample & 1) == 0) {
        ...
        std.debug.print(
            "\n  typed BASE_ALU_IMM log_rows={d}: generated={d} ns " ++
        ...
        // Fast local screen before the normative benchmark campaign: speed
        // is baseline/candidate and paired median must retain the 0.97 floor.
        try std.testing.expect(generated_median * 97 <= legacy_median * 100);
    }

    // Each adjacent case has 16x as many rows; allow 10% scheduling noise.
