...
test "typed MULH rejects every binding identity mutation" {
    ...
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.slots[0].source = .pc;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[0].opcode_id = 99;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[1].flag_column = 40;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[2].algorithm = .rv32_signed_signed_high;
    try expectInvalidBinding(&definition, &malformed);
}

test "typed MULH authenticates definition before row recipe" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
...
test "typed MULH rejects semantic row forgeries before mutation" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
...
test "typed MULH validates the complete batch before the first write" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
    ...
    var rows = [_]witness.TraceRow{
        support.makeRow(.MULH, 3, 4, 5, 0x8000_0000, 7, 20, 0x1000, 0, 0, 0, 0),
        support.makeRow(.MULHSU, 6, 7, 8, 0xffff_ffff, 8, 21, 0x1004, 0, 0, 0, 0),
        support.makeRow(.MULHU, 9, 10, 11, 0xffff_ffff, 0xffff_ffff, 22, 0x1008, 0, 0, 0, 0),
    };
    rows[1].rd_val ^= 1;
    try std.testing.expectError(
...
test "typed MULH rejects shapes aliases and address overflow atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
...
test "typed MULH construction releases every allocation failure" {
