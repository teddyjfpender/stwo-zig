//! Parallel main-trace writer for the degree-bounded Poseidon2 candidate.
//!
...
pub fn generateMain(
    ...
    if (candidate.profile != .degree6 or log_size >= @bitSizeOf(usize))
...
comptime {
    if (candidate_mod.Profile.degree6.expected().main_columns != 161) {
        @compileError("degree-six trace geometry drifted");
    }
}
