...
const backend = @import("typed_poseidon2_degree_bounded_backend.zig");
...
test "degree-six backend export covers the exact direct and lookup relation" {
    ...
    try std.testing.expectEqual(@as(usize, 149), exported.lookup_constraints.start);
    ...
    try std.testing.expectEqual(@as(usize, 161), exported.lookup.main_column_count);
    ...
    for (exported.base_partitions[0..exported.base_partition_count]) |partition| {
        ...
        try std.testing.expectEqual(@as(usize, 162), program.column_count);
    ...
    try std.testing.expectEqual(@as(usize, 149), covered);
    ...
    try std.testing.expectEqual(@as(usize, 161), lookups.column_count);
...
test "degree-six backend export rejects noncanonical partitions" {
    var candidate = try candidate_mod.Candidate.init(
        std.testing.allocator,
        .degree6,
    );
