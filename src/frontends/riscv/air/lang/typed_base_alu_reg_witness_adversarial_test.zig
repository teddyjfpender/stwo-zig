...
test "typed BASE_ALU_REG rejects every binding identity mutation" {
    ...
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[2].result = .rv32_bitwise_and;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[3].bitwise_active = false;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[4].bitwise_operation_id = 1;
    try expectInvalidBinding(&definition, &malformed);
}

test "typed BASE_ALU_REG authenticates definition before row recipe" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
...
test "typed BASE_ALU_REG row forgeries reject atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
...
test "typed BASE_ALU_REG rejects shapes aliases and address overflow atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
...
test "typed BASE_ALU_REG construction releases every allocation failure" {
