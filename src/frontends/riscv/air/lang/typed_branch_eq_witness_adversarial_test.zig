...
test "typed BRANCH_EQ rejects every binding identity mutation" {
    ...
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[0].opcode_id = typed.BNE_OPCODE_ID;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[0].flag_column = 29;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.operations[0].decision = .not_equal;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.inverse_algorithm = @enumFromInt(1);
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.events[0].kind = .state_consume;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.events[0].domain = .registers_state;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.events[0].role = .emit;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.events[0].arity = 4;
    try expectInvalidBinding(&definition, &malformed);
    malformed = canonical;
    malformed.events[0].access_ordinal = 1;
    try expectInvalidBinding(&definition, &malformed);
}

test "typed BRANCH_EQ authenticates definition before row recipe" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
...
test "typed BRANCH_EQ row forgeries reject atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
...
test "typed BRANCH_EQ ignores B-immediate destination metadata" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
    const valid = support.makeRow(.BEQ, 2, 3, 4, 4, 4092, 10, 0x10_000, 0, 0);
    var decoded = valid;
    decoded.rd = 31;
...
test "typed BRANCH_EQ rejects shapes aliases input overlap and overflow atomically" {
    var definition = try typed.build(std.testing.allocator, .generated);
    defer definition.deinit();
    const binding = try witness.WitnessBinding.canonical(&definition);
    const executor = try witness.Executor.init(&definition, &binding);
    ...
    const rows = [_]witness.TraceRow{
        support.makeRow(.BNE, 2, 3, 4, 5, 8, 10, 0x1000, 0, 0),
    };
