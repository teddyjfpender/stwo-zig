...
const legacy = @import("../semantics/shifts_reg_legacy_test_oracle.zig");
const symbolic = @import("../extract/symbolic.zig");
const authority = @import("typed_shifts_reg_authority.zig");
...
test "SHIFTS_REG fixed authority binding is source independent pointer-free and pinned" {
    ...
    var relocated = try typed.build(std.testing.allocator, .{ .file = .{
        .path = "relocated/shift/shifts_reg.air",
    ...
    try std.testing.expectEqual(@as(usize, 60), authority.MAIN_COLUMN_COUNT);
    try std.testing.expectEqual(@as(usize, 70), authority.DIRECT_CONSTRAINT_COUNT);
    try std.testing.expectEqual(@as(usize, 20), authority.LOOKUP_COUNT);
    ...
    try std.testing.expectEqual(@as(usize, 276), @sizeOf(authority.Binding));
    try std.testing.expectEqual(@as(usize, 308), @sizeOf(authority.Authority));
}

test "SHIFTS_REG authority rejects execution semantic witness direct and relation mutations" {
    ...
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
    malformed.lookups[14].domain = .range_check_20;
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
    malformed.lookups[18].access_ordinal = 2;
    try expectInvalidBinding(&definition, malformed);
    malformed = canonical;
...
test "SHIFTS_REG authority direct roots and ordered relations equal legacy polynomials" {
    const admitted = try authenticatedAuthority();
    var prng = std.Random.DefaultPrng.init(0x5348_4654_5245_4741);
    ...
    for (0..128) |_| {
        ...
        for (expected_direct.values, actual.direct_constraints.values[0..69]) |
            expected,
            got,
        | try std.testing.expect(expected.eql(got));
        try std.testing.expect(
            expected_placement.eql(actual.direct_constraints.values[69]),
        );
        try expectLookupListsEqual(QM31, expected_lookups, actual.lookup_entries);
    }
}

test "SHIFTS_REG authority direct and lookup programs are symbolically identical" {
    ...
    try std.testing.expectEqual(row.semantic.active().id, actual.active_row.id);
    for (direct.values, actual.direct_constraints.values[0..69]) |want, got|
        try std.testing.expectEqual(want.id, got.id);
    try std.testing.expectEqual(
        placement.id,
        actual.direct_constraints.values[69].id,
    );
    try expectLookupListsEqual(symbolic.Scalar, lookups, actual.lookup_entries);
}

test "SHIFTS_REG authority owns low-five shifts aliases witness and AIR" {
    const admitted = try authenticatedAuthority();
    const Case = struct {
        rd: u5,
        rs1: u5,
    ...
    const cases = [_]Case{
        ...
        .{ .rd = 9, .rs1 = 5, .rs2 = 9, .value = 0x8000_0001, .raw_amount = 0xffff_ffe7 },
        .{ .rd = 5, .rs1 = 5, .rs2 = 5, .value = 0x7fff_ffff, .raw_amount = 0 },
    ...
    var rows: [operations.len * cases.len]authority.TraceRow = undefined;
    var count: usize = 0;
    for (operations) |opcode| for (cases) |case| {
        ...
        const retirement = try admitted.retire(.{
            ...
            .rs2 = case.rs2,
            .imm = 0,
        }, rows[count].rs1_val, rows[count].rs2_val);
        try std.testing.expectEqual(@as(u5, @truncate(rows[count].rs2_val)), retirement.shift_amount);
        try std.testing.expectEqual(
            support.result(opcode, rows[count].rs1_val, rows[count].rs2_val),
            retirement.attempted_value,
        );
        try std.testing.expectEqual(rows[count].rd_val, retirement.visible_value);
        count += 1;
    };

    var storage: [authority.MAIN_COLUMN_COUNT][32]M31 = undefined;
    var columns: [authority.MAIN_COLUMN_COUNT][]M31 = undefined;
    for (&storage, &columns) |*owned, *view| view.* = owned;
    try admitted.generateMainInto(&columns, &rows, 5);
    ...
    try std.testing.expectError(
        error.WrongShiftsRegOpcode,
        admitted.retire(.{ .opcode = .ADD, .rd = 1, .rs1 = 2, .rs2 = 3, .imm = 0 }, 1, 2),
    );
    try std.testing.expectError(
        error.InvalidImmediate,
        admitted.retire(.{ .opcode = .SLL, .rd = 1, .rs1 = 2, .rs2 = 3, .imm = 1 }, 1, 2),
    );
}

test "SHIFTS_REG authority rejects forged rows atomically and releases allocation failures" {
    const admitted = try authenticatedAuthority();
    ...
    var columns: [authority.MAIN_COLUMN_COUNT][]M31 = undefined;
    for (&storage, &columns) |*owned, *view| view.* = owned;
    const honest = support.makeRow(.SRA, 3, 1, 2, 0x8000_0000, 31, 2, 0x1000, 0, 0, 0, 0);

    var forged = honest;
    forged.rd_val ^= 1;
    try expectInvalidRowAtomic(&admitted, &columns, &storage, forged, sentinel);
    forged = honest;
    forged.rs2_prev_clk = 8;
    try expectInvalidRowAtomic(&admitted, &columns, &storage, forged, sentinel);
    forged = honest;
    forged.next_pc +%= 4;
    try expectInvalidRowAtomic(&admitted, &columns, &storage, forged, sentinel);
...
test "SHIFTS_REG fixed direct and lookup evaluators retain legacy throughput" {
    ...
    const selector = QM31.fromBase(M31.fromCanonical(17));
    _ = try measureFixedDirect(&admitted, rows, selector, 1 << 10);
    _ = try measureLegacyDirect(rows, selector, 1 << 10);
    _ = try measureFixedLookups(&admitted, rows, 1 << 10);
    _ = try measureLegacyLookups(rows, 1 << 10);
    const samples = 15;
    ...
    for (0..samples) |sample| if ((sample & 1) == 0) {
        fixed_direct[sample] = try measureFixedDirect(&admitted, rows, selector, iterations);
        legacy_direct[sample] = try measureLegacyDirect(rows, selector, iterations);
    ...
    std.debug.print(
        "\n  SHIFTS_REG fixed authority: direct={d} ns legacy={d} ns " ++
...
fn legacyLookups(
    ...
    for (module.carryRangePairs(row.semantic)) |values|
        e.range88(&list, active.neg(), values);
    for (module.rdRangePairs(row.semantic)) |values|
        e.range88(&list, active.neg(), values);
...
fn measureLegacyDirect(
    ...
    for (0..iterations) |index| {
        ...
        const direct = Legacy.evaluate(row);
        for (direct.values) |value| absorbScalar(&checksum, value);
        absorbScalar(&checksum, Legacy.placementConstraint(row, selector));
    }
    const elapsed = timer.read();
    std.mem.doNotOptimizeAway(checksum);
    return elapsed;
