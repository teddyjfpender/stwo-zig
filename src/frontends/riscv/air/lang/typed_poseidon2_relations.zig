//! Authenticated relation lowering for the canonical typed Poseidon2 provider.
//!
...
//! shared typed plan used by production witness and cold equation admission.
...
const contract = @import("typed_poseidon2_relation_contract.zig");
pub const FORMAT_VERSION = contract.FORMAT_VERSION;
pub const POLICY_VERSION = contract.POLICY_VERSION;
pub const RELATION_DIGEST_FORMAT_VERSION = contract.RELATION_DIGEST_FORMAT_VERSION;
pub const RELATION_DIGEST_DOMAIN_SEPARATOR = contract.RELATION_DIGEST_DOMAIN_SEPARATOR;
pub const WIDTH = contract.WIDTH;
pub const N_EVENTS = contract.N_EVENTS;
pub const N_BATCHES = contract.N_BATCHES;
pub const N_SUMS = contract.N_SUMS;
pub const N_INTERACTION_COLUMNS = contract.N_INTERACTION_COLUMNS;
pub const MAX_ARITY = contract.MAX_ARITY;
pub const OUTPUT_COLUMN_START = contract.OUTPUT_COLUMN_START;
pub const Error = contract.Error;
pub const AuthenticationError = contract.AuthenticationError;
pub const ClaimError = contract.ClaimError;
pub const InteractionError = contract.InteractionError;
pub const PolicyId = contract.PolicyId;
pub const Identity = contract.Identity;
pub const EventId = contract.EventId;
pub const NumeratorFormula = contract.NumeratorFormula;
pub const TupleProjection = contract.TupleProjection;
pub const EventPlan = contract.EventPlan;
pub const BatchPlan = contract.BatchPlan;
...
const canonicalEvent = contract.canonicalEvent;
const canonicalBatch = contract.canonicalBatch;
