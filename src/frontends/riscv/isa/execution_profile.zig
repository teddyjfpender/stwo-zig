...
pub const secp256k1_recover_capability_bit: u64 = 4;
pub const ethereum_capability_bits: u64 =
    keccakf_capability_bit | secp256k1_recover_capability_bit;
pub const poseidon2_abi_version: u16 = 1;
pub const keccakf_abi_version: u16 = 1;
pub const ethereum_abi_version: u16 = 1;
...
/// SHA-256 of the canonical combined execution authority
/// `riscv.ethereum.keccakf_1600.secp256k1_recover.v1`.
pub const ethereum_semantic_digest = [32]u8{
    0xfb, 0xe8, 0x83, 0x3d, 0xe3, 0x5b, 0x29, 0xab,
    0x15, 0x5a, 0xfe, 0xd5, 0x8f, 0x59, 0x3d, 0x44,
    0xd2, 0xa7, 0x25, 0x7a, 0xd4, 0x49, 0x1d, 0x95,
    0x37, 0x42, 0xd3, 0x94, 0xda, 0x66, 0xcf, 0xc2,
};
...
test "execution profile wire IDs, names, and semantic identity are pinned" {
    ...
    try std.testing.expectEqual(@as(u16, 3), @intFromEnum(ExecutionProfile.rv32im_zkvm_ethereum_v1));
    ...
    try std.testing.expectEqualStrings(ethereum_name, ExecutionProfile.rv32im_zkvm_ethereum_v1.name());
    ...
    try std.testing.expectEqual(@as(u16, 1), ethereum_abi_version);
    try std.testing.expectEqual(@as(u64, 6), ethereum_capability_bits);
    ...
    try std.testing.expectEqual(ethereum_capability_bits, ExecutionProfile.rv32im_zkvm_ethereum_v1.requiredCapabilities());
    ...
    try std.testing.expectEqualSlices(u8, &semantic_digest, &ethereum_semantic_digest);
}
