//! Shared proof transaction for the 39-component resumed-segment outer AIR.
//!
...
pub fn ForBackend(comptime Backend: type, comptime Diagnostics: type) type {
    return struct {
        ...
        const canonical_identity =
            @import("canonical_proof_identity_v1.zig");
        const verified_publication =
            @import("segment_verified_publication_v2.zig");
        const verified_artifact =
            @import("segment_verified_artifact_v2.zig");
        const engine_storage = @import("transaction_storage_v2.zig");
        const engine_support = @import("segment_outer_transaction_support_v2.zig");

        const M31 = stwo_core.fields.m31.M31;
        const QM31 = stwo_core.fields.qm31.QM31;
        const recursion = struct {
            const engine = @import("engine.zig");
            const outer_parent_child_admission = @import("outer_parent_child_admission.zig");
        ...
        pub const Engine = recursion.engine.ProverEngineForBackend(Backend);
        ...
        pub const ProducerAllocator = prover_engine.tracked_smp_allocator.TrackedSmpAllocator;
        ...
        pub fn EngineKernel(comptime Cohort: type) type {
            ...
            return struct {
                ...
                fn proveAndVerifyTransaction(
                    ...
                    const proof_identity = try canonical_identity
                ...
                fn prove(
                    ...
                    const composition_diagnostic_enabled =
                        std.process.hasEnvVarConstant(
                            Diagnostics.COMPOSITION_DIAGNOSTIC_ENV,
                        );
                    if (composition_diagnostic_enabled)
                        try Diagnostics.validateCompositionDiagnosticRoster();
                    ...
                    if (composition_diagnostic_enabled) {
                        ...
                        try Diagnostics.diagnoseCompositionComponents(
                            allocator,
                            &scheme,
                            diagnostic_provers,
                ...
                fn verify(
                    ...
                    proof_identity: canonical_identity.CanonicalProofIdentityV1,
                    capture_out: *OuterProofCapture,
                    publication_out: *VerifiedSegmentV2PublicationV1,
                    witness_out: *RecursiveWitnessV1,
        ...
        pub fn testProtocolContract() !void {
