    ...
    return struct {
        ...
        pub fn proveAndVerifyCapturedImpl(
            ...
            return .{
                ...
                .vm_graph_nodes = if (vm_air) |prepared| prepared.view().circuit.nodes.len else 0,
                .vm_graph_inputs = if (vm_air) |prepared| prepared.view().circuit.bindings.len else 0,
                .vm_graph_outputs = if (vm_air) |prepared| prepared.view().circuit.outputs.len else 0,
                .vm_schedule_rows = if (vm_air) |prepared| prepared.view().preprocessing.rows.len else 0,
