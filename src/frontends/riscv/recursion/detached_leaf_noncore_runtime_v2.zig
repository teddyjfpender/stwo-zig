//! Shared leaf noncore runtime; the caller supplies the prepared leaf types.
pub fn For(comptime leaf_outer: type) type {
    return struct {
        const std = @import("std");
        const stwo_core = @import("stwo_core");
        const contract = @import("detached_leaf_noncore_contract_v2.zig");
        const support = @import("detached_leaf_noncore_support_v2.zig").For(leaf_outer);

        const M31 = stwo_core.fields.m31.M31;
        const manifest_mod = @import("air/segment_outer_adapter_manifest_v2.zig");
        const transcript_components = @import("segment_transcript_outer_components_v2.zig");
        const statement_components = @import("segment_statement_outer_components_v2.zig");
        const statement_source = @import("segment_statement_outer_source_v2.zig");
        const public_components = @import("segment_public_outer_components_v2.zig");
        const public_native_sum = @import("segment_public_native_sum_authority_v2.zig");
        const range_authority = @import("segment_range_authority_v2.zig");
        const boundary_authority = @import("segment_leaf_outer_authority_v2.zig");
        const input_provider_authority = @import("segment_publication_input_provider_authority_v2.zig");
        const input_provider_component = @import("air/segment_publication_input_provider_component_v2.zig");
        ...
        const noncore_audits = @import("segment_outer_noncore_audits_v2.zig");
        ...
        pub fn initOwner(
            ...
            manifest_mod.validateAgainstSources(
                manifest,
        ...
        pub fn prepareInteractions(
            ...
            return prepareInteractionsWithGenerator(self, audit_allocator, relations, provider_relations, &generator);
        }
        ...
        pub fn prepareInteractionsWithGenerator(
            ...
            const transcript_claims = try transcript_components.fillInteractionIntoWithGenerator(&self.transcript_owner, &self.transcript_workspace, &self.transcript_prepared, &self.manifest, relations, self.tree2.columns, generator);
            const statement_claims = try statement_components.fillInteractionIntoWithGenerator(&self.statement_owner, &self.statement_workspace, &self.statement_prepared, self.statement_destinations.logical_rows, &self.manifest, relations, self.tree2.columns, generator);
            const public_claims = try public_components.fillInteractionIntoWithGenerator(&self.public_owner, &self.public_workspace, &self.public_prepared, &self.manifest, relations, self.tree2.columns, generator);
            ...
            var rebuilt_boundary: boundary_authority.PreparedNativeVerifierOuterAuthorityV2 =
                undefined;
            try boundary_authority.prepareNativeVerifierIntoWithGenerator(&rebuilt_boundary, &self.boundary_workspace, &self.boundary_owner, self.boundary_staging_traces.view(), &self.prepared_leaf.capture.public_data.data, &self.prepared_leaf.verifier_keys, &self.native_relations, &self.prepared_leaf.capture.native_public_sums, &self.prepared_leaf.capture.receipt, self.prepared_leaf.capture.vm_air.component_descs, self.prepared_leaf.capture.vm_air.infra_descs, relations, generator);
            ...
            try input_provider_authority.prepareIntoWithGenerator(&rebuilt_input_provider, &self.input_provider_workspace, &self.input_provider_owner, self.input_provider_staging_traces.view(), .{
