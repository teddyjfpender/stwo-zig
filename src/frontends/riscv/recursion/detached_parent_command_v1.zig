...
const verifier = @import("detached_parent_verifier_v1.zig");
...
pub fn deriveExpected(allocator: std.mem.Allocator, paths: [2][]const u8, mode: protocol.PublicationMode) !verifier.ExpectedV1 {
    const child = @import("detached_segment_expected_v1.zig");
    ...
    _ = try @import("../air/public_data_v2.zig").PublicDataV2.authenticateAdjacent(&inputs[0].data, &inputs[1].data);
    const left = try inputs[0].data.authenticatedView();
    const right = try inputs[1].data.authenticatedView();
    const continuation = @import("span_continuation_v1.zig");
...
pub fn foldExpected(allocator: std.mem.Allocator, paths: [2][]const u8, mode: protocol.PublicationMode) !verifier.ExpectedV1 {
    ...
    return @import("span_continuation_v1.zig").fold(&inputs[0], &inputs[1], mode);
}
