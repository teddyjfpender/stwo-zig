//! Shared concrete leaf cohort ownership.
pub fn For(comptime leaf_outer: type, comptime core_mod: type) type {
    return struct {
        ...
        const noncore_mod = @import("detached_leaf_noncore_owner_v2.zig").For(leaf_outer);
        const diagnostics = @import("detached_leaf_cohort_diagnostics_v2.zig");
        const tuple_diagnostic = @import("detached_leaf_tuple_diagnostic_v2.zig");
        const contract = @import("detached_leaf_cohort_contract_v2.zig").For(leaf_outer, core_mod);
        const support = @import("detached_leaf_cohort_support_v2.zig").For(leaf_outer, core_mod);
        const verifier_components_mod = @import("detached_segment_recording_components_v1.zig");

        const M31 = stwo_core.fields.m31.M31;
        const QM31 = stwo_core.fields.qm31.QM31;
        const m31 = stwo_core.fields.m31;

        const composition_v3 = @import("recursion_air_composition_circuit_v3.zig");
        ...
        const cohort_protocol = @import("segment_outer_cohort_v2.zig");
        const public_native_sum = @import("segment_public_native_sum_authority_v2.zig");
        const authority_boundary = @import("detached_segment_authority_boundary_v1.zig");
        ...
        pub const Cohort = struct {
            ...
            pub fn publicationAuthority(
                ...
                const view = try @import("segment_statement_v2.zig")
            ...
            pub fn fillPreprocessedInto(
                ...
                if (std.process.hasEnvVarConstant(CIRCUIT_CENSUS_ENV))
                    diagnostics.printCircuitCensus(self, destination, &circuit_census_sequence);
            }
            ...
            pub fn fillInteractionInto(
                ...
                return fillInteractionIntoWithGenerator(self, manifest_value, relations, provider_relations, destination, &generator);
            }
            ...
            pub fn fillInteractionIntoWithGenerator(
                ...
                const noncore_generated = try self.noncore.prepareInteractionsWithGenerator(self.allocator, relations, provider_relations, generator);
                const core_generated = try self.core.prepareInteractionsWithGenerator(self.allocator, relations, provider_relations, generator);
            ...
            pub fn diagnoseRedTupleClosure(
                ...
                try self.validateEnvelope();
                return diagnostics.diagnoseRedTupleClosure(self, allocator);
            }
            ...
            ) !support.CollectedClosure {
                return support.collectClosure(self, generated, public_wire_boundary);
            }
        ...
        pub fn testMovedLeafCohort0() !void {
