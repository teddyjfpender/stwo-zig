...
const interaction_workspace = dependency_0.interaction_workspace;
const domain_audit = dependency_0.domain_audit;
const component_init = dependency_0.component_init;
const interaction_fill = dependency_0.interaction_fill;
...
const logicalStorageCount = dependency_2.logicalStorageCount;
const carveRows = dependency_2.carveRows;
const validateCarvedRows = dependency_2.validateCarvedRows;
const deriveLogSizes = dependency_2.deriveLogSizes;
const sourceDigest = dependency_2.sourceDigest;
...
const sourceColumnCount = dependency_3.sourceColumnCount;
...
pub fn Source(comptime dimensions: fixed_wire.Dimensions) type {
    ...
    return struct {
        ...
        recursion_schedule_digests: [2][8]u32,
        log_sizes: LogSizes,
        ...
        const Self = @This();
        pub const Prepared = binary_authority.Prepared(dimensions);

        /// Worker-private retained storage for rows 0--9 interaction
        /// generation. One tightly packed M31 slab owns every heterogeneous
        /// logical row; each authenticated framework runtime retains its own
        /// inversion workspace. Construction is cold, while repeated fills
        /// perform no heap operations.
        pub const InteractionWorkspace = interaction_workspace.Type(
            Self,
            Prepared,
            LogSizes,
            logicalStorageCount,
        ...
        pub fn init(
            ...
            try prepared.validateTranscriptSnapshots(
                vm_plan,
                recursion_plans,
                preprocessing,
            );
            ...
            var result = Self{
                ...
                .recursion_schedule_digests = .{
                    recursion_plans[0].authority_digest,
                    recursion_plans[1].authority_digest,
                },
            ...
            result.authority_seal = sourceDigest(
                &result,
                preprocessing,
                prepared,
            );
        ...
        pub fn validateAgainst(
            ...
            if (!std.meta.eql(self.vm_schedule_digest, vm_plan.authority_digest) or
                !std.meta.eql(self.recursion_schedule_digests[0], recursion_plans[0].authority_digest) or
                !std.meta.eql(self.recursion_schedule_digests[1], recursion_plans[1].authority_digest))
            {
                return error.ScheduleAuthorityMismatch;
            }
            try self.control_preprocessing.validateAgainst(vm_plan, recursion_plans[0]);
            try prepared.validateTranscriptSnapshots(
                vm_plan,
                recursion_plans,
                preprocessing,
            ...
            if (!std.meta.eql(self.parameters, Parameters.binaryNode()))
            ...
            if (!std.mem.eql(
                u8,
                &self.authority_seal,
                &sourceDigest(self, preprocessing, prepared),
            )) return error.PreparedAuthorityMismatch;
        }
        ...
        pub fn fillInteractionInto(
            ...
            var workspace = try InteractionWorkspace.init(
                self.allocator,
                self,
                preprocessing,
                prepared,
            ...
            return self.fillInteractionIntoWithWorkspace(
                &workspace,
                vm_plan,
                recursion_plans,
                preprocessing,
        ...
        /// Allocation-free interaction fill for a retained worker workspace.
        /// The same workspace may be reused across proofs of the exact sealed
        /// source geometry; all proof-dependent logical rows are overwritten
        /// before relation evaluation.
        pub fn fillInteractionIntoWithWorkspace(
            self: *const Self,
            ...
            return interaction_fill.fill(
                ...
                generateControlInteraction,
                generateTranscriptAirInteraction,
                generateTranscriptBindingInteraction,
                generateTranscriptStateInteraction,
                generateTranscriptWordInteraction,
                generateTranscriptPayloadInteraction,
                generatePowCheckInteraction,
                generatePowFrameInteraction,
                generateRelationChallengeInteraction,
                generateVerifierRandomnessInteraction,
            );
        }
        ...
        fn generateControlInteraction(
            ...
            const rows = workspace.control_rows;
            for (rows, self.control_preprocessing.rows) |*target, source|
                target.* = control_witness.logicalRow(source, .binary_node);
            return generateIntoStage(
                ControlFramework,
                &workspace.control_framework,
        ...
        fn generateTranscriptAirInteraction(
            ...
            const rows = workspace.transcript_air_rows;
            ...
            return generateIntoStage(
                ...
                &workspace.transcript_air_framework,
        ...
        fn generateTranscriptBindingInteraction(
            ...
            const rows = workspace.transcript_binding_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.transcript_binding_framework,
        ...
        fn generateTranscriptStateInteraction(
            ...
            const rows = workspace.transcript_state_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.transcript_state_framework,
        ...
        fn generateTranscriptWordInteraction(
            ...
            const rows = workspace.transcript_word_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.transcript_word_framework,
        ...
        fn generateTranscriptPayloadInteraction(
            ...
            const rows = workspace.transcript_payload_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.transcript_payload_framework,
        ...
        fn generatePowFrameInteraction(
            ...
            const rows = workspace.pow_frame_rows;
            for (rows, 0..) |*target, index|
                target.* = prepared.pow_frame.preparedRow(index);
            return generateIntoStage(
                PowFrameFramework,
                &workspace.pow_frame_framework,
        ...
        fn generateRelationChallengeInteraction(
            ...
            const rows = workspace.relation_challenge_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.relation_challenge_framework,
        ...
        fn generateVerifierRandomnessInteraction(
            ...
            const rows = workspace.verifier_randomness_rows;
            for (
                rows,
            ...
            return generateIntoStage(
                ...
                &workspace.verifier_randomness_framework,
