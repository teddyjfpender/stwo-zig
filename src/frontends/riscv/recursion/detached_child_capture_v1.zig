...
const recursion = struct {
    ...
    const detached_child_views_v1 = @import("detached_child_views_v1.zig");
...
const components = @import("detached_segment_recording_components_v1.zig");
...
const views = recursion.detached_child_views_v1;
pub const CaptureViewV1 = views.CaptureViewV1;
pub const FriLayerViewV1 = views.FriLayerViewV1;
pub const TracePathViewV1 = views.TracePathViewV1;
pub const RecordingViewV1 = views.RecordingViewV1;
pub const Family = views.Family;
...
pub fn OwnedFor(comptime family: Family) type {
    const Verifier = if (family == .segment) verifier else @import("detached_parent_verifier_v1.zig");
    const Command = if (family == .segment) command else @import("detached_parent_command_v1.zig");
    ...
    return opaque {
        ...
        pub fn init(allocator: std.mem.Allocator, key_json: []const u8, independent_key_sha256: [32]u8, expected_input: *const Expected, input_claims: Verifier.ClaimsV1, proof_bytes: []const u8) !*Self {
            ...
            const result = try @import("detached_recording_verifier_v1.zig").verify(Verifier, allocator, owned_key.key(), expectedData(&owned_expected), input_claims, proof_bytes, &channel, &capture);
            ...
            try @import("detached_recording_draws_v1.zig").validate(&execution, &capture, &result.relations, owned_key.key().pcs_config.fri_config.n_queries);
        ...
        pub fn writeCompositionInputs(self: *const Self, profile: recursion.recursion_air_composition_circuit_v3.InputProfileV3, destination: []QM31) !void {
            ...
            const kind: air.composition_circuit.ProofKind = if (family == .segment) .segment_leaf else .binary_node;
            ...
            const boundary = if (family == .segment) blk: {
                const hash_boundary = try @import("detached_segment_authority_boundary_v1.zig").derive(self.expected(), key_value.native_descriptors, self.relations());
                break :blk (try key_value.wireClaim(self.relations())).add(hash_boundary.claimed_sum);
            } else try recursion.detached_parent_protocol_v1.publicBoundary(self.expected(), self.relations());
...
pub fn testOwnedCapture() !void {
    ...
    try @import("detached_composition_preparation_v1.zig").testFromVerifiedChild(allocator, owner);
    try @import("detached_prefix_preparation_v1.zig").testFromVerifiedChild(allocator, owner);
    try @import("detached_pcs_rows_v1.zig").testFromVerifiedChild(allocator, owner);
    try @import("detached_pcs_preparation_v1.zig").testFromVerifiedChild(allocator, owner);
    ...
    try @import("detached_boundary_preparation_v1.zig").testFromVerifiedChild(allocator, owner, .{ .counts = .{ 1, 1, 0, 1 } }, .{ .entry_addresses = &.{1048832}, .exit_addresses = &.{1048832} });
    {
