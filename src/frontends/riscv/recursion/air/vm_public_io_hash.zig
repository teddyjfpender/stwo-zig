//! Exact typed logical AIR for Stark-V universal VM public-I/O hash row 14.
//!
//! Verifier-owned preprocessing concatenates the public-input and public-output
//! sponge schedules derived from row 12's sealed shape. The committed row
//! carries only the chained state, absorbed chunk, and permutation output.
//! Permutation correctness remains exclusively owned by the shared general-mode
//! Poseidon2 provider through the standard 32-word I/O relation.
...
pub const STABLE_NAME = "recursion.vm_public_io_hash.v1";
...
pub const PREPROCESSED_COLUMN_COUNT: usize = 6 + RATE * 3;
...
pub const PUBLIC_INPUT_TAG: u32 = 20;
pub const PUBLIC_OUTPUT_TAG: u32 = 21;

pub const STARK_V_REVISION: [40]u8 =
    "59172a201bd01f2f4b699bc2f7d4442d8ee81597".*;
pub const STARK_V_HASH_AIR_PATH =
    "crates/recursion/src/vm_public_io_hash_air.rs";
pub const STARK_V_HASH_AIR_SHA256 = hexDigest(
    "daede2b0d277d48ac45db5039c51c72613c5c61ddafbdb8c667bcdf6f7ceeff8",
    "invalid pinned Stark-V vm_public_io_hash_air.rs digest",
);
...
pub const STARK_V_CLAIM_SEMANTICS_INPUT_PATH =
    "crates/recursion/src/vm_public_claim_semantics_input_air.rs";
pub const STARK_V_CLAIM_SEMANTICS_INPUT_SHA256 = hexDigest(
    "93d426c207e2081adb85f724723082426a3b15293e58b99368c27260c27629e8",
    "invalid pinned Stark-V vm_public_claim_semantics_input_air.rs digest",
);
pub const STARK_V_WIRE_PATH = "crates/recursion/src/wire.rs";
pub const STARK_V_WIRE_SHA256 = hexDigest(
    "df0dae99afc7c54975c48133bc22ad66634ecd792770804fc9418a3c26873564",
    "invalid pinned Stark-V wire.rs digest",
);
...
pub const SOURCE_AUTHORITY_DOMAIN =
    "stwo-zig/typed-air/recursion-vm-public-io-hash-source/v1\x00";
pub const SOURCE_AUTHORITY_DIGEST_HEX =
    "61bc05b0ebf1c3d7761ec16d8a182105e5d5a414edc181508ca04733cc37355d";
pub const SOURCE_AUTHORITY_DIGEST = hexDigest(
    SOURCE_AUTHORITY_DIGEST_HEX,
    "invalid recursion VM public-I/O hash source-authority digest",
);
...
pub const SourceAuthority = struct {
    ...
    claim_semantics_input_sha256: digest.Digest,
    wire_sha256: digest.Digest,
    ...
    pub fn pinned() SourceAuthority {
        return .{
            ...
            .claim_semantics_input_sha256 = STARK_V_CLAIM_SEMANTICS_INPUT_SHA256,
            .wire_sha256 = STARK_V_WIRE_SHA256,
    ...
    pub fn validate(self: SourceAuthority) error{AuthorityMismatch}!void {
        ...
        const arities = [_]usize{ 32, 3, 18, 3 };
    ...
    pub fn identityDigest(self: SourceAuthority) digest.Digest {
        ...
        hashSource(
            &hash,
            STARK_V_CLAIM_SEMANTICS_INPUT_PATH,
            self.claim_semantics_input_sha256,
        );
        hashSource(&hash, STARK_V_WIRE_PATH, self.wire_sha256);
...
pub const PREPROCESSED_COLUMN_NAMES = blk: {
    ...
    names[0] = "recursion_vm_io_hash_row_mask";
    names[1] = "recursion_vm_io_hash_io_kind";
    names[2] = "recursion_vm_io_hash_step";
    names[3] = "recursion_vm_io_hash_first_mask";
    names[4] = "recursion_vm_io_hash_last_mask";
    names[5] = "recursion_vm_io_hash_domain";
    for (0..RATE) |index| {
        names[6 + 3 * index] = std.fmt.comptimePrint(
            "recursion_vm_io_hash_chunk_{d}_source_mask",
            .{index},
        );
        names[7 + 3 * index] = std.fmt.comptimePrint(
            "recursion_vm_io_hash_chunk_{d}_word_index",
            .{index},
        );
        names[8 + 3 * index] = std.fmt.comptimePrint(
            "recursion_vm_io_hash_chunk_{d}_constant",
            .{index},
        );
    }
    break :blk names;
...
pub const PARAMETER_NAMES = [PARAMETER_COUNT][]const u8{
    "recursion.vm_public_io_hash.param.segment_active",
};
...
pub const ValidationError = validate_mod.Error || relation_effect.Error || error{
    AuthorityMismatch,
    InvalidVmPublicIoHashDefinition,
};
...
pub const Definition = struct {
    ...
    pub fn validate(self: *const Definition) ValidationError!void {
        ...
        {
            return error.InvalidVmPublicIoHashDefinition;
        }
        try validateInputs(
            &self.arena,
            &self.main.physical(),
        ...
        pp_selectors[0..3].* = .{ 0, 3, 4 };
        for (0..RATE) |index| pp_selectors[3 + index] = 6 + 3 * index;
        try validateInputs(
            &self.arena,
...
fn buildDefinition(allocator: std.mem.Allocator) !Definition {
    ...
    for (&pp_values, PREPROCESSED_COLUMN_NAMES, 0..) |*value, name, index| {
        const selector = index == 0 or index == 3 or index == 4 or
            (index >= 6 and (index - 6) % 3 == 0);
    ...
    for (&chunks, 0..) |*chunk, index| chunk.* = .{
        .source_mask = pp_values[6 + 3 * index],
        .word_index = pp_values[7 + 3 * index],
        .constant = pp_values[8 + 3 * index],
    };
    ...
    for (main.previous, 0..) |value, index| {
        const expected = if (index == STATE_WIDTH - 1)
            try arena.sub(value, preprocessed.hash_domain, span)
        else
            value;
    ...
    for (&claim_tuples, preprocessed.chunks, main.chunks) |*tuple, metadata, value|
        tuple.* = .{ preprocessed.io_kind, metadata.word_index, value };
    const current_state = .{ preprocessed.io_kind, preprocessed.step } ++ main.previous;
    const next_state = .{
        preprocessed.io_kind,
        try arena.add(preprocessed.step, one, span),
    } ++ main.output;
    const state_step_next = next_state[1];
    var digest_limb_indices: [DIGEST_WORD_COUNT]types.ValueId = undefined;
    var digest_tuples: [DIGEST_WORD_COUNT][3]types.ValueId = undefined;
    ...
    | {
        ...
        tuple.* = .{ preprocessed.io_kind, limb_value.*, output };
    }
    ...
    specs[9] = .{
        .domain = .recursion_vm_public_io_hash_state,
        .role = .consume,
...
fn validateInputs(
...
) error{InvalidVmPublicIoHashDefinition}!void {
    if (values.len != names.len) return error.InvalidVmPublicIoHashDefinition;
    for (values, names, 0..) |value, expected_name, local_index| {
        ...
        const name_id = switch (node.key.op) {
            ...
            else => return error.InvalidVmPublicIoHashDefinition,
        };
...
fn validateEvents(self: *const Definition) error{InvalidVmPublicIoHashDefinition}!void {
    ...
    | {
        const values = self.arena.effectValues(self.events[11 + index]) orelse
            return error.InvalidVmPublicIoHashDefinition;
        if (values.len != 3 or values[0] != self.preprocessed.io_kind or
            values[1] != limb or values[2] != output)
        {
            return error.InvalidVmPublicIoHashDefinition;
        }
...
fn constraintName(index: usize, buffer: *[112]u8) ![]const u8 {
    if (index == 0)
        return "recursion.vm_public_io_hash.enabler_matches_active";
    if (index <= STATE_WIDTH) return std.fmt.bufPrint(
        buffer,
        "recursion.vm_public_io_hash.inactive_previous_{d}_zero",
        .{index - 1},
    );
    if (index <= STATE_WIDTH + RATE) return std.fmt.bufPrint(
        buffer,
        "recursion.vm_public_io_hash.inactive_chunk_{d}_zero",
        .{index - 1 - STATE_WIDTH},
    );
    if (index <= 2 * STATE_WIDTH + RATE) return std.fmt.bufPrint(
        buffer,
        "recursion.vm_public_io_hash.inactive_output_{d}_zero",
        .{index - 1 - STATE_WIDTH - RATE},
    );
    if (index <= 3 * STATE_WIDTH + RATE) return std.fmt.bufPrint(
        buffer,
        "recursion.vm_public_io_hash.initial_previous_{d}",
        .{index - 1 - 2 * STATE_WIDTH - RATE},
    );
    if (index < DIRECT_CONSTRAINT_COUNT) return std.fmt.bufPrint(
        buffer,
        "recursion.vm_public_io_hash.fixed_chunk_{d}",
...
comptime {
    if (STATE_WIDTH != 16 or RATE != 8 or
        PHYSICAL_MAIN_COLUMN_COUNT != 41 or PREPROCESSED_COLUMN_COUNT != 30 or
        PARAMETER_COUNT != 1 or LOGICAL_INPUT_COUNT != 72 or
    ...
    {
        @compileError("universal VM public-I/O hash geometry drifted");
    }
}
