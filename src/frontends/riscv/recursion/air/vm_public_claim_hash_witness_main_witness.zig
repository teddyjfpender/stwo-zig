...
const dependency_0 = @import("vm_public_claim_hash_witness_contract.zig");
...
pub const MainWitness = struct {
    ...
    pub fn initRaw(
        ...
        state[STATE_WIDTH - 1] = M31.fromCanonical(component.VM_PUBLIC_CLAIM_HASH_DOMAIN);
        ...
        for (rows, preprocessing.rows) |*target, metadata| {
            ...
            const main_row = materialize(metadata, words.?, state);
            target.* = main_row;
        ...
        const output_digest: [RATE]u32 = if (proof_kind == .segment_leaf) blk: {
            var result: [RATE]u32 = undefined;
            for (&result, state[0..RATE]) |*target, word| target.* = word.toU32();
            break :blk result;
        } else .{0} ** RATE;
    ...
    pub fn validateAgainst(
        ...
        var expected_digest = [_]u32{0} ** RATE;
        if (self.proof_kind == .segment_leaf) {
            for (&expected_digest, state[0..RATE]) |*target, word|
                target.* = word.toU32();
        }
        if (!std.meta.eql(expected_digest, self.output_digest))
    ...
    pub fn validateAgainstSource(
        ...
        for (self.rows, preprocessing.rows) |row, metadata| {
            const expected = if (self.proof_kind == .segment_leaf)
                materialize(metadata, words, state)
            else
                zeroMainRow();
    ...
    pub fn validateDigest(
        ...
        try self.validateAgainst(preprocessing);
        if (!std.meta.eql(self.output_digest, expected)) return error.DigestMismatch;
    }
};
...
pub const Executor = struct {
    ...
    /// or provider witness logic is duplicated by row 13.
...
pub fn logicalInputs(
    ...
    return main.values() ++ preprocessing.values() ++ .{
        ...
        M31.fromCanonical(component.VM_PUBLIC_CLAIM_HASH_DOMAIN),
