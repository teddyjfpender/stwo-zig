//! Exact typed logical AIR for Stark-V universal query-bits row 20.
//!
...
pub const STABLE_NAME = "recursion.query_bits.v1";
...
pub const PARAMETER_COUNT: usize = 3 + M31_BIT_COUNT;
...
// Projection masks are verifier-owned adapter scalars and therefore degree
// zero in the committed trace.  The generic logical profiler conservatively
// models every input as degree one; the protocol bound below records the exact
// native lowering semantics and remains cubic without extra trace columns.
pub const REFERENCE_MAXIMUM_CONSTRAINT_DEGREE: u32 = 3;
pub const MAXIMUM_CONSTRAINT_DEGREE: u32 = 3;

pub const SEMANTIC_DIGEST_HEX =
    "568d9396cf3d0556b092ccb061097b29fd84e1d7a9c046f3a79ffa9b2f59e3fb";
...
pub const STATIC_PROFILE_DIGEST_HEX =
    "6335db2759d884f0b6b657effc5a4d1c5a6eede7d678935df91d80b3ced3bed2";
...
pub const PARAMETER_NAMES = blk: {
    var names: [PARAMETER_COUNT][]const u8 = undefined;
    names[0] = "recursion.query_bits.param.segment_active";
    names[1] = "recursion.query_bits.param.binary_active";
    names[2] = "recursion.query_bits.param.raw_query_kind";
    for (0..M31_BIT_COUNT) |bit| {
        names[3 + bit] = std.fmt.comptimePrint(
            "recursion.query_bits.param.position_bit_mask_{d}",
            .{bit},
        );
    }
    break :blk names;
...
fn buildDefinition(allocator: std.mem.Allocator) !Definition {
    ...
    const parameters = Parameters{
        ...
        .position_bit_masks = parameter_values[3..][0..M31_BIT_COUNT].*,
    };
    ...
    for (&projected_bits, main.bits, parameters.position_bit_masks) |
        *projected,
        bit,
        mask,
