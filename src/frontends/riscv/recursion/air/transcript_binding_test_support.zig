...
pub const OwnedTrace = struct {
    ...
    pub fn init(
        ...
        rows: []const witness.PreprocessedRow,
        seed: u32,
    ) !OwnedTrace {
        ...
        while (row_at < rows.len) : (frame_at += 1) {
            ...
            const stream_count = try testStreamWordCount(plan, step, first.is_draw == 1);
            const words = try allocator.alloc(M31, stream_count);
            word_storage[frame_at] = words;
            initialized_words += 1;
            fillFrameWords(words, step, first.sequence, first.is_draw == 1, seed, frame_at);
...
pub fn findPowFinal(rows: []const witness.PreprocessedRow) ?usize {
    for (rows, 0..) |row, index| if (row.pow_final_mask == 1) return index;
    return null;
}

pub fn findLast(rows: []const witness.PreprocessedRow) ?usize {
...
pub fn findCheck(
    ...
    call_id: u32,
) ?pow_check_witness.Check {
    for (trace.pow_checks) |check| if (check.call_id == call_id) return check;
    return null;
}
...
pub fn testShape() !@import("../fixed_profile.zig").ProofShapeV1 {
    ...
    return .{
        .air_program_id = channel.hashBytes("transcript-binding-air", 0x5450),
        .preprocessing_id = channel.hashBytes("transcript-binding-preprocessing", 0x5450),
        .table_layout_id = channel.hashBytes("transcript-binding-layout", 0x5450),
        .table_count = 16,
        .claimed_sum_count = 4,
...
pub fn assertPreprocessedWriter(
    ...
    try assertMalformedPreprocessedWriter(executor, preprocessing, storage, columns);
}
...
pub fn assertMalformedPreprocessedWriter(
    executor: *const witness.Executor,
    preprocessing: *const witness.Preprocessed,
    storage: []M31,
    columns: [witness.PREPROCESSED_COLUMN_COUNT][]M31,
) !void {
    const sentinel = M31.fromCanonical(12345);
    @memset(storage, sentinel);
    var malformed = columns;
    malformed[0] = malformed[0][0 .. malformed[0].len - 1];
    try std.testing.expectError(
        error.InvalidTraceShape,
