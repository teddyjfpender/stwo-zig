...
pub fn completeRelationSum(
    ...
    for (fixture.preprocessing.rows) |metadata| for (metadata.chunks) |chunk| {
        if (chunk.source_mask == 0) continue;
        const claim_index = witness.sourceClaimIndex(
            fixture.preprocessing.shape,
            metadata.io_kind,
            chunk.word_index,
        ).?;
        sum = sum.add(try sourceTerm(
            ...
            &.{
                M31.fromCanonical(metadata.io_kind),
                M31.fromCanonical(chunk.word_index),
                fixture.words[claim_index],
            },
        ));
    };
    for (main.rows) |row| {
    ...
    for (main.output_digests, 0..) |output_digest, io_kind| {
        for (output_digest, 0..) |word, limb| sum = sum.sub(try sourceTerm(
            relations,
            .recursion_vm_public_io_digest,
            &.{
                M31.fromCanonical(@intCast(if (swap_digest_kinds) 1 - io_kind else io_kind)),
