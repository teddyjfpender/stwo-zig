...
test "R-012 FRI Merkle anchor writers are allocation-free padded and atomic" {
    ...
    try executor.generateMainInto(&preprocessing, fixtures.reference, &fixtures.vm_plan, &fixtures.recursion_plan, &expected_columns, opening);
    for ([_]u32{ 0, 1, 2 }) |lane| {
        try executor.generateMainForLaneInto(&preprocessing, fixtures.reference, &fixtures.vm_plan, &fixtures.recursion_plan, &columns, opening, lane);
    ...
    try std.testing.expectError(error.InvalidWitness, executor.generateMainForLaneInto(&preprocessing, fixtures.reference, &fixtures.vm_plan, &fixtures.recursion_plan, &columns, opening, 3));
