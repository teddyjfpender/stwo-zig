//! Direct witness writers for the complete recursion QM31 multiplication AIR.
//!
//! Main and preprocessing writers share the failure-atomic direct executor:
//! exact geometry, alias exclusion, source validation, deterministic zero
//! padding, and no allocation after cold authentication.
...
pub const BINDING_DOMAIN =
    "stwo-zig/typed-air/recursion-qm31-mul-full-witness/v1\x00";
pub const BINDING_DIGEST_HEX =
    "eef81f53c2de7e5992e9867fa874472243837871e3416fd061f581ee744eae36";
pub const BINDING_DIGEST: digest.Digest = hexDigest(
    BINDING_DIGEST_HEX,
    "invalid full recursion QM31 witness-binding digest",
);
...
pub const ScheduleRow = CircuitMetadata;

/// One row across all three fixed proof-kind schedules. Each mode is an
/// independent preprocessing table, so more than one optional may be present.
...
pub const ConstructionError = full.ValidationError || error{
...
pub fn mainRow(invocation: Invocation) [MAIN_COLUMN_COUNT]M31 {
...
pub inline fn writePreprocessedRow(
    ...
    inline for (PREPROCESSED_RECIPE, 0..) |source_value, column| {
        ...
        columns[column][row_index] = if (metadata) |present| switch (source_value) {
            ...
            .segment_rhs_id, .binary_rhs_id, .empty_rhs_id => present.rhs_id,
