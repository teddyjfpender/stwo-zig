//! Backend-generic proof harness for the typed Keccak-f profile.
//!
...
pub const Timings = struct {
    ...
    pub fn proveProductionNs(self: Timings) u64 {
        return self.witness_ns + self.preprocessed_commit_ns + self.main_commit_ns +
            self.interaction_generation_ns + self.interaction_commit_ns + self.prove_ns;
    }

    pub fn totalNs(self: Timings) u64 {
        return self.proveProductionNs() + self.verify_ns;
    }
};

pub fn run(comptime Engine: type, allocator: std.mem.Allocator) !Timings {
    const VerifierScheme = stwo_core.pcs.verifier.CommitmentSchemeVerifier(
        Engine.Hasher,
        Engine.MerkleChannel,
    );
    const Tree = OwnedTree(Engine);
    ...
    var pp_tree = try Tree.init(allocator, &pp_logs);
    ...
    var main_tree = try Tree.init(allocator, &main_logs);
    ...
    var interaction_tree = try Tree.init(allocator, &interaction_logs);
    ...
    return .{
        ...
        .preprocessed_commit_ns = pp_commit_ns,
        .main_commit_ns = main_commit_ns,
        .interaction_generation_ns = interaction_gen_ns,
