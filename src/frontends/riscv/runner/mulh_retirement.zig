//! Failure-atomic retirement for the typed RV32 `MULH` authority.
//!
//! The fixed authority is the sole signedness-aware high-product and x0 authority. This
...
const typed = @import("../air/lang/typed_mulh.zig");
const typed_authority = @import("../air/lang/typed_mulh_authority.zig");
...
/// Pointer-free projection of the only three accesses `MULH` can emit. Raw
...
pub inline fn compileCompact(
    ...
    const retirement = authority.retire(
        ...
        error.WrongMulhOpcode => error.WrongMulhOpcode,
        error.InvalidImmediate => error.InvalidMulImmediate,
    };
...
test "MULH closed-form clock gaps equal state-chain geometry" {
...
comptime {
    if (@sizeOf(CompactTransaction) != MAX_TRANSACTION_BYTES)
        @compileError("typed MULH compact transaction footprint drifted");
    if (@sizeOf(Plan) != MAX_PLAN_BYTES)
        @compileError("typed MULH retirement plan footprint drifted");
    if (@sizeOf(Prepared) != MAX_PREPARED_BYTES)
        @compileError("typed MULH prepared token footprint drifted");
}
