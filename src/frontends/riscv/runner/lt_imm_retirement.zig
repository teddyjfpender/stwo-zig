//! Failure-atomic retirement for typed RV32 SLTI/SLTIU.
//!
//! The fixed typed authority owns signed/unsigned comparison, x0 result
//! visibility, witness projection, roots, and ordered relations. This
//! family-local boundary owns exact I-type word admission, ordered rs1/rd
//! predecessor clocks, capacity preflight, and atomic CPU/trace/state-chain
//! publication. Staging and the warm fused path allocate nothing.

const std = @import("std");
const builtin = @import("builtin");
const access_clock = @import("../access_clock.zig");
const typed = @import("../air/lang/typed_lt_imm.zig");
const typed_authority = @import("../air/lang/typed_lt_imm_authority.zig");
...
pub inline fn compileCompact(
    ...
    const retirement = authority.retire(
        ...
        error.WrongLtImmOpcode => error.WrongLtImmOpcode,
        error.InvalidImmediate => error.InvalidLtImmImmediate,
    };
...
/// Admit all generic decoder fields. For I-type instructions `rs2` is an
/// encoding diagnostic containing bits 24:20 of the sign-extended immediate.
...
test "LT_IMM closed-form clock gaps equal state-chain geometry" {
...
comptime {
    if (@sizeOf(CompactTransaction) > MAX_TRANSACTION_BYTES)
        @compileError("typed LT_IMM compact transaction exceeded its stack budget");
    if (@sizeOf(Plan) > MAX_PLAN_BYTES)
        @compileError("typed LT_IMM retirement plan exceeded its stack budget");
    if (@sizeOf(Prepared) > MAX_PREPARED_BYTES)
        @compileError("typed LT_IMM prepared token exceeded its stack budget");
}
