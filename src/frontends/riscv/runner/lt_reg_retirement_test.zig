...
const subject = @import("lt_reg_retirement.zig");
const Cpu = @import("cpu.zig").Cpu;
const StateChainTracker = @import("state_chain.zig").StateChainTracker;
const Trace = @import("trace.zig").Trace;

const operations = [_]decode.Opcode{ .SLT, .SLTU };

test "LT_REG retirement is exact across opcodes aliases x0 and extrema" {
...
test "LT_REG compact compiler exactly equals generic three-register transactions" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
...
test "LT_REG accepts exactly every architectural R-type family encoding" {
...
test "LT_REG forged stale and reused plans reject before mutation" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
...
test "LT_REG cold allocation failure is atomic and warm path allocates nothing" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    {
    ...
    for (0..iterations) |index| {
        ...
        const opcode = operations[index & 1];
...
test "LT_REG retirement retains at least 0.97x legacy throughput" {
    ...
    std.debug.print(
        "\n  LT_REG retirement={d} ns legacy={d} ns speed={d:.4}x " ++
...
fn encode(opcode: decode.Opcode, rd: u5, rs1: u5, rs2: u5) u32 {
    ...
    return (@as(u32, rs2) << 20) | (@as(u32, rs1) << 15) |
