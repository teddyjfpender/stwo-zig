...
const support = @import("../air/lang/typed_mulh_witness_test_support.zig");
const access_witness = @import("access_witness.zig");
const decode = @import("decode.zig");
const subject = @import("mulh_retirement.zig");
const Cpu = @import("cpu.zig").Cpu;
const StateChainTracker = @import("state_chain.zig").StateChainTracker;
const Trace = @import("trace.zig").Trace;

test "MULH retirement is exact across boundary corpus aliases x0 and high words" {
    ...
    var index: usize = 0;
    for (support.operations) |opcode| for (support.boundary_operands) |lhs_input| for (support.boundary_operands) |rhs_input| {
        ...
        const expected = support.result(opcode, lhs, rhs);
...
test "MULH compact compiler exactly matches generic three-register transactions" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
...
test "MULH admission exhausts legal registers and every opcode selector" {
    var legal_count: usize = 0;
    for (support.operations) |opcode| for (0..32) |rd_raw| for (0..32) |rs1_raw| for (0..32) |rs2_raw| {
    ...
    for (0..128) |opcode_field| for (0..128) |funct7| for (0..8) |funct3| {
        ...
        const expected_mulh = opcode_field == 0x33 and funct7 == 1 and
            funct3 >= 1 and funct3 <= 3;
        if (maybe_instruction) |instruction| {
            ...
            try std.testing.expect(!expected_mulh);
        }
        selector_count += 1;
    };
    try std.testing.expectEqual(@as(usize, 131_072), selector_count);

    const word = encode(.MULH, 3, 4, 5);
...
test "MULH forged plan dimensions reject before logical mutation" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
    ...
    var tracker = StateChainTracker.init(std.testing.allocator);
    defer tracker.deinit();
    const word = encode(.MULHSU, 4, 2, 3);
    const instruction = try decode.DecodedInst.decode(word);
...
test "MULH prepared token is single-use and rejects stale state" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
...
test "MULH prepared token rejects append-log staleness" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
...
test "MULH cold allocation failure is atomic and warm path allocates nothing" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    {
    ...
    std.debug.print(
        "\n  MULH footprint transaction={d}B plan={d}B prepared={d}B\n",
        .{
...
test "MULH reserve reentrancy is detected before retirement publication" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
...
test "MULH retirement strictly preserves paired legacy throughput" {
    ...
    std.debug.print(
        "\n  MULH retirement={d} ns legacy={d} ns speed={d:.4}x\n",
...
fn measureLegacy(iterations: usize) !u64 {
    ...
    for (0..iterations) |index| {
        ...
        const next = support.result(instruction.opcode, lhs, rhs);
