...
const corpus = @import("../air/lang/typed_mul_corpus.zig");
const access_witness = @import("access_witness.zig");
const decode = @import("decode.zig");
const subject = @import("mul_retirement.zig");
const Cpu = @import("cpu.zig").Cpu;
const StateChainTracker = @import("state_chain.zig").StateChainTracker;
const Trace = @import("trace.zig").Trace;

test "MUL retirement is exact across boundary corpus aliases x0 and high words" {
    ...
    for (0..corpus.CORPUS_ROW_COUNT) |index| {
        const pair = corpus.operandPair(index);
        ...
        cpu.writeReg(rs1, pair.lhs);
        if (rs2 != rs1) cpu.writeReg(rs2, pair.rhs);
        ...
        const expected = independentLowProduct(lhs, rhs);
        const word = encode(rd, rs1, rs2);
...
test "MUL compact compiler exactly matches generic three-register transactions" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    ...
    for (cases) |case| {
        ...
        const instruction = try decode.DecodedInst.decode(
            encode(case.rd, case.rs1, case.rs2),
        );
...
test "MUL admission exhausts legal registers and every opcode selector" {
    var legal_count: usize = 0;
    for (0..32) |rd_raw| for (0..32) |rs1_raw| for (0..32) |rs2_raw| {
        const rd: u5 = @intCast(rd_raw);
        const rs1: u5 = @intCast(rs1_raw);
        const rs2: u5 = @intCast(rs2_raw);
        const word = encode(rd, rs1, rs2);
        const instruction = try decode.DecodedInst.decode(word);
        try std.testing.expectEqual(decode.Opcode.MUL, instruction.opcode);
    ...
    try std.testing.expectEqual(@as(usize, 32_768), legal_count);
    ...
    for (0..128) |opcode_field| for (0..128) |funct7| for (0..8) |funct3| {
        ...
        const expected_mul = opcode_field == 0x33 and funct7 == 1 and funct3 == 0;
    ...
    try std.testing.expect(!subject.instructionMatchesWord(instruction, word));
    instruction = try decode.DecodedInst.decode(word);
    try std.testing.expect(!subject.instructionMatchesWord(instruction, word ^ (1 << 12)));
}

test "MUL forged plan dimensions reject before logical mutation" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
    ...
    inline for (.{
        ...
        if (std.mem.eql(u8, mutation, "opcode")) forged.instruction.opcode = .MULH;
...
test "MUL prepared token is single-use and rejects stale state" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
...
test "MUL prepared token rejects append-log staleness" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
...
test "MUL cold allocation failure is atomic and warm path allocates nothing" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    {
    ...
    std.debug.print(
        "\n  MUL footprint transaction={d}B plan={d}B prepared={d}B\n",
        .{
...
test "MUL retirement strictly preserves paired legacy throughput" {
    ...
    std.debug.print(
        "\n  MUL retirement={d} ns legacy={d} ns speed={d:.4}x\n",
...
fn measureLegacy(iterations: usize) !u64 {
    ...
    for (0..iterations) |index| {
        ...
        const next = independentLowProduct(lhs, rhs);
