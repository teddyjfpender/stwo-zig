//! Failure-atomic retirement for the typed RV32 `MUL` authority.
//!
//! The fixed authority is the sole low-product and x0 authority.  This
...
const typed = @import("../air/lang/typed_mul.zig");
const typed_authority = @import("../air/lang/typed_mul_authority.zig");
...
/// Pointer-free projection of the only three accesses `MUL` can emit. Raw
...
pub inline fn compileCompact(
    ...
    const retirement = authority.retire(
        ...
        error.WrongMulOpcode => error.WrongMulOpcode,
...
test "MUL closed-form clock gaps equal state-chain geometry" {
...
comptime {
    if (@sizeOf(CompactTransaction) > MAX_TRANSACTION_BYTES)
        @compileError("typed MUL compact transaction exceeded its stack budget");
    if (@sizeOf(Plan) > MAX_PLAN_BYTES)
        @compileError("typed MUL retirement plan exceeded its stack budget");
    if (@sizeOf(Prepared) > MAX_PREPARED_BYTES)
        @compileError("typed MUL prepared token exceeded its stack budget");
}
