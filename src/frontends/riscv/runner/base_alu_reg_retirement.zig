//! Failure-atomic retirement for typed RV32 ADD/SUB/XOR/OR/AND.
//!
//! The fixed authority is the sole architectural result/x0 authority. This
...
//! fallible destination before publication; the warm fused path allocates
//! nothing and cannot expose a partially retired instruction.

const std = @import("std");
const builtin = @import("builtin");
const access_clock = @import("../access_clock.zig");
const typed = @import("../air/lang/typed_base_alu_reg.zig");
...
pub const CompileError = error{
    ...
    ClockOutOfRange,
    InvalidBaseAluRegImmediate,
    NonIncreasingClock,
    WrongBaseAluRegOpcode,
    ZeroRegisterValue,
};
...
pub const MAX_TRANSACTION_BYTES: usize = 112;
...
/// Fixed projection of the only three accesses this family can produce.
/// Raw clocks authenticate the staged tracker snapshot; effective clocks are
/// the predecessor values committed to the AIR row after any synthetic gaps.
...
/// Compile all semantic, alias, x0, and clock facts without allocation or
/// mutation. Aliases always chain to the latest earlier physical phase.
pub inline fn compileCompact(
    ...
    const retirement = authority.retire(
        ...
        error.WrongBaseAluRegOpcode => error.WrongBaseAluRegOpcode,
        error.InvalidImmediate => error.InvalidBaseAluRegImmediate,
    };
...
pub const Plan = struct {
    ...
    fn reserveAndRevalidate(
        ...
        // Allocator calls are the only operations that may re-enter external
        // code. The warm path proceeds directly from this authenticated plan.
...
inline fn instructionMatchesWord(instruction: DecodedInst, inst_word: u32) bool {
    ...
    const funct7: u7 = if (instruction.opcode == .SUB) 0b0100000 else 0;
    const reconstructed = (@as(u32, funct7) << 25) |
...
comptime {
    if (@sizeOf(CompactTransaction) > MAX_TRANSACTION_BYTES)
        @compileError("typed BASE_ALU_REG compact transaction exceeded its stack budget");
    if (@sizeOf(Plan) > MAX_PLAN_BYTES)
        @compileError("typed BASE_ALU_REG retirement plan exceeded its stack budget");
    if (@sizeOf(Prepared) > MAX_PREPARED_BYTES)
        @compileError("typed BASE_ALU_REG prepared token exceeded its stack budget");
}
