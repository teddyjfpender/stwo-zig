...
const subject = @import("base_alu_reg_retirement.zig");
const Cpu = @import("cpu.zig").Cpu;
const StateChainTracker = @import("state_chain.zig").StateChainTracker;
const Trace = @import("trace.zig").Trace;

const operations = [_]decode.Opcode{ .ADD, .SUB, .XOR, .OR, .AND };

test "E-020 BASE_ALU_REG retirement is exact across opcodes all aliases x0 and boundaries" {
    ...
    const cases = [_]Case{
        ...
        .{ .rd = 7, .rs1 = 7, .rs2 = 9, .lhs = 0x7fff_ffff, .rhs = 1, .previous = 0 },
        .{ .rd = 9, .rs1 = 7, .rs2 = 9, .lhs = 0x8000_0000, .rhs = 0xffff_ffff, .previous = 0 },
        .{ .rd = 31, .rs1 = 3, .rs2 = 3, .lhs = 0x0102_0304, .rhs = 0, .previous = 0x1020_3040 },
        .{ .rd = 1, .rs1 = 2, .rs2 = 3, .lhs = 0xffff_00ff, .rhs = 0x00ff_ffff, .previous = 0x55aa_1234 },
    };
    ...
    for (operations) |opcode| for (cases) |case| {
        ...
        const rhs = if (case.rs2 == 0)
            0
        else if (case.rs2 == case.rs1)
            lhs
        else
            case.rhs;
        ...
        try std.testing.expectEqual(
            ...
            tracker.accesses.items[1].clk,
        );
        try std.testing.expectEqual(
            access_clock.encode(instruction_clock, .third),
            tracker.accesses.items[2].clk,
        );
...
test "E-020 compact BASE_ALU_REG compiler exactly equals generic three-register transactions" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    const Case = struct { rd: u5, rs1: u5, rs2: u5 };
    const cases = [_]Case{
        .{ .rd = 0, .rs1 = 0, .rs2 = 0 },
        .{ .rd = 3, .rs1 = 3, .rs2 = 3 },
        .{ .rd = 3, .rs1 = 3, .rs2 = 4 },
    ...
    for (operations) |opcode| for (cases) |case| {
        ...
        const rhs: u32 = if (case.rs2 == 0)
            0
        else if (case.rs2 == case.rs1)
            lhs
        else
            0x1234_5678;
        const previous: u32 = if (case.rd == 0)
            0
        else if (case.rd == case.rs2)
            rhs
        else if (case.rd == case.rs1)
            lhs
        else
        ...
        const raw_clocks = [_]u32{
            compact.source_1_raw_previous_clock,
            compact.source_2_raw_previous_clock,
            compact.destination_raw_previous_clock,
        };
        const previous_clocks = [_]u32{
            compact.source_1_previous_clock,
            compact.source_2_previous_clock,
            compact.destination_previous_clock,
        };
        for (generic.events[0..3], raw_clocks, previous_clocks) |event, raw, effective| {
            try std.testing.expectEqual(raw, event.raw_previous_clock);
            try std.testing.expectEqual(effective, event.previous_clock);
        }
        try std.testing.expectEqual(
...
test "E-020 BASE_ALU_REG forged and stale plans reject before logical mutation" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    var cpu = Cpu.init(0x1000, 0);
...
test "E-020 BASE_ALU_REG cold allocation failure is atomic and warm path allocates nothing" {
    const authority = try subject.authenticateCanonical(std.testing.allocator);
    {
    ...
    try std.testing.expect(
        @sizeOf(subject.CompactTransaction) <= subject.MAX_TRANSACTION_BYTES,
    );
}

test "E-020 BASE_ALU_REG retirement retains at least 0.97x legacy throughput" {
    ...
    std.debug.print(
        "\n  E-020 BASE_ALU_REG retirement={d} ns legacy={d} ns speed={d:.4}x\n",
...
fn encode(opcode: decode.Opcode, rd: u5, rs1: u5, rs2: u5) u32 {
    ...
    const funct7: u7 = if (opcode == .SUB) 0x20 else 0;
