//! Failure-atomic retirement for the typed RV32 SLL/SRL/SRA authority.
//!
//! The fixed authority is the sole shift-result and x0 authority. This
...
const typed = @import("../air/lang/typed_shifts_reg.zig");
const typed_authority = @import("../air/lang/typed_shifts_reg_authority.zig");
...
pub const CompileError = error{
    ...
    ClockOutOfRange,
    InvalidShiftsRegImmediate,
    NonIncreasingClock,
    WrongShiftsRegOpcode,
    ZeroRegisterValue,
};
...
pub inline fn compileCompact(
    ...
    const retirement = authority.retire(
        ...
        error.WrongShiftsRegOpcode => error.WrongShiftsRegOpcode,
        error.InvalidImmediate => error.InvalidShiftsRegImmediate,
    };
...
inline fn instructionMatchesWord(instruction: DecodedInst, inst_word: u32) bool {
    ...
    const funct7: u7 = switch (instruction.opcode) {
        .SLL, .SRL => 0b0000000,
        .SRA => 0b0100000,
        else => return false,
    };
...
comptime {
    if (@sizeOf(CompactTransaction) > MAX_TRANSACTION_BYTES)
        @compileError("typed SHIFTS_REG compact transaction exceeded its stack budget");
    if (@sizeOf(Plan) > MAX_PLAN_BYTES)
        @compileError("typed SHIFTS_REG retirement plan exceeded its stack budget");
    if (@sizeOf(Prepared) > MAX_PREPARED_BYTES)
        @compileError("typed SHIFTS_REG prepared token exceeded its stack budget");
}
