...
pub const Policy = struct {
    ...
    pub fn init(geometry: geometry_mod.Geometry) Policy {
        ...
        for (result.offset_points[12..16]) |*offset| {
...
test "exact OODS execution order preserves canonical source/sample pairs" {
    ...
    try std.testing.expect(!std.meta.eql(policy.offset_points[12], identity));
    try std.testing.expect(std.meta.eql(policy.offset_points[16], identity));
}
...
test "exact OODS batches reuse cumulative coefficient columns without copy" {
    ...
    const trees = [_]resident_views.TraceTree{
        testTree(.preprocessed, 0x10_0000, 4, 16),
        testTree(.main, 0x20_0000, 4, 16),
        testTree(.interaction, 0x30_0000, 8, 16),
    ...
    try std.testing.expectEqual(@as(usize, 6), batches.len);
    try std.testing.expectEqual(
        batches[3].coefficients.storage.address,
        batches[4].coefficients.storage.address,
    );
    try std.testing.expectEqual(@as(usize, 12), batches[3].first_sample);
    try std.testing.expectEqual(@as(usize, 16), batches[4].first_sample);
}
