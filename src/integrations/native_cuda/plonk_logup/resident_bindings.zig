...
pub fn bind(
    ...
    const source_columns = [relation_mod.source_pointer_count]Words{
        try coefficientColumn(preprocessed, 0, rows),
        try coefficientColumn(main, 1, rows),
        try coefficientColumn(preprocessed, 1, rows),
        try coefficientColumn(main, 2, rows),
        try coefficientColumn(preprocessed, 2, rows),
    ...
    const relation = Relation{
        .buffers = .{
            ...
            .reduction_partials = try exactWords(
                ...
                try relationScratchWords(geometry.statement.log_n_rows),
            ),
...
test "exact Plonk binds four trees and one complete relation instance" {
