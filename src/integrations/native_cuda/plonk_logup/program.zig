...
fn digestKernelPack() ir.Digest {
    ...
    return ir.identityDigest(
        "stwo-zig/native-cuda/plonk-logup/aot-pack/v1",
    );
}
...
fn emitWithBuffers(
    ...
    for (&columns) |*column| column.log_rows = geometry.statement.log_n_rows;
...
fn transcriptNode(operation: u32, fri_tree_count: u32) u32 {
    ...
    if (operation <= 9) return 3;
    const after_fri = 10 + 2 * fri_tree_count;
...
fn transcriptKind(operation: u32, fri_tree_count: u32) ir.TranscriptKind {
    if (operation <= 2 or operation == 4 or
        operation == 6 or operation == 8)
    {
        return .mix;
    }
    if (operation == 3 or operation == 5 or
        operation == 7 or operation == 9)
    {
        return .challenge;
    }
    const fri_end = 10 + 2 * fri_tree_count;
    if (operation < fri_end) {
        return if ((operation - 10) % 2 == 0) .mix else .challenge;
    }
...
fn transcriptValueCount(
    ...
    if (operation == 8) return geometry_mod.sampled_mask_points;
    if (operation == 12 + 2 * geometry.fri_tree_count)
...
test "Plonk emits exact generic Native AIR geometry and proof semantics" {
    ...
    try std.testing.expectEqual(@as(u32, 28), program.quotient.term_count);
...
test "Plonk program tree and sample counts match a decoded CPU proof" {
    ...
    var program = try emit(allocator, &materialized);
    defer program.deinit(allocator);
    var output = try cpu_plonk.prove(allocator, protocol, request);
...
test "Plonk public statement changes semantic identity" {
    const allocator = std.testing.allocator;
...
test "Plonk production program emission does not materialize a CPU trace" {
    const allocator = std.testing.allocator;
