//! Fail-closed Fiat-Shamir order for exact four-tree Plonk/LogUp.
...
pub const Schedule = struct {
    ...
    pub fn init(geometry: geometry_mod.Geometry) !Schedule {
        var seed = shared.SeedBuilder.init(0x504c_4f4e_4b4c_4f47);
...
test "exact schedule draws lookup before interaction commitment" {
    const pcs = @import("stwo_core").pcs;
    const schedule = try Schedule.init(try geometry_mod.admit(
        .{ .log_n_rows = 8 },
        pcs.PcsConfig.default(),
    ));
    try std.testing.expectEqual(@as(u32, 29), schedule.operationCount());
    try std.testing.expectEqual(
        Operation.draw_lookup_elements,
        try schedule.operation(3),
    );
    try std.testing.expectEqual(
