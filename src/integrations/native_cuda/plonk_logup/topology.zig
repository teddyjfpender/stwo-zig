//! Exact OODS and quotient topology over 24 sources and 28 mask points.
...
pub const sample_batches = [_]SampleBatch{
    ...
    .{ .role = .main, .first_column = 0, .column_count = 4, .first_execution_sample = 4 },
...
test "exact quotient reuses cumulative sources for two mask points" {
    ...
    try std.testing.expectEqual(@as(u32, 24), quotient.source_count);
    try std.testing.expectEqual(@as(usize, 28), quotient.batch_terms.len);
    try std.testing.expectEqual(
        quotient.batch_terms[12].source_index,
        quotient.batch_terms[13].source_index,
    );
    try std.testing.expectEqual(@as(usize, 4), decommit.trace_trees.len);
    try std.testing.expectEqual(@as(usize, 24), decommit.column_log_sizes.len);
    try std.testing.expectEqual(@as(usize, 6), sample_batches.len);
}
