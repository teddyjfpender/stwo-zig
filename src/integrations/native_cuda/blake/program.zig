//! Backend-neutral proof-program emission for Native Native Blake.
...
pub const kernel_pack_identity: ir.Digest = blk: {
    ...
    break :blk ir.identityDigest(
        "stwo-zig/native-cuda/blake/aot-pack/v1",
    );
};
...
test "Native Blake emits one validated backend-neutral program" {
...
fn testRequest() geometry_mod.Request {
    ...
    return .{
        .statement = .{ .log_n_rows = 14, .n_rounds = 100 },
