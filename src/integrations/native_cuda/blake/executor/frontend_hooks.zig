//! Native Blake policy hooks for the shared resident proof pipeline.
...
pub const Hooks = struct {
    ...
    pub fn ingress(
        ...
        try ingress_stage.run(transaction, prepared, pack, &views);
    }
    ...
    pub fn oods(
        ...
        const views = try bindings.bind(transaction, prepared);
        try oods_executor.run(transaction, prepared, pack, &views);
    }
    ...
    pub fn friCommit(
        ...
        const views = try bindings.bind(transaction, prepared);
        try fri_executor.run(transaction, prepared, pack, &views);
    }
...
test "Blake hooks keep every downstream stage shared" {
    inline for (&.{
        "oods",
        "quotient",
        "friCommit",
