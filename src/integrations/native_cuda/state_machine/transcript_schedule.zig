//! state-machine statement policy for the shared transcript schedule.
...
test "state-machine schedule binds the complete public statement" {
    ...
    const second = try Schedule.init(try geometry_mod.admit(
        .{
            .log_n_rows = 7,
            .initial_state = .{
                @import("stwo_core").fields.m31.M31.fromU64(10),
                @import("stwo_core").fields.m31.M31.fromU64(3),
            },
        },
        pcs.PcsConfig.default(),
    ));
    try std.testing.expectEqual(@as(u32, 26), first.operationCount());
