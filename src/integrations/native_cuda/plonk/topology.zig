//! Plonk aliases for the shared uniform proof topology.
...
test "Plonk topology opens all three trace trees and sixteen columns" {
    ...
    try std.testing.expectEqual(@as(usize, 16), quotient.prepared_terms.len);
    try std.testing.expectEqual(@as(u32, 16), quotient.source_count);
    ...
    try std.testing.expectEqual(@as(usize, 16), decommit.column_log_sizes.len);
