...
fn digestKernelPack() ir.Digest {
    ...
    return ir.identityDigest("stwo-zig/native-cuda/plonk/aot-pack/v1");
}
...
test "Plonk emits exact generic Native AIR geometry and proof semantics" {
    ...
    try std.testing.expectEqual(@as(u64, 8 * (1 << 7)), contract.ingress.element_count);
    try std.testing.expectEqual(@as(usize, 16), program.trace_columns.len);
    ...
    try std.testing.expectEqual(@as(u32, 16), program.quotient.term_count);
...
test "Plonk program tree and sample counts match a decoded CPU proof" {
    ...
    var program = try emit(allocator, &materialized);
    defer program.deinit(allocator);
    var output = try cpu_plonk.prove(allocator, protocol, statement);
...
test "Plonk production program emission does not materialize a CPU trace" {
    ...
    try std.testing.expectEqual(@as(u32, 16), emitted.quotient.term_count);
}
