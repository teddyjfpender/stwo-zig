//! Source- and protocol-bound identities for the Native Plonk adapter.
...
pub const zig_artifact = Artifact{
    ...
    .revision = "src/examples/plonk.zig+src/examples/plonk/input.zig",
    .trace_recipe = digest("stwo-zig/src/examples/plonk/input.zig:prepare:v1"),
    .air_recipe = digest("stwo-zig/src/examples/plonk.zig:PlonkComponent:v1"),
};
...
pub const rust_artifact = Artifact{
    ...
    .trace_recipe = digest(
        "stwo-rust:a8fcf4bd:examples-plonk:gen-trace:v1",
    ),
    .air_recipe = digest(
        "stwo-rust:a8fcf4bd:examples-plonk:PlonkComponent:v1",
    ),
};

pub const air = pairDigest(
    "stwo/native/plonk/air/bilateral/v1",
    zig_artifact.air_recipe,
    rust_artifact.air_recipe,
);
pub const ingress_recipe = pairDigest(
    "stwo/native/plonk/materialized-trace/bilateral/v1",
    zig_artifact.trace_recipe,
    rust_artifact.trace_recipe,
);
pub const ingress_layout = digest(
    "stwo/native/plonk/trace-layout:m31-column-major:bit-reversed-circle:v1",
);
pub const transcript_recipe = digest(
    "stwo/native/plonk/transcript:pcs-config,preprocessed,main,statement:u32:v1",
);
pub const public_input_abi = digest(
    "stwo/native/plonk/public-input:u32-log-n-rows:v1",
);
pub const sampling_recipe = digest(
    "stwo/native/plonk/oods:referenced-preprocessed-points,main-point,composition-split-points:v1",
);
pub const mask_layout = digest(
    "stwo/native/plonk/mask:preprocessed[4x[0]],main[4x[0]],interaction[],composition[8x[0]]:v1",
);
pub const constraint_parameter_abi = digest(
    "stwo/native/plonk/constraint-abi:statement[4xu32le],alpha[4xm31],total[8xu32]:v1",
);
pub const constraint_expression = pairDigest(
    "stwo/native/plonk/constraint:constant-qm31(log_n_rows,4,1,1):v1",
    zig_artifact.air_recipe,
    rust_artifact.air_recipe,
);

pub fn statement(value: cpu_plonk.Statement) ir.Digest {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("stwo/native/plonk/statement/v1");
