//! Metal proof gate for the first hash-pinned Pokémon checkpoint slice.
...
const VerifierEngine =
    verifier.ProverEngineForBackend(CpuBackend);

const EXPECTED_OBSERVATIONS: usize = 2;
...
comptime {
    ...
    if (ProverEngine.Backend != metal.MetalCommitBackend or
        VerifierEngine.Backend != CpuBackend)
    {
        @compileError(
            "Pokemon cross-backend gate requires Metal proving and CPU preprocessing",
        );
    }
}
...
pub fn main() !void {
    ...
    try validateFixtureCounts(fixture.summary(), input, options.fixture);
    const config = try proofConfig(options.security);
    var output = environment.proveExecution(
        allocator,
        config,
        input,
    ) catch |err| {
        std.debug.print(
            "SM83 Pokemon Metal proof: FAIL stage=prove error={s}\n",
            .{@errorName(err)},
        );
        return err;
    };
    ...
    proof_owned = false;
    verifier.verifyExecutionWithEngine(
        VerifierEngine,
        allocator,
        config,
        input.rom,
        ...
        std.debug.print(
            "SM83 Pokemon Metal proof: FAIL stage=cpu_verify error={s}\n",
            .{@errorName(err)},
        );
        return err;
    };
    std.debug.print(
        "SM83 Pokemon Metal proof: PASS profile={s} fixture_profile={s} " ++
            "security_bits={d} rows={d} mcycles={d} callbacks={d} " ++
            "actions={d} dma_sources={d} apu_events={d} observations={d} " ++
            "verifier_preprocessing=cpu\n",
        .{
            @tagName(options.security),
            @tagName(options.fixture),
            config.securityBits(),
            input.results.len,
            fixture.prefix_mcycles,
            fixture.prefix_instructions,
            input.actions.len,
...
fn validateFixtureCounts(
    ...
    if (summary.rows != input.results.len or
        ...
        input.intermediate_observations.len != EXPECTED_OBSERVATIONS)
    {
        return error.InvalidPokemonFixtureCounts;
    }
}

fn validateStatementCounts(
...
fn rejectObservationMutation(
    ...
    const mutated = try allocator.dupe(
        frontend.air.intermediate_ram_observation_lookup.Sample,
        input.intermediate_observations,
    );
...
test "SM83 Pokemon Metal checkpoint proof defaults to the secure profile" {
    const arguments = [_][]const u8{
        "sm83-pokemon-metal-proof",
        "/path/to/PE-AGI/v1",
    };
...
test "SM83 Pokemon Metal checkpoint smoke profile requires an explicit flag" {
    const arguments = [_][]const u8{
        "sm83-pokemon-metal-proof",
        "/path/to/PE-AGI/v1",
        "--smoke",
...
test "SM83 Pokemon Metal checkpoint proof rejects duplicate and unknown flags" {
    ...
    try std.testing.expectError(
        ...
        parseOptions(&empty_path),
    );

    const flag_as_path = [_][]const u8{
        "sm83-pokemon-metal-proof",
        "--smoke",
    };
    try std.testing.expectError(
        error.InvalidArguments,
        parseOptions(&flag_as_path),
    );
