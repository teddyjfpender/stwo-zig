//! Verifier-owned input boundary for the single common recursive fold.
//!
//! A fold may be prepared only while both child wrapper cold-verifier leases
//! are live.  This owner borrows their actual PCS captures, re-admits both
//! artifacts against the same circuit registry, derives the exact parent
//! `NodePublic`, and seals the ordered dependency identity used by the fold
//! cohort.  It contains no proof bytes and no serializable freshness bit.
...
const IDENTITY_DOMAIN = "stwo-zig/recursive-common-fold-input/v1\x00";
...
/// Borrowed, process-local fold input.  The two wrapper admissions which own
/// these views must outlive this value and the complete fold proof transaction.
pub const FreshFoldInputV1 = struct {
    ...
    child_views: [CHILD_COUNT]authority_mod.FreshWrapperViewV1,
    child_refs: [CHILD_COUNT]artifact_mod.ArtifactRefV1,
    campaign_namespace_sha256: [32]u8,
    registry_identity_sha256: [32]u8,
    parent: authority_mod.ParentNodePublicDerivationV1,
    identity_sha256: [32]u8,

    pub fn init(
        left: authority_mod.FreshWrapperViewV1,
        right: authority_mod.FreshWrapperViewV1,
        parent_coordinate: artifact_mod.TaskCoordinateV1,
        ...
        var result = FreshFoldInputV1{
            .parent_coordinate = parent_coordinate,
            .parent_node_kind = try artifact_mod.expectedNodeKind(
                parent_coordinate,
            ),
    ...
    pub fn validateAgainst(
        ...
        if (self.parent_node_kind != try artifact_mod.expectedNodeKind(
    ...
    pub fn outputNodePublic(
        self: *const FreshFoldInputV1,
    ) *const artifact_mod.NodePublicV1 {
...
fn childCircuit(
    child: authority_mod.FreshWrapperViewV1,
...
fn inputIdentity(value: *const FreshFoldInputV1) [32]u8 {
    ...
    hash.update(&value.parent.node_public.output_identity_sha256);
...
comptime {
    if (FORMAT_VERSION != 1 or SCHEMA_VERSION != 1 or CHILD_COUNT != 2 or
        PRODUCTION_ACTIVATION or SERIALIZABLE_FRESH_INPUT)
    {
        @compileError("common fold input boundary drifted");
    }
}
