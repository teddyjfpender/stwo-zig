...
const detached = @import("recursive_segment_v2_detached_proof.zig");
...
const ProducerAllocator = @import("stwo_prover_engine").tracked_smp_allocator.TrackedSmpAllocator;
...
pub fn produceSegmentsWithEngines(comptime count: usize, comptime NativeEngine: type, comptime OuterEngine: type, allocator: std.mem.Allocator, address_count: usize, options: OptionsFor(count)) !ReceiptFor(count) {
    ...
    var segments = try @import("recursive_segment_v2_memory_workload.zig").materialize(count, allocator, address_count, options.initial_memory_word);
    var owned = true;
...
fn produceChild(
    ...
    var candidate_receipt: ChildCandidate = undefined;
    {
        ...
        var prepared = try ingress.prepareWithProfile(
            NativeEngine,
            producer_allocator,
            result,
            statement,
        ...
        if (comptime NativeEngine != leaf.Engine) {
            ...
            if (try NativeEngine.Backend.supportsFrameworkInteractions() and delta.counters.metal_framework_interaction_dispatches != 24)
                return error.NativeTableInteractionDispatchMissing;
            std.debug.print("SEGMENT_V2_TWO_CHILD_NATIVE_METAL segment={d} dispatches={d} poseidon_commits={d} table_interaction_dispatches={d}\n", .{
                result.segment_index, delta.counters.metalDispatchTotal(), delta.counters.metal_poseidon2_merkle_commits, delta.counters.metal_framework_interaction_dispatches,
            });
        }
...
pub fn checkNativeProfile(comptime NativeEngine: type, allocator: std.mem.Allocator, address_count: usize, profile: ingress.NativeProfile) !void {
    ...
    var prepared = try ingress.prepareWithProfile(NativeEngine, allocator, results[0], statements[0], keys, profile);
    defer prepared.deinit();
