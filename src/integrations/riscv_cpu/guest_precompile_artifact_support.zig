//! Fixed authority shared by the two-process C-009 functional harness.
//!
//! The verifier never accepts PCS parameters or resource policy selected by the
//! artifact. Both executables are compiled against these exact values.
...
pub const proof_artifact = riscv_cpu.poseidon2_proof_artifact;

pub const max_steps: usize = 16;

/// Deliberately functional rather than release-security parameters. This is the
/// same small configuration used by the one-call in-process proof test.
...
/// Explicit rather than default-initialized so a library-default change cannot
/// silently widen one side of the fresh-process boundary.
...
/// Public statement data plus the two buffers borrowed by its I/O slices.
pub const OwnedPublicData = struct {
    ...
    pub fn init(
        ...
        const clock = std.math.cast(u32, run.base.step_count) orelse
            return error.ExecutionClockOutOfRange;
        const input_len = std.math.cast(u32, run.base.input.len) orelse
