...
test "V4 publication reexecutes and cold-adopts a durable prefix" {
    ...
    const fixture = try artifact_v4_support.Fixture.init();
    ...
    var first_owner = try owner_mod.PublicationOwnerV4.initExisting(
        allocator,
        root,
        &first_capture,
        execution,
    ...
    var left_wire = try artifact_v4_support.OwnedWire.init(
        allocator,
        &fixture.leftSource(),
    );
    ...
    var owner = try owner_mod.PublicationOwnerV4.initExisting(
        allocator,
        root,
        &resumed_capture,
        execution,
    ...
    var right_wire = try artifact_v4_support.OwnedWire.init(
        allocator,
        &right_source,
    );
...
test "V4 segment and manifest codecs reject mutations" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expectError(
        error.IncrementalSegmentRefContentMismatchV4,
        publication.decodeSegmentRef(encoded),
    );

    var segments = [_]publication.CommittedSegmentV4{
    ...
    try std.testing.expectError(
        error.IncrementalCaptureManifestContentMismatchV4,
        publication.decodeManifestAlloc(allocator, manifest_bytes),
    );
}

test "V4 command options distinguish create from full-VM resume" {
    const create = try options.OptionsV4.parse(&.{
        "--retained-materialization-result",
        "/tmp/materialization.json",
        "--publication-root-parent",
        "/tmp",
    });
    try std.testing.expectEqual(options.RootModeV4.create_under_parent, create.root_mode);
    const reopened = try options.OptionsV4.parse(&.{
        "--publication-root",
        "/tmp/existing",
        "--retained-materialization-result",
        "/tmp/materialization.json",
    ...
    try std.testing.expectEqual(
        options.RootModeV4.reopen_unsealed,
        reopened.root_mode,
    );
    try std.testing.expectError(error.DuplicateArgument, options.OptionsV4.parse(&.{
        "--publication-root",
        "/tmp/a",
        "--publication-root-parent",
        "/tmp/b",
...
fn executionAuthority(segment_count: u32) publication.ExecutionAuthorityV4 {
    return .{
        .elf = identity(1),
        .input = identity(2),
