//! Local canonical SegmentV2 fixture for the STWIMT03 transport tests.
...
const Cpu = frontend.runner.cpu.Cpu;
    ...
    pub fn rightSource(self: *const Fixture) segment_v2.SourceV2 {
        return .{
            ...
            .completion = .{
                ...
                .value = frontend.air.public_data.CANONICAL_SELF_LOOP_WORD,
                .clock = 0,
            },
...
pub const OwnedWire = struct {
    ...
    words: []M31,
    data: frontend.air.public_data_v2.PublicDataV2,
    ...
    pub fn init(
        ...
        return .{
            .allocator = allocator,
            .words = words,
            .data = try frontend.air.public_data_v2.PublicDataV2.authenticate(words),
        };
    }
...
fn id(label: []const u8) segment_v2.Digest {
    return channel.hashBytes(label, 0x5032_5453); // "P2TS"
}

fn scalarDigest(value: u32) segment_v2.Digest {
