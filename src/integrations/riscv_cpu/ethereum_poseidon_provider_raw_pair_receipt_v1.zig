//! Canonical telemetry for serial-vs-parallel two-ordinal raw provider proofs.
...
pub const schema = "stwo.ethereum.poseidon-provider-raw-pair-hpc-benchmark.v1";
pub const timing_scope = "retained-provider-pair-self-process";
pub const status_fresh = "diagnostic-pair-fresh-verified";
pub const status_nonranking = "diagnostic-pair-fresh-verified-nonranking";
...
pub const Correctness = struct {
    parallel_canonical_proof_bytes_equal: [2]bool,
    parallel_fresh_verified: [2]bool,
    parallel_roots_equal_proof: [2]bool,
    serial_canonical_proof_bytes_equal: [2]bool,
    serial_fresh_verified: [2]bool,
    serial_roots_equal_proof: [2]bool,
    stage_a_serial_parallel_roots_equal: bool,
    statement_identities_equal: [2]bool,
    native_claims_equal: [2]bool,
    ordered_claims_equal: [2]bool,
};

pub const Timings = struct {
    parallel_cold_verify_wall_ns: u64,
    parallel_proof_batch_wall_ns: u64,
...
pub const ProofPair = struct {
    parallel: [2]receipt_v1.FileIdentity,
    serial: [2]receipt_v1.FileIdentity,
};
...
pub const Receipt = struct {
    ...
    parallel_admission: RawAdmission,
    ...
    pub fn validate(self: Receipt) !void {
        if (!std.mem.eql(u8, self.schema, schema) or
            ...
            self.workload.ordinals[0] >= self.workload.ordinals[1] or
            self.workload.ordinals[1] >= self.workload.shard_count or
            ...
            self.timings.parallel_proof_batch_wall_ns == 0 or
            ...
            self.timings.parallel_cold_verify_wall_ns == 0 or
        ...
        {
            return error.InvalidProviderRawPairReceipt;
        }
        try validateAdmission(self.serial_admission, 1);
        try validateAdmission(self.parallel_admission, 2);
        ...
        for (self.proofs.parallel) |proof| try validateFile(proof);
        inline for (.{
            self.content_sha256,
        ...
        ) catch return error.InvalidProviderRawPairReceipt;
        ...
        const correct = all(self.correctness.serial_canonical_proof_bytes_equal) and
            all(self.correctness.parallel_canonical_proof_bytes_equal) and
            all(self.correctness.serial_fresh_verified) and
            all(self.correctness.parallel_fresh_verified) and
            all(self.correctness.serial_roots_equal_proof) and
            all(self.correctness.parallel_roots_equal_proof) and
            all(self.correctness.statement_identities_equal) and
            all(self.correctness.native_claims_equal) and
            all(self.correctness.ordered_claims_equal) and
            self.correctness.stage_a_serial_parallel_roots_equal;
...
fn validateAdmission(value: RawAdmission, expected_jobs: u32) !void {
    ...
    if (admitted.admitted_concurrent_jobs != expected_jobs or
