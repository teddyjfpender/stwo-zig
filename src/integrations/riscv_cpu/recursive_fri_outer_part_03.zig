    ...
    return struct {
        ...
        pub const NativeSegmentCoreV2 = struct {
            ...
            pub fn prepareInteractions(
                ...
                defer audit.deinit();
                const claims: NativeInteractionClaims = if (self.prepared_inputs_v4) |owned| try fillNativeInteraction(
                    allocator,
                    &self.authority,
                    &self.interaction_tree,
                    owned.view().input_values,
                    self.pcsInputs(),
                    &self.invocations,
                    self.prepared_query.value,
                    self.preparedRootWitness(),
                    &self.merkle_paths,
                    self.poseidonBuffersMut(),
                    &self.prepared_relation_rows,
                    provider_relations,
                    ...
                    var arithmetic_storage: [MAX_ARITHMETIC_EVALUATION_LANES]lowering.Evaluation = undefined;
                    const legacy = try fillInteraction(
                        allocator,
                        &self.authority,
                        &self.interaction_tree,
                        self.evaluations(),
