//! Verifier-reconstructible algebraic closure for the role-0 cohort.
//!
//! All 36 row claims are decomposed by their authenticated relation plans.
//! Non-row terms are the native graph's public-wire boundary and the public
//! NodePublic statement boundary independently reconstructed by the verifier.
...
const public_statement_mod =
    @import("recursive_common_ethereum_incremental_leaf_public_statement_boundary_v4.zig");
...
pub const PUBLIC_WIRE_SCHEMA_VERSION: u16 = 3;
pub const PublicStatementBoundaryV4 = public_statement_mod.PublicStatementBoundaryV4;
...
const CLOSURE_IDENTITY_DOMAIN =
    "stwo-zig/common-ethereum-incremental-closure/v4-schema4\x00";

pub const Error = public_statement_mod.Error || error{
...
pub const PublicWireBoundaryV4 = struct {
    ...
    schema_version: u16 = PUBLIC_WIRE_SCHEMA_VERSION,
    ...
    pub fn validate(self: *const PublicWireBoundaryV4) Error!void {
        if (self.format_version != FORMAT_VERSION or
            self.schema_version != PUBLIC_WIRE_SCHEMA_VERSION or
...
pub const ReceiptV4 = struct {
    ...
    public_statement: PublicStatementBoundaryV4,
    ...
    pub fn validate(self: *const ReceiptV4) Error!void {
        ...
        if (self.format_version != FORMAT_VERSION or
            ...
            self.active_domain_mask & domainBit(@intFromEnum(PUBLIC_WIRE_DOMAIN)) == 0 or
            self.active_domain_mask & domainBit(@intFromEnum(public_statement_mod.DOMAIN)) == 0 or
            self.logical_rows == 0 or
...
pub fn auditAndClose(
    ...
    return closeAudits(manifest, claims, &audits, try PublicWireBoundaryV4.derive(native, relations), try PublicStatementBoundaryV4.derive(node_public, relations));
}

/// One finalizer for each explicitly admitted roster. All row audits must be
/// rebuilt from authenticated plans before this boundary; none may be omitted.
pub fn closeAudits(manifest: anytype, claims: anytype, audits: []const relation_interaction.DomainAudit, public_wire: PublicWireBoundaryV4, public_statement: PublicStatementBoundaryV4) !ReceiptV4 {
    try manifest.validate();
    try claims.validate(manifest);
    if (audits.len != manifest.roster_count or audits.len != claims.values.len) return error.EthereumIncrementalClosureMismatchV4;
    ...
    for (audits, claims.values, 0..) |audit, claim, row| {
        try validateRowAudit(row, audit, claim);
    ...
    const statement_index = @intFromEnum(public_statement.domain);
    domain_totals[statement_index] = domain_totals[statement_index].add(public_statement.claimed_sum);
    framework_total = framework_total.add(public_statement.claimed_sum);
    active_domain_mask |= domainBit(statement_index);
...
fn validateRowAudit(row: usize, audit: relation_interaction.DomainAudit, claim: QM31) Error!void {
    ...
    // Statement-input contributions close against other components. The
    // receipt requires both the full sum and every relation domain to close.
}

test "role0 statement input allows its authenticated cross-component claim" {
    const allocator = std.testing.allocator;
    const component = air.statement_input;
    const witness = air.statement_input_witness;
    const interaction_mod = air.statement_input_relation;
    var preprocessing = try witness.Preprocessed.init(allocator);
    ...
    var words = [_]M31{M31.zero()} ** component.CANONICAL_WORD_COUNT;
    words[0] = M31.one();
    const rows = [_]interaction_mod.Row{
        try witness.logicalRow(preprocessing.rows[0], .{ .segment_leaf = &words }),
    };
    const relations = universal.UniversalRelations.dummy();
    var interaction = try plan.generateInteraction(allocator, &definition.arena, component.SEMANTIC_DIGEST, definition.events.ordered(), &rows, 0, &relations);
    defer interaction.deinit(allocator);
    const claim = interaction.claims.total();
    try std.testing.expect(!claim.isZero());
    const audit = try plan.auditPreparedDomainSums(allocator, &rows, &relations, claim);
    try validateRowAudit(10, audit, claim);
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, validateRowAudit(10, audit, claim.add(QM31.one())));
    var mutated = audit;
    mutated.values[@intFromEnum(RelationDomain.recursion_statement_word)] = QM31.zero();
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, validateRowAudit(10, mutated, claim));
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, validateRowAudit(34, audit, claim));
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, validateRowAudit(35, audit, claim));

    var boundary = PublicWireBoundaryV4{
        .term_count = 1,
        .source_authority_identity_sha256 = [_]u8{1} ** 32,
        .claimed_sum = QM31.zero(),
        .identity_sha256 = undefined,
    };
    boundary.identity_sha256 = boundaryIdentity(&boundary);
    var statement_boundary = PublicStatementBoundaryV4{
        .source_authority_identity_sha256 = [_]u8{3} ** 32,
        .claimed_sum = QM31.zero(),
        .identity_sha256 = undefined,
    };
    statement_boundary.identity_sha256 = statement_boundary.identity();
    var receipt = ReceiptV4{
        .manifest_seal = [_]u8{2} ** 32,
        .public_wire = boundary,
        .public_statement = statement_boundary,
        .domain_totals = [_]QM31{QM31.zero()} ** DOMAIN_COUNT,
        .framework_total = QM31.zero(),
        .active_domain_mask = domainBit(@intFromEnum(PUBLIC_WIRE_DOMAIN)) |
            domainBit(@intFromEnum(public_statement_mod.DOMAIN)),
        .logical_rows = 1,
        .event_terms = 1,
        .identity_sha256 = undefined,
    };
    receipt.identity_sha256 = closureIdentity(&receipt);
    try receipt.validate();
    receipt.domain_totals[@intFromEnum(RelationDomain.recursion_statement_word)] = claim;
    receipt.domain_totals[@intFromEnum(RelationDomain.recursion_verifier_input_word)] = claim.neg();
    receipt.identity_sha256 = closureIdentity(&receipt);
    try std.testing.expectError(error.EthereumIncrementalRelationNotClosedV4, receipt.validate());
    receipt.domain_totals = [_]QM31{QM31.zero()} ** DOMAIN_COUNT;
    receipt.framework_total = claim;
    receipt.identity_sha256 = closureIdentity(&receipt);
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, receipt.validate());
}
...
comptime {
    if (FORMAT_VERSION != 4 or SCHEMA_VERSION != 4 or PUBLIC_WIRE_SCHEMA_VERSION != 3 or
...
test "Ethereum initial complete closure requires all38 audits and every domain to cancel" {
    const initial = air.ethereum_initial_input_manifest_v1;
    ...
    const manifest = try initial.build(&ordinary, 40);
    var claims = try initial.ClaimVector.init(&manifest);
    for (0..38) |row| try claims.bind(@enumFromInt(row), if (row == 36) QM31.one() else if (row == 37) QM31.one().neg() else QM31.zero());
    try claims.sealClaims(&manifest);
    var audits = [_]relation_interaction.DomainAudit{.{ .values = @splat(QM31.zero()), .total = QM31.zero(), .logical_rows = 1, .event_terms = 1 }} ** 38;
    const wire = @intFromEnum(RelationDomain.recursion_wire);
    audits[36].values[wire] = QM31.one();
    audits[36].total = QM31.one();
    audits[37].values[wire] = QM31.one().neg();
    audits[37].total = QM31.one().neg();
    var boundary = PublicWireBoundaryV4{ .term_count = 1, .source_authority_identity_sha256 = @splat(1), .claimed_sum = QM31.zero(), .identity_sha256 = undefined };
    boundary.identity_sha256 = boundaryIdentity(&boundary);
    var statement = PublicStatementBoundaryV4{ .source_authority_identity_sha256 = @splat(3), .claimed_sum = QM31.zero(), .identity_sha256 = undefined };
    statement.identity_sha256 = statement.identity();
    const receipt = try closeAudits(&manifest, &claims, &audits, boundary, statement);
    try receipt.validate();
    try std.testing.expectError(error.EthereumIncrementalClosureMismatchV4, closeAudits(&manifest, &claims, audits[0..36], boundary, statement));
    // The scalar sum remains zero, but moving the packet obligation to a
    // different domain must fail the same per-domain check used by ordinary36.
    audits[37].values[wire] = QM31.zero();
    audits[37].values[@intFromEnum(RelationDomain.recursion_verifier_input_word)] = QM31.one().neg();
    try std.testing.expectError(error.EthereumIncrementalRelationNotClosedV4, closeAudits(&manifest, &claims, &audits, boundary, statement));
}
