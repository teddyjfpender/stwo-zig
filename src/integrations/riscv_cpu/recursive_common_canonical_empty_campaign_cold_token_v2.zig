//! Campaign canonical-empty projection into the generic process-local token wire.
...
pub const Error = error{
    CampaignCanonicalEmptyColdTokenMismatch,
};
...
pub fn validateConstructed(
    ...
    if (value.fresh.capture.commitments.len == 0)
        return error.CampaignCanonicalEmptyColdTokenMismatch;
...
pub fn snapshot(value: anytype) !process_validation.SnapshotV1 {
    ...
    const node_public_bytes = try campaign_public.encodeCanonical(
        &value.source_input.shape,
        &value.node_public,
    );
...
fn validationAuthorityKind() [32]u8 {
    ...
    Sha256.hash(
        "stwo-zig/campaign-canonical-empty-validated-cold-proof/v2\x00",
        &result,
        .{},
    );
    return result;
...
fn querySnapshotIdentity(value: anytype) [32]u8 {
    var hash = Sha256.init(.{});
    hash.update("stwo-zig/campaign-canonical-empty-query-snapshot/v2\x00");
