//! Exact universal-tree materializer for the field-native empty wrapper.
//!
...
pub fn fillMain(
    ...
    try manifest_mod.validateExact(manifest);
    try schedule.validateAgainst(
        schedule.node_public.statement_words,
        schedule.node_public.coordinate,
    );
    try preflightFreshTree(
        manifest,
...
comptime {
    if (FORMAT_VERSION != 2 or SCHEMA_VERSION != 1 or
        field_public.POSEIDON_CALL_COUNT != 113 or
        manifest_mod.POSEIDON_LOG_SIZE != 7 or PRODUCTION_ACTIVATION)
    {
        @compileError("canonical-empty universal trace V2 drifted");
    }
}
