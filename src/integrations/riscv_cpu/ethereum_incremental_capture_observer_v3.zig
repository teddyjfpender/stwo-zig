...
//!
//! One execution simultaneously mints STWEMT01 and STWIMT03.  Retained
...
pub const ObserverV3 = struct {
    ...
    owner: ?publication_owner.PublicationOwnerV3 = null,
    ...
    pub fn deinit(self: *ObserverV3) void {
    ...
    fn beginRetirementSegment(context: *anyopaque, segment_index: u32) !void {
        const self: *ObserverV3 = @ptrCast(@alignCast(context));
    ...
    pub fn observe(
        ...
        if (comptime profile != .rv32im_zkvm_ethereum_v1) {
            ...
            const live_metadata = try global_source.metadata();
            if (!std.meta.eql(live_metadata, retained_source.value.metadata))
                return error.RetainedIncrementalSourceMismatch;
            const projection = try projection_v3.ProjectionV3.init(&global_source);
            ...
            const local_wire = try support.encodeLocalPublicData(
                self.allocator,
                &local_source,
            );
            ...
            if (self.capture == null) {
                ...
                self.owner = try publication_owner.PublicationOwnerV3
                    .initExisting(
                    self.allocator,
                    self.root,
                    &self.capture.?,
            ...
            _ = try self.owner.?.captureAndPublish(
                index,
                &segment.rw_memory,
                touched,
                live_metadata.entry.continuation_root,
    ...
    pub fn validateComplete(self: *const ObserverV3) !void {
    ...
    pub fn programIdentity(self: *const ObserverV3) ![32]u8 {
