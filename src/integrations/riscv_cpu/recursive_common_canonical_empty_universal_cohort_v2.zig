//! Secure universal cohort for one field-native canonical-empty wrapper.
//!
//! NodePublicV2 is mixed word-for-word before relation challenges.  The
//! existing Poseidon2 AIR proves the exact 113 permutations.  Its positive
...
const trace_mod =
    @import("recursive_common_canonical_empty_universal_trace_v2.zig");
...
const GENERATED_IDENTITY_DOMAIN =
    "stwo-zig/common-canonical-empty-universal-interactions/v2\x00";
const AUDIT_IDENTITY_DOMAIN =
    "stwo-zig/common-canonical-empty-universal-audit/v2\x00";
const BOUNDARY_IDENTITY_DOMAIN =
    "stwo-zig/common-canonical-empty-universal-boundary/v2\x00";
...
pub const BoundaryEvidenceV2 = struct {
    ...
    pub fn zero(domain: RelationDomain) BoundaryEvidenceV2 {
        ...
        var sealed = result;
        sealed.tuple_provenance_sha256 = boundaryIdentity(&sealed);
        return sealed;
    }
...
pub const CohortV2 = struct {
    ...
    pub fn init(
        ...
        var statement_words_m31: recursion.span_statement.StatementWords =
            undefined;
        for (&statement_words_m31, inputs.statement_words) |*target, word|
    ...
    pub fn validate(self: *CohortV2) !void {
        ...
        try self.schedule.validateAgainst(
            self.inputs.statement_words,
            self.inputs.coordinate,
        );
        if (self.logical_initialized != catalog.LOGICAL_COUNT or
            !std.meta.eql(
                self.schedule.node_public.statement_words,
                self.inputs.statement_words,
            ) or !std.meta.eql(
            self.schedule.node_public.coordinate,
            self.inputs.coordinate,
        )) return error.CanonicalEmptyCohortAuthorityMismatch;
        for (self.statement_words_m31, self.inputs.statement_words) |felt, word|
            if (felt.toU32() != word)
                return error.CanonicalEmptyCohortAuthorityMismatch;
        inline for (catalog.LOGICAL_ROWS, 0..) |entry, index| {
    ...
    pub fn recursiveStatementWords(
        ...
        try self.schedule.node_public.validateLeafSource(
    ...
    pub fn sessionAuthority(
        self: *const CohortV2,
    ) !secure_artifact.CanonicalEmptySessionAuthorityV1 {
        return .{
    ...
    pub fn validateGenerated(
        ...
        if (!generatedEql(generated_value, &expected))
            return error.CanonicalEmptyGeneratedInteractionMismatch;
    }
    ...
    pub fn auditGlobalClosureV2(
        ...
        wire.tuple_provenance_sha256 = boundaryIdentity(&wire);
...
fn generatedEql(
    left: *const GeneratedInteractionsV1,
    right: *const GeneratedInteractionsV1,
) bool {
...
fn cohortIdentity(value: *const CohortV2) [32]u8 {
...
fn relationsIdentity(value: *const universal.UniversalRelations) [32]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
...
fn generatedIdentity(value: *const GeneratedInteractionsV1) [32]u8 {
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
...
comptime {
    ...
    {
        @compileError("canonical-empty universal cohort V2 drifted");
    }
}
