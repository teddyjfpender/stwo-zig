...
pub fn updatesForSegments(comptime count: usize) usize {
    if (count != 1 and count != 2 and count != 4 and count != 8) @compileError("memory segment ladder is 1,2,4,8");
...
fn validateSegmentWithCompletion(
    ...
    // A completed one-segment job can be shorter than one address sweep.
    // Loads start at step 2 and recur every three retired instructions.
    if (first_step == 0) try std.testing.expectEqual(@min(address_count, cumulative_steps / 3), std.mem.count(bool, &touched, &.{true}));
    std.debug.print(
