//! Verifier-rerecorded composition capture for the field-native common fold.
//!
//! The q193 proof is cold verified by the common-fold secure kernel first.
//! This owner then reconstructs the exact dynamic universal-36 manifest,
//! relations, claims, fixed-wire components, row-34 Poseidon provider, and
//! global closure.  The returned graph is process-local and has no codec.

const std = @import("std");
const stwo_core = @import("stwo_core");
const frontend = @import("stwo_riscv_frontend");

const canonical_graph_abi =
...
const CAPTURE_DOMAIN =
    "stwo-zig/common-fold-composition-capture/v2\x00";

/// Temporary neutral ABI bridge. The type contains only a borrowed lane and
/// evaluation; no canonical-empty session, role, or authority is imported.
pub const FreshGraphViewV2 = canonical_graph_abi.FreshGraphViewV2;

pub fn Types(comptime dimensions: recursion.fixed_wire.Dimensions) type {
    const Graph = graph_mod.Types(dimensions);
    return TypesForGraph(dimensions, Graph);
}

pub fn TypesForGraph(
    comptime dimensions: recursion.fixed_wire.Dimensions,
    comptime Graph: type,
) type {
    dimensions.validate();
    const Cohort = Graph.CohortV2;
    const VerifiedReplay = Graph.VerifiedReplay;

    return struct {
        const Self = @This();

        pub const KernelV2 = Graph.KernelV2;
        pub const SecureCohortV2 = Cohort;
        pub const VerifiedReplayV2 = VerifiedReplay;

        /// Process-local transcript authority retained by the cold common-fold
        /// proof owner. It deliberately has no codec: every extraction is
        /// checked against a newly verifier-replayed transcript first.
        pub const VerifierQueryAuthorityV2 = struct {
            ...
            pub fn validateAgainstReplay(
                ...
                if (!m31SliceEql(&self.query_words, &replay.query_words) or
                    ...
                    )) return error.InvalidCommonFoldQueryAuthority;
            }
        };
        ...
        pub const CaptureV2 = struct {
            ...
            claim_inputs: Graph.ClaimInputsV2,
            ...
            /// Replays the complete cohort before borrowing graph memory.
            pub fn revalidateAndView(
                ...
                try query_authority.validateAgainstReplay(replay);
                return .{
            ...
            /// Cheap view borrow after the exact cold owner minted a
            /// nonserializable token. No transcript replay, graph rerecord,
            /// or graph evaluation occurs on this path.
            ...
            pub fn validateProcessLocalClosure(
                ...
                if (self.format_version != FORMAT_VERSION or
                    ...
                    &self.circuit.identity_digest,
                ) or snapshot.graph_node_ptr !=
                    @intFromPtr(self.circuit.nodes.ptr) or
                    snapshot.graph_node_len != self.circuit.nodes.len or
                    snapshot.graph_binding_ptr !=
                        @intFromPtr(self.bindings.ptr) or
                    snapshot.graph_binding_len != self.bindings.len or
                    snapshot.graph_input_ptr !=
                        @intFromPtr(self.input_values.ptr) or
                    snapshot.graph_input_len != self.input_values.len or
                    snapshot.graph_output_ptr !=
                        @intFromPtr(self.node_values.ptr) or
            ...
            /// Hostile-mutation audit for tests and explicit diagnostics.
            /// Normal view extraction uses the pointer/identity closure above.
            pub fn validateProcessLocalRetained(
                self: *const CaptureV2,
            ) !void {
                try self.validateRetained();
            }
        ...
        fn validateTransaction(
            ...
            if (session.source_kind != .common_fold_field_v2 or
                !std.meta.eql(
        ...
        fn captureIdentity(value: *const CaptureV2) [32]u8 {
            ...
            hashInt(
                &hash,
                u8,
                @intFromBool(value.production_activation),
            );
