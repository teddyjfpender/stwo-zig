...
//! and STWIMT03 from the same live segments.
...
pub const OwnedSourceV3 = struct {
    bytes: []u8,
    value: source_wire.Source,
    identity: publication.ArtifactIdentityV3,
};
...
pub const RetainedAuthorityV3 = struct {
    allocator: std.mem.Allocator,
    materialization_bytes: []u8,
    materialization: std.json.Parsed(contract.MaterializationResult),
    materialization_identity: publication.ArtifactIdentityV3,
    source_request_bytes: []u8,
    source_authority: SourceAuthority,
    source_request_identity: publication.ArtifactIdentityV3,
    journal_bytes: []u8,
    journal_identity: publication.ArtifactIdentityV3,
    journal_records: [][32]u8,
    elf_bytes: []u8,
    elf_identity: publication.ArtifactIdentityV3,
    input_bytes: []u8,
    input_identity: publication.ArtifactIdentityV3,
    output_bytes: []u8,
    output_identity: publication.ArtifactIdentityV3,
    sources: []OwnedSourceV3,
    ...
    pub fn open(
        ...
        for (
            ...
            destination.* = .{
                .bytes = bytes,
                .value = source,
                .identity = publication.ArtifactIdentityV3.fromBytes(bytes),
            };
            opened_count += 1;
        }
        ...
        return .{
            ...
            .materialization_identity = publication.ArtifactIdentityV3.fromBytes(
                materialization_bytes,
            ),
            .source_request_bytes = source_request_bytes,
            .source_authority = source_authority,
            .source_request_identity = publication.ArtifactIdentityV3.fromBytes(
                source_request_bytes,
            ),
            .journal_bytes = journal_bytes,
            .journal_identity = publication.ArtifactIdentityV3.fromBytes(
                journal_bytes,
            ),
            .journal_records = records,
            .elf_bytes = elf_bytes,
            .elf_identity = publication.ArtifactIdentityV3.fromBytes(elf_bytes),
            .input_bytes = input_bytes,
            .input_identity = publication.ArtifactIdentityV3.fromBytes(input_bytes),
            .output_bytes = output_bytes,
            .output_identity = publication.ArtifactIdentityV3.fromBytes(output_bytes),
            .sources = sources,
        };
    }

    pub fn deinit(self: *RetainedAuthorityV3) void {
    ...
    pub fn executionAuthority(
        self: *const RetainedAuthorityV3,
    ) !publication.ExecutionAuthorityV3 {
        const value = publication.ExecutionAuthorityV3{
    ...
    pub fn elfEvidence(self: *const RetainedAuthorityV3) evidence.FileIdentity {
        return evidence.identity(self.source_authority.elf().path, self.elf_bytes);
    }

    pub fn inputEvidence(self: *const RetainedAuthorityV3) evidence.FileIdentity {
    ...
    pub fn outputEvidence(self: *const RetainedAuthorityV3) evidence.FileIdentity {
        return evidence.identity(
    ...
    pub fn journalEvidence(self: *const RetainedAuthorityV3) evidence.FileIdentity {
        return evidence.identity(
