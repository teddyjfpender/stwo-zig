...
const artifact = @import("ethereum_incremental_boundary_artifact_v4.zig");
...
const left_input_words = [_]u32{ 11, 0 };

test "STWIMT04 round trips and coldly derives touched and sparse-zero public inputs" {
    ...
    authority_v1.testing.resetValidationCallCount();
    ...
    try std.testing.expectEqual(
        @as(u64, 1),
        authority_v1.testing.validationCallCount(),
    );
    try std.testing.expectEqual(@as(usize, 2), cold.sources.len);
    ...
    try std.testing.expectEqual(@as(u32, 0x2004), cold.sources[1].word.addr);
    try std.testing.expectEqual(@as(u32, 0), cold.sources[1].entry_clock);
    try std.testing.expectEqual(@as(u32, 0), cold.sources[1].word.final_clock);
    try std.testing.expect(cold.sources[1].word.role.is_public_input);
    var rows: [4]boundary_v4.BoundaryRowContractV4 = undefined;
    try boundary_v4.writeBoundaryRows(case.authority(), cold.transitions, &rows);
    try std.testing.expectEqual(
        boundary_v4.MemoryMultiplicityV4.exit,
        rows[3].memory_multiplicity,
    );
...
test "STWIMT02 sparse parents retain exact topology across cold reopen" {
    const allocator = std.testing.allocator;
    const word_count = 17;
    var state: [word_count]authority_v1.SparseWordV1 = undefined;
    var touched: [word_count]authority_v1.TouchedWordV1 = undefined;
    for (&state, &touched, 0..) |*word, *transition, index| {
        const address: u32 = 0x1000 + @as(u32, @intCast(index)) * 0x10000;
        const value: u32 = @intCast(index + 1);
        word.* = .{ .address = address, .value = value };
        transition.* = .{
            .address = address,
            .old_word = value,
            .new_word = value,
    ...
    const root = try artifact_v2.testing.fullRoot(allocator, &state);
    ...
    var transition = try session.apply(0, &touched);
    defer transition.deinit();
    const encoded = try artifact_v2.encodeAlloc(
        allocator,
        &transition,
        artifact_v2.default_limits,
    );
    defer allocator.free(encoded);
    var reopened = try artifact_v2.decodeAlloc(
        allocator,
        encoded,
        artifact_v2.default_limits,
    );
    defer reopened.deinit();
    const roundtrip = try artifact_v2.encodeAlloc(
        allocator,
        &reopened.authority,
        artifact_v2.default_limits,
    );
    ...
    try std.testing.expectEqual(root, reopened.authority.entry_root);
    try std.testing.expectEqual(root, reopened.authority.exit_root);
}

test "STWIMT04 binds policy 2 and rejects resealed reserved bytes" {
    const allocator = std.testing.allocator;
    var case = try Case.init(allocator, .left);
    defer case.deinit();
    ...
    try std.testing.expectError(
        error.IncrementalBoundaryArtifactV4ContentMismatch,
        artifact.decodeAlloc(allocator, changed, artifact.default_limits),
    );
    changed[artifact.testing.ENTRY_CLOCKS_OFFSET] ^= 1;
    changed[artifact.testing.POLICY_OFFSET] = 1;
    try artifact.testing.reseal(changed);
    try std.testing.expectError(
        error.InvalidIncrementalBoundaryArtifactV4Header,
        artifact.decodeAlloc(allocator, changed, artifact.default_limits),
    );
    changed[artifact.testing.POLICY_OFFSET] = 2;
...
test "STWIMT04 rejects a resealed entry-clock mutation" {
    const allocator = std.testing.allocator;
    var case = try Case.init(allocator, .left);
    defer case.deinit();
    const changed = try allocator.dupe(u8, case.artifact_bytes);
    ...
    defer decoded.deinit();
    try std.testing.expectError(
        error.IncrementalBoundaryArtifactV4EntryClockMismatch,
...
test "STWIMT04 binds ordered clocks to the V2 touched-word order" {
    const allocator = std.testing.allocator;
...
test "STWIMT04 rejects public-wire identity and backing-word mutations" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expectError(
        error.IncrementalBoundaryArtifactV4PublicWireMismatch,
        artifact.coldReconstruct(
            allocator,
            &decoded,
            &right.wire.data,
...
test "STWIMT04 rejects resealed root drift against nested STWIMT02" {
    const allocator = std.testing.allocator;
    var case = try Case.init(allocator, .left);
    defer case.deinit();
    const changed = try allocator.dupe(u8, case.artifact_bytes);
    ...
    try artifact.testing.reseal(changed);
    try std.testing.expectError(
        error.IncrementalBoundaryArtifactV4TransitionMismatch,
        artifact.decodeAlloc(allocator, changed, artifact.default_limits),
    );
}

test "STWIMT04 derives public values and rejects caller-side raw IO drift" {
    const allocator = std.testing.allocator;
    var case = try Case.init(allocator, .left);
    defer case.deinit();
    var decoded = try artifact.decodeAlloc(
    ...
    defer decoded.deinit();
    const changed_input = [_]u32{ 12, 0 };
...
test "STWIMT04 sparse-zero merge rejects nonzero ABI clock and value drift" {
    const allocator = std.testing.allocator;
    var case = try Case.init(allocator, .left);
    defer case.deinit();
    var decoded = try artifact.decodeAlloc(
    ...
    const changed_input = [_]u32{ 11, 1 };
    var changed_data = case.public_data;
    changed_data.io_entries.input_words = &changed_input;
    var changed_authority = case.authority();
    changed_authority.public_data = &changed_data;
    try std.testing.expectError(
        error.IncrementalBoundaryArtifactV4InventoryMismatch,
        artifact.coldReconstruct(
            allocator,
            &decoded,
            &case.wire.data,
    ...
    decoded.entry_clocks[1] = 1;
    try std.testing.expectError(
        error.InvalidIncrementalBoundaryArtifactV4EntryClock,
        artifact.coldReconstruct(
            allocator,
            &decoded,
            &case.wire.data,
...
const Case = struct {
    ...
    fn init(allocator: std.mem.Allocator, side: Side) !Case {
        ...
        const retained_public_data = publicData(side, metadata);
        const retained_layout = layout(side);
    ...
    fn authority(self: *const Case) boundary_v4.SegmentPublicAuthorityV4 {
...
fn layout(side: Side) memory_state.MemoryLayout {
    return .{
        ...
        .input_end = if (side == .left) 0x2008 else 0x2080,
        .output_len_addr = 0x2100,
        .output_data_addr = 0x2104,
        .output_base = 0x2100,
        .output_end = 0x2200,
