//! Proof-level acceptance for the degree-five Poseidon2 materialization.
//!
//! This target is intentionally separate from the production 445-column
//! narrow-memory provider.  It proves that the append-only 239-column AIR can
//! traverse the complete PCS/FRI path with its genuine `log_n + 2` quotient
...
const component_mod = frontend.air.typed_poseidon2_degree5_component;
const trace_mod = frontend.air.typed_poseidon2_degree5_trace;
...
test "degree-five Poseidon candidate proves and cold fresh-verifies at quotient plus two" {
    const allocator = std.testing.allocator;
    const calls = try callsFixture(allocator, 13);
    defer allocator.free(calls);

    var prover_candidate = try candidate_mod.Candidate.init(allocator, .degree5);
    ...
    var verifier_candidate = try candidate_mod.Candidate.init(allocator, .degree5);
    ...
    try std.testing.expectEqual(@as(u32, 6), 4 + component_mod.QUOTIENT_EXPANSION_BITS);
    try std.testing.expectEqual(@as(usize, 239), component_mod.MAIN_COLUMNS);
    try std.testing.expectEqual(@as(usize, 8), component_mod.INTERACTION_COLUMNS);
    try std.testing.expectEqual(@as(usize, 227), component_mod.DIRECT_CONSTRAINTS);
    try std.testing.expect(!component_mod.PRODUCTION_ACTIVATION);
}

test "degree-five Poseidon candidate recomputes discarded coefficients with exact proof parity" {
    const allocator = std.testing.allocator;
    const calls = try callsFixture(allocator, 13);
    defer allocator.free(calls);
...
test "degree-five Poseidon log16 retained-call A B benchmark" {
    ...
    const degree5_a = try benchmarkDegree5Arm(allocator, &candidate, calls, log_size);
    const degree5_b = try benchmarkDegree5Arm(allocator, &candidate, calls, log_size);
    ...
    const degree5_retained = try benchmarkDegree5ArmWithRetention(
        allocator,
        &candidate,
        calls,
        log_size,
    ...
    try std.testing.expectEqualSlices(u8, &degree5_a.proof_sha256, &degree5_b.proof_sha256);

    std.debug.print(
        "{{\"schema\":\"stwo.riscv.poseidon-degree5-ab.v1\",\"production\":false,\"log_size\":{d},\"call_count\":{d},\"engine_workers\":{d},\"legacy_main_columns\":445,\"degree5_main_columns\":239,\"legacy_never\":[{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}},{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}}],\"degree5_never\":[{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}},{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}}],\"legacy_always\":{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}},\"degree5_always\":{{\"prove_ns\":{d},\"verify_ns\":{d},\"proof_bytes\":{d}}}}}\n",
        .{
            log_size,
            call_count,
            engine_workers,
            ...
            degree5_a.prove_ns,
            degree5_a.verify_ns,
            degree5_a.proof_bytes,
            degree5_b.prove_ns,
            degree5_b.verify_ns,
            degree5_b.proof_bytes,
            legacy_retained.prove_ns,
            legacy_retained.verify_ns,
            legacy_retained.proof_bytes,
            degree5_retained.prove_ns,
            degree5_retained.verify_ns,
            degree5_retained.proof_bytes,
        },
    );
    printPreparedTelemetry("degree5_never_a", degree5_a.prepared.?);
    printPreparedTelemetry("degree5_never_b", degree5_b.prepared.?);
    printPreparedTelemetry("degree5_always", degree5_retained.prepared.?);
}
...
const BenchmarkSample = struct {
    ...
    proof_bytes: usize,
    proof_sha256: [32]u8,
    prepared: ?component_mod.PreparedTelemetrySnapshot,
};
...
fn benchmarkDegree5Arm(
    allocator: std.mem.Allocator,
    candidate: *const candidate_mod.Candidate,
    calls: []const poseidon2_air.Call,
    log_size: u32,
) !BenchmarkSample {
    return benchmarkDegree5ArmWithRetention(
        allocator,
        candidate,
        calls,
        log_size,
...
fn benchmarkDegree5ArmWithRetention(
    allocator: std.mem.Allocator,
    candidate: *const candidate_mod.Candidate,
    calls: []const poseidon2_air.Call,
    log_size: u32,
    ...
    component_mod.resetPreparedTelemetry();
    var timer = try std.time.Timer.start();
    var output = try prove(
        Engine,
        allocator,
    ...
    defer output.proof.deinit(allocator);
    const prove_ns = timer.read();
    const prepared = component_mod.preparedTelemetrySnapshot();
    try std.testing.expectEqual(@as(u64, 1), prepared.prepare_calls);
    try std.testing.expectEqual(@as(u64, 1), prepared.run_calls);
    var encoded: std.ArrayList(u8) = .{};
    defer encoded.deinit(allocator);
    try postcard.serializeProof(Engine.Hasher, encoded.writer(allocator), output.proof);
    var proof_sha256: [32]u8 = undefined;
...
fn printPreparedTelemetry(
    arm: []const u8,
    value: component_mod.PreparedTelemetrySnapshot,
) void {
    std.debug.print(
        "{{\"schema\":\"stwo.riscv.poseidon-degree5-prepared-telemetry.v1\",\"arm\":\"{s}\",\"prepare_ns\":{d},\"layout_ns\":{d},\"source_stage_ns\":{d},\"twiddle_ns\":{d},\"retained_extension_ns\":{d},\"recomputed_extension_ns\":{d},\"finalize_ns\":{d},\"row_evaluation_ns\":{d},\"source_columns\":{d},\"borrowed_columns\":{d},\"retained_columns\":{d},\"recomputed_columns\":{d},\"evaluated_rows\":{d}}}\n",
        .{
            arm,
            value.prepareNs(),
            value.layout_ns,
            value.source_stage_ns,
            value.twiddle_ns,
            value.retained_extension_ns,
            value.recomputed_extension_ns,
            value.finalize_ns,
            value.row_evaluation_ns,
            value.source_columns,
            value.borrowed_columns,
            value.retained_columns,
            value.recomputed_columns,
            value.evaluated_rows,
        },
    );
}
...
comptime {
    ...
    {
        @compileError("degree-five proof test geometry drifted");
    }
}
