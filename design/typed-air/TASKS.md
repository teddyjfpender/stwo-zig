...
| R-006 | P1 | Thread-count and workload scaling study | R-005 | verified 1/N-worker sweep and resource disclosure | done with explicit no-verdict boundary (the independently replayed append-compatible prefix validates 1,754 retained attempts and scores 1,600 attempts in 24 complete CPU/Metal cells across `multi_shard_addi`, `memcpy_loop`, and balanced Poseidon2. Every scored attempt has complete exact-work disclosure, every same-cell result is deterministic, and failures are zero. A/A calibration and the two-qualifying-workload gate do not close, so the result is `NO_VERDICT_PARTIAL_FROZEN_MATRIX`, not an M7 promotion receipt. The capture remains resumable with 326 attempts unexecuted if a later promotion decision requires the full frozen matrix. M6's balanced-4096 row remains unchanged and open pending segmented/recursive proving) |
